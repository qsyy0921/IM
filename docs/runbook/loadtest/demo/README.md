...
`loadtest/demo` 和 `loadtest/demo/run-local-demo.ps1` 默认仍使用 plaintext gRPC。若外部启动的 delivery-service / receipt-service 已开启 gRPC TLS 或 mTLS，可给 demo runner 传入对应 client 参数：
...
这些参数只覆盖 demo runner 到 delivery / receipt gRPC server 的静态 TLS / mTLS 连接。未配置时保持 plaintext，兼容现有本地演示；证书生命周期和全服务 mTLS rollout 不在 demo runner 范围内。
