...
- 2026-06-10：已新增 contacts-service SDD v0.1 Draft，作为第三层联系人 / 好友关系事实源。设计刻意和 conversation-service 解耦：好友关系不写 `conversation_members`，接受好友不自动创建会话，后续如需创建 direct conversation 必须通过显式 app port 或 saga；第一阶段只做好友申请、接受 / 拒绝、联系人列表和 contacts outbox。当前已完成 `contacts_service.proto`、`im.contact.events` Kafka schema、`000001_contacts_core.sql`、生成代码、contacts-service 六层骨架、noop/grpc 入口和 PostgreSQL repository 真实事务；本轮保持低耦合和复杂度控制，不改 message/conversation/delivery/push 的内部表或同步依赖。真实 PG 集成测试覆盖申请、接受、拒绝、列表分页、幂等 replay、并发首次申请和终态相反 decision。下一步做 contacts outbox relay、Kafka `im.contact.events` publish path 和最小真实进程 smoke。
