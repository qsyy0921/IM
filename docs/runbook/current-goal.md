...
5. push-gateway 后续优先补 slow-client 真实进程负向 smoke、基础 metrics 和 Redis route；queue-full active close 与单实例 resume buffer 已有单元 / 集成测试，但还没有跨实例 route / TTL / 真实慢客户端验证；不要把当前 in-memory `all` 模式表述为多实例生产方案。
