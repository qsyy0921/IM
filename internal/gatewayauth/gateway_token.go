...
type RevocationChecker interface {
	IsRevoked(context.Context, AuthContext) (bool, error)
}
...
func NewAuthExpired(reason string) error {
	...
	return errors.Join(ErrAuthExpired, errors.New(reason))
}
...
func (authenticator *Authenticator) Authenticate(request *http.Request) (AuthContext, error) {
	...
	case ModeJWT:
		return authenticator.authenticateSignedToken(request)
	default:
		return AuthContext{}, ErrPermissionDenied
	}
}

func (authenticator *Authenticator) authenticateSignedToken(request *http.Request) (AuthContext, error) {
...
func (authenticator *Authenticator) parseToken(token string) (tokenClaims, error) {
	...
	case 2:
		if authenticator.mode != ModeHMAC {
			return tokenClaims{}, ErrPermissionDenied
		}
...
func (authenticator *Authenticator) parseClaims(payloadBytes []byte) (tokenClaims, error) {
	...
	if claims.Expires <= 0 || authenticator.now().Unix() >= claims.Expires {
		return tokenClaims{}, NewAuthExpired("token expired")
	}
	return claims, nil
}
...
func authenticateMock(request *http.Request) (AuthContext, error) {
	query := request.URL.Query()
	auth := AuthContext{
	...
	if auth.TenantID == "" {
		return AuthContext{}, NewInvalidRequest("tenant_id is required")
	}
	if auth.UserID == "" {
		return AuthContext{}, NewInvalidRequest("user_id is required")
	}
	return auth, nil
}
