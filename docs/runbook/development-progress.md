...
- `admin-service` product-active：SDD v0.1 和 stage-switch review 已通过，
  ...
  `POLICY_RULESET_REF` 配置引用；本地多进程 config publish / rollback /
  tenant quota / policy ruleset smoke 已通过；first-stage
  `compensation-request` 本地 operator 已支持把 `FAILED` operation 标记为
  `COMPENSATION_REQUESTED` 并写低敏 compensation-requested outbox；
