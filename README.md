...
| 工作流 / 审批 | workflow-service 内部状态机、compensation instruction registry、external callback delivery persistent worker + redrive operator first path、compensation execution readiness / invocation / result manifest | Temporal、Cadence、Argo Workflows | 审批和补偿审计归 workflow / audit 所有；callback worker / redrive / compensation invocation / result 都只处理低敏 refs，不记录 decision、不执行 target。 |
