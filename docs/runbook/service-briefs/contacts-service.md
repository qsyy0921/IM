...
- 已有好友申请、申请来源 metadata、列表 / 搜索 / 分组过滤、接受、拒绝、取消、删除、拉黑、解除拉黑、备注、联系人分组、联系人申请隐私开关；新建申请会被双向 `BLOCKED` 关系、租户级来源策略、目标用户 / 租户默认隐私 gate；目标用户可在总体允许好友申请的前提下单独拒绝 `SEARCH` 来源的陌生申请；申请 `source_ref` 只允许低敏业务引用，types 层已 fail-fast 拒绝邮箱、手机号样式、token / password / secret / bearer 等认证敏感字段和控制字符，不放原始 invite token、手机号 / 邮箱或认证 secret；联系人搜索覆盖 `contact_user_id / remark / group_name`；联系人申请隐私支持 `USER -> TENANT_DEFAULT -> SYSTEM_DEFAULT` 三层读取，并提供租户默认值 / 来源策略本地 operator mode。
...
- 已补只读 `outbox-audit`、只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式，可直接审计当前 `contacts_outbox`，并按 retention 清理 contacts outbox repair 历史；`outbox-audit` / `outbox-repair-audit` / `outbox-repair-cleanup` 可通过 `NEXUSIM_CONTACTS_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_CONTACTS_OUTBOX_REPAIR_AUDIT_OUTPUT` / `NEXUSIM_CONTACTS_OUTBOX_REPAIR_CLEANUP_OUTPUT` 写低敏 JSON 结果或 cleanup summary；租户隐私默认值 / 来源策略 audit 可通过 `NEXUSIM_CONTACTS_TENANT_PRIVACY_AUDIT_OUTPUT` / `NEXUSIM_CONTACTS_SOURCE_POLICY_AUDIT_OUTPUT` 写低敏策略 JSON，便于 operator 留存证据。
