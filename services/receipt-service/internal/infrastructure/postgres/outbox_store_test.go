...
`)
	if err != nil {
		t.Fatalf("seed receipt outbox dry-run cleanup rows: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("dry-run cleanup receipt outbox repairs: %v", err)
	}
	if stats.Deleted != 2 {
		t.Fatalf("unexpected dry-run deleted count: %+v", stats)
	}
	assertReceiptOutboxRepairAuditCount(t, ctx, pool, "tenant-cleanup-dry-run", 3)
}
