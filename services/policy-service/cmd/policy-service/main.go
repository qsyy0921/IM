...
func runGRPC() error {
	...
	if rulesEnabled {
		...
		messageAuditor, closeMessageAuditor, err := policyDecisionAuditorFromEnv(
			auditPool,
			decisionMetrics,
		)
		if err != nil {
		...
		defer closeMessageAuditor()
		useCaseOptions = append(useCaseOptions, app.WithPolicyDecisionAuditor(messageAuditor))
