...
func TestParseDeviceIDs(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "empty uses default device",
			input:           "",
			defaultDeviceID: "default-device",
			want:            []string{"default-device"},
		},
		{
			name:            "commas use default device",
			input:           ",,",
			defaultDeviceID: "default-device",
			want:            []string{"default-device"},
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got := parseDeviceIDs(test.input, test.defaultDeviceID)
			if !reflect.DeepEqual(got, test.want) {
				t.Fatalf("parseDeviceIDs(%q, %q) = %#v, want %#v", test.input, test.defaultDeviceID, got, test.want)
			}
		})
	}
}
