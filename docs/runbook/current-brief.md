...
5. 下一批执行顺序：先保持入口文档短，再按 `api-gateway -> identity-service -> message-service -> conversation-service -> delivery-service -> push-gateway -> receipt-service -> contacts-service -> policy-service` 逐个收口可落地 P2。
