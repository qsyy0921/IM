...
1. 当前已完成：identity-service 已补只读 `session-mfa-proof-audit`、只读 `challenge-delivery-repair-audit` 和 `challenge-delivery-repair-cleanup` operator；contacts-service 已补只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式；policy-service 已补只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式；receipt-service 已补只读 `outbox-audit`、`outbox-repair`、只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式，用于直接审计、redrive 和按 retention 清理 `receipt_outbox` repair 历史。
