...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path (Split-Path -Parent ([System.IO.Path]::GetFullPath($AuditAppendResultPath))) "provider-replay-redrive-audit-append-result-manifest.json"
}
Assert-ExternalRepairOutputPath -Value $OutputPath -FieldName "OutputPath"

if ([string]::IsNullOrWhiteSpace($ResultManifestID)) {
    $ResultManifestID = "provider-replay-audit-append-result-" + [System.Guid]::NewGuid().ToString("N")
}
...
$auditManifest = Get-JsonDocument -Path $AuditManifestPath -Label "Provider replay audit append manifest"
$auditResult = Get-JsonDocument -Path $AuditAppendResultPath -Label "Provider replay audit append execution summary"

Assert-True ((Get-JsonString -Object $auditManifest -Name "schema_version") -eq "nexusim.action_executor.external_audit_append.v1") "Unsupported audit append manifest schema_version."
...
Assert-Same -Actual $sourceService -Expected "action-executor" -FieldName "audit_manifest.source_service"
Assert-Same -Actual $recordType -Expected "ACTION_PROVIDER_REDRIVE" -FieldName "audit_manifest.record_type"
Assert-Same -Actual $action -Expected "REDRIVE_PROVIDER_FAILURE" -FieldName "audit_manifest.action"
...
$manifest = [ordered]@{
    schema_version = "nexusim.action_executor.provider_replay_audit_append_result.v1"
    ...
    note = "Low-sensitive provider replay audit append result manifest. It binds an executed external audit append summary to its manifest but does not append audit, execute redrive, mutate DLQ rows, or embed raw operator input."
}

$encoded = $manifest | ConvertTo-Json -Depth 30 -Compress
Assert-NoRawDocumentText -Value $encoded -FieldName "provider replay audit append result manifest"
...
Write-Host "OK   provider replay audit append result manifest written: $OutputPath"
