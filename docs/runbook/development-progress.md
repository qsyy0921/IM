...
- future service boundary 门禁仍保护未授权服务目录；`search-service v0.1` 和 `memory-service v0.1` 已作为 AI 底座 foundation-active 服务落地基础目录和 timeline consumer，后续 `retrieval-gateway` / RAG / `summary-service` / Agent / `skill-registry` / `mcp-gateway` / `action-executor` 不能绕过 search / memory / retrieval / policy 直接落 demo
...
| `retrieval-gateway` | 已跑通第一轮 EvidencePack smoke | `retrieval_gateway.proto`、SDD、六层 skeleton、`RetrieveEvidence` app / gRPC adapter、search / memory RPC clients、可选 policy-service retrieval precheck、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring 和真实本地 `search + memory -> RetrieveEvidence` smoke 已落地；定位为 EvidencePack 统一检索边界，不读业务库、不调用 LLM、不执行 Agent 动作 | `service-briefs/retrieval-gateway.md` |
