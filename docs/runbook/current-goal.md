...
5. admission token / concurrency limit 已实现并跑完 v1 矩阵；`MaxInFlight=64` 是当前 Windows 本机下一轮候选，60s 下 1200/1600 VU accepted RPS 约 `1.92k`、success p99 约 `63ms`、outbox pending 为 0。
6. 下一步围绕 `MaxInFlight=64` 做 retry 策略矩阵，例如 `RetryBaseDelay=250ms/500ms/1000ms` 与 `MaxRetries=1/2`，目标是降低 logical p99，同时保持 outbox pending 为 0。
7. 继续采集 PostgreSQL wait_event，重点看 `LWLock:WALWrite`、`LWLock:WALInsert`、`LWLock:BufferContent` 和 `CheckpointWriteDelay`。
8. adaptive admission 合并阶段评审已完成，无 P0/P1；下一轮 retry 策略矩阵需要显式展示 recent sample count、retry delay histogram、retry attempt count、logical success rate、overload rate、success p99 和 logical p99。
8. 暂不推 GitHub；等 adaptive token / retry 策略矩阵和评审 P2 口径收敛后再做批量同步。
...
- 2026-06-09：已在 clean commit `b6f0b82` 跑 batch mark 后的 client retry 对照矩阵，并新增 `docs/runbook/loadtest-report-20260609-outbox-batchmark.md`。结论：pending 明显下降，但容量提升需要重复矩阵或进一步 batch publish 验证。
...
- 2026-06-09：已在 clean commit `b98e3c9` 跑 `AdaptiveMaxPoolAcquireP95=250/500/750ms` 矩阵，并新增 `docs/runbook/loadtest-report-20260609-adaptive-poolp95-v1.md`。结论：outbox 全部清零，但 accepted RPS 偏低、logical p99 仍为 5s 以上；下一步应设计 admission token / concurrency limit，而不是继续只调 p95 阈值。
- 2026-06-09：已实现 app 入口 `NEXUSIM_ADAPTIVE_MAX_IN_FLIGHT` token / concurrency gate，并在 clean commit `7bf37fe` 跑 30s cap 梯度和 60s 候选重复验证；新增 `docs/runbook/loadtest-report-20260609-adaptive-inflight-v1.md`。结论：`MaxInFlight=64` 当前最稳，accepted RPS 回到约 `1.92k` 且 outbox 可清零，但 logical p99 仍约 `2.2s`；下一步调 retry delay / max retries，而不是继续放宽 token 上限。
