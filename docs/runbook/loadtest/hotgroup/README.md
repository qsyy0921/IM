...
| `hotgroup-message-cdc-wal-shadow-plan-20260702.md` | Backend Lab 已实现 DB-first CDC/WAL shadow export：`cdc_shadow` 保留旧 `message_outbox` 主路径并把 `conversation_timeline_events` 通过 Debezium + `message-service cdc-bridge` 发布到 `conversation.timeline.events.cdc`；本地 hotgroup smoke 已修正 Debezium source key 导致的同 conversation seq 乱序，修正后 `cdc_shadow` 和 `cdc_only` 小规模 smoke 均通过。远端 `cdc_only` 6000 人双客户端压测已完成，3s bounded reorder hold 下 5000 条 CDC shadow-check 无 missing / duplicate / out-of-order。 |
| `hotgroup-analysis-20260703-cdc-only-reorderhold.md` | clean commit `aad98064` 的 `cdc_only` 验证：Windows / Mac 各 2500 条、384 并发、6000 人 READ_FANOUT / SEQUENCER_BLOCK，SendMessage 5000/5000 成功，message / delivery outbox pending=0，CDC source 和 delivery consumer lag=0，shadow-check `expected_count=5000 / observed_count=5000` 且 out-of-order 为空。Prometheus 显示 `repository_insert_outbox_p99_recent_ms=0`、message outbox relay samples=0；剩余长尾回到 dependency / policy waiting 和主 PG append。 |
