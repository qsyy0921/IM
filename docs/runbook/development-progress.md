...
| `identity-service` | 已落地、已接登录主链路 | login / refresh / MFA / recovery code / JWKS / challenge delivery、SMTP subject/body templates、first-stage OTel gRPC server span | WebAuthn/passkeys、OIDC、多 issuer、KMS/HSM、完整风控、生产级 email/SMS provider、OTel collector / alerting |
| `message-service` | 已落地、已接主链路 | `SendMessage` / outbox / Kafka timeline、first-stage OTel gRPC server span | 更多消息类型、会话级删除策略深化、合规删除、容量和生产观测、OTel collector / alerting |
...
| `delivery-service` | 已落地、已接主链路 | projection / `PullInbox` / `AckDelivery` / `HideInboxItem` / delivery outbox、first-stage OTel gRPC server span | Projection DLQ / repair 深化、更多 delivery event 消费方、隐藏项跨设备删除提示、OTel collector / alerting |
