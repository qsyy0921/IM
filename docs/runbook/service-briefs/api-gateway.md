...
- rate limiter 已支持 token scope、tenant scope、静态 tenant plan override、first-stage tenant plan 文件热更新、版本化 quota snapshot 文件和 tenant plan source fail-closed guard；tenant scope 使用已验证 gateway token 的 `tenant_id` 做 per-method quota key。
...
- `ADR-033` 已固定 tenant quota source 边界：api-gateway 不直接读业务内部表；后续配置中心 / DB-backed quota 必须通过版本化控制面 / 配置契约。

## 后续

- 后端服务 server span rollout、采样治理 hardening、legacy descriptor opt-in 实际迁移观察和移除计划、配置中心 / DB-backed quota hardening。
