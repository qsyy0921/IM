...
- 下游服务仍通过 `x-nexusim-trace-id` 和 `x-nexusim-request-id` metadata 接收；contacts-service gRPC access log 已完成第一站落地，后续可在其它服务结构化日志、Kafka envelope 和 OpenTelemetry exporter 中继续收敛。
