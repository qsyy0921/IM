...
ai/python/
  ...
  fixtures/
    agent_eval/
      ...
      memory_calibration_public_export.json
...
The runtime-control deeper hardening fixture covers checkpoint version drift
detection, workflow wakeup race dedupe and ReplayBundle lineage completeness.
The ReplayBundle observability fixture covers low-sensitive observability refs,
hash refs, version metadata refs, failure taxonomy refs and trace linkage refs
across `EvalReport`, `ReplayBundle` and `AgentRunTrace`.
...
services.
The public-export memory calibration fixture expands that surface to 15 memory
gate cases, 8 policy-window cases and 12 review-backoff cases, with
dataset-source refs and per-dataset case counts in the low-sensitive report.
...
The state-diff deeper hardening fixture covers state dependency graph refs,
cross-action compensation chain refs and operator redrive review refs.
