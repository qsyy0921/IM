...
| `identity-service` | SDD v0.1 Draft、proto / migration / 六层骨架、gateway token 签发、refresh rotation、邮箱/手机验证、密码重置 challenge 和 TOTP MFA 因子生命周期已落地 | 支撑 push-gateway 本地 token 验签；MFA 当前只覆盖因子注册/确认/禁用，登录强制 MFA、OIDC federation 和 KMS/HSM 仍是后续项 |
