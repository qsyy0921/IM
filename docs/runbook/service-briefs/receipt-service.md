...
- 已支持 unread、pinned、muted、tag 过滤，`UPDATED_AT` / `PINNED_UPDATED_AT` / `UNREAD_UPDATED_AT` 排序，以及 archive / pin / mute / tags 的最小会话列表能力；分页 cursor 会绑定 sort / archived / unread / pinned / muted / tag 过滤条件和排序边界，避免串页。
...
- 已补只读 `outbox-audit`，以及 `outbox-repair` / 只读 `outbox-repair-audit` / `outbox-repair-cleanup` 运维模式，可直接审计、redrive 和清理 `receipt_outbox` repair 历史；`outbox-audit` / `outbox-repair` / `outbox-repair-audit` / `outbox-repair-cleanup` 可通过 `NEXUSIM_RECEIPT_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_AUDIT_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_CLEANUP_OUTPUT` 写低敏 JSON 结果或 cleanup summary，便于 operator 留存证据。
...
- 会话列表更多产品化能力（草稿、更多摘要策略等）、长时间容量曲线；生产级 OTel collector、Alertmanager、SLO dashboard 和容量验证仍属于后续统一观测治理。
