...
| `message-service` | `SendMessage`、编辑、撤回、删除，`TEXT` + `IMAGE` / `FILE` 附件引用消息，message log，outbox，Kafka timeline event，first-stage OTel gRPC server span | 业务事务不直接 publish Kafka，使用 outbox 保证事件传播，核心写服务已进入 trace rollout；图片 / 文件二进制处理后续交给 media 能力 |
...
| `receipt-service` | 已读 / 未读，会话列表，archive / pin / mute，receipt projection，receipt outbox，`ListReceiptStates` repository 级批量查询，first-stage Prometheus text `/metrics`、本地 Prometheus scrape / alert rules 原型、本地 Grafana dashboard 原型、first-stage OTel gRPC server span | 会话列表和回执从投递事件投影，不跨服务读内部表，回执服务已进入观测 rollout |
...
| `message-service` | 语音 / 位置 / 卡片等更多消息类型、私有删除、合规删除、容量压测、生产级发送链路观测；图片 / 文件二进制上传处理后续交给 media 能力 |
...
| `receipt-service` | 送达回执扩展、会话列表产品化；当前已有第一阶段 Prometheus text `/metrics`、本地 alert rules 和本地 Grafana dashboard，但还不是生产观测平台 |
