...
- 下游服务仍通过 `x-nexusim-trace-id` 和 `x-nexusim-request-id` metadata 接收；contacts-service 和 message-service gRPC access log 已完成前两站落地，后续可在其它服务结构化日志、Kafka envelope 和 OpenTelemetry exporter 中继续收敛。
