...
| `agent-service` | 第一版 proposal-only path 和真实本地 adapter smoke 已落；当前 proposal 前调用 `mcp-gateway.PrepareToolCall`，通过 skill catalog / policy precheck / prepare audit 后再消费 EvidencePack 生成可引用 proposal，不执行真实动作 |
