...
5. push-gateway slow-client 真实进程负向 smoke 已通过；该 smoke 证明 durable `PullInbox` fallback，不证明 resume buffer replay。Redis route 最小 adapter、route TTL 续期和真实跨进程 route smoke 已完成，下一步优先补 Redis 故障语义、route cleanup ticker、resume buffer TTL，并单独设计 resume replay smoke。可以把当前能力表述为“最小分布式在线路由模拟”，但不要把当前 `all` 模式或 Redis route adapter 表述为完整生产多实例方案。
