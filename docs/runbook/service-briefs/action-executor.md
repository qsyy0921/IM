...
- 当前能力：first approved execution audit path + Agent approval preflight + low-sensitive tool result projection
- 运行模式：
  - `noop`
  - `grpc`
...
- 不读其它服务私有表。
- 第一版返回 `executed=false`，只证明执行边界、audit 和 result projection 已建立。

## 下一步

- 接真实 MCP / tool adapter。
- 增加 rate limit、failure fallback、真实 tool output projection 和 DLQ / repair。
