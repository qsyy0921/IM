...
- `vector-index-service`：first-stage rebuild checkpoint worker 和本地 JSONL
  `embedding-worker -> model-gateway.InvokeEmbedding -> vector upsert` 已落；后续仍需
  真实 knowledge / memory / search chunk consumer、embedding task 持久队列、真实
  Milvus / pgvector / OpenSearch backend，以及 provider backend rebuild / backfill worker。
