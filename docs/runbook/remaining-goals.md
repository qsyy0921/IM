...
| `policy-service` | 完整 ReBAC graph / DSL；provider-grade moderation / risk scoring；provider-grade tenant DSL / quota；provider-grade 外部 audit sink；已完成 first-stage decision source、keyword / HTTP content moderation、低敏 `decision-audit-export` 本地审计交接、first-stage tenant action quota 和本地 direct 短基线，仍需长时间容量曲线和生产 sizing。 |
