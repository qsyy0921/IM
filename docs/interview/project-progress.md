...
search-service v0.1 / group memory / retrieval / RAG 是下一阶段基础能力，其中 search-service 第一实现切片、memory-service source-backed projection smoke、retrieval-gateway EvidencePack smoke、retrieval policy precheck、EvidencePack field hardening first pass、AI eval harness first pass、rag-service first read-only answer path、`loadtest/rag`、RAG eval adapter 和真实本地 adapter smoke 已分别完成 smoke 或实现；下一步进入 LLM provider 边界 / citation verifier / summary / Agent / skill-registry / MCP gateway / action-executor；
...
| `rag-service` | 第一版只读问答路径已落并通过真实本地 adapter smoke；基于权限过滤后的 EvidencePack 生成 deterministic extractive answer，保留 citations，无 evidence 时拒答 |
