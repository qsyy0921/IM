...
状态：Draft，proto / Kafka schema / migration / 六层骨架、PostgreSQL repository、delivery event consumer、`MarkRead` 事务、`ListReceiptStates` 薄批量查询、receipt outbox relay、只读 `outbox-audit` operator、最小 `ListConversations`、`unread_only` 未读过滤、Archive / Pin / Mute 用户列表偏好、第一阶段 gRPC server TLS / mTLS 配置，以及 receipt / demo smoke runner 的 delivery / receipt client TLS 配置已落地；真实进程 smoke 已覆盖 `im.delivery.events -> receipt projection -> MarkRead -> receipt_outbox -> im.receipt.events` 和会话列表偏好链路。
...
第一阶段运维面补充：`receipt-service` 提供只读 `outbox-audit` 运行模式，用于直接按 `outbox_id / event_id / tenant_id / conversation_id / status / event_type` 审计 `receipt_outbox` 当前状态；它不修改 row、不执行 repair，也不替代后续更完整的 repair / audit / cleanup 体系。
