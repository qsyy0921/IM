...
- `retrieval-gateway`：foundation-active 第一版 EvidencePack 边界和真实 `search + memory -> RetrieveEvidence` smoke 已通过；policy-service retrieval precheck 已有 first-stage 可选接入；后续补 EvidencePack 字段 hardening（rerank score、source coverage、dedupe reason），再进入 RAG / summary / Agent。
