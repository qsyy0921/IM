...
- `message-service` 还补了 `outbox-audit`、`outbox-repair`、只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式，方便直接列出 `message_outbox` 当前行、按 event 精确 redrive DLQ、审计 repair 历史并按 retention 清理 repair 审计；这些 operator 不改消息业务事实，只处理 outbox 发布状态和 repair 审计历史。
