...
| `push-gateway` | SDD v0.1 Draft、WebSocket frame、`im.delivery.events` consumer、ACK 转发、HMAC auth、Redis route / resume、slow session close、多实例 smoke 和 `loadtest/pushgateway` conversation / message / delivery / identity client TLS config 已落地 | 只做在线唤醒和回源协调；push-gateway 进程自身 delivery RPC client TLS 与 smoke runner gRPC client TLS 默认 plaintext、可静态配置；后续补 WebSocket TLS、证书治理、Redis route hardening 和生产指标 |
