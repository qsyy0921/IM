...
`check:no-toolchain` 聚合 Web PWA、Web shell lifecycle / automation / smoke-report
contracts、shell asset、desktop / Android action asset、clientweb smoke hook
contract、artifact readiness / install-plan contracts、desktop WebView metadata /
login dry-run contracts、Android metadata / login dry-run contracts、Android
device / WebView devtools readiness parsers、Android platform readiness 和 shell
smoke plan checks；它不构建 native artifact
