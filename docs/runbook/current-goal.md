...
```text
push-gateway slow session active close
-> Redis route / resume buffer
```
...
5. push-gateway 后续优先补 Redis route 和 resume buffer；queue-full active close 已有单元 / 集成测试，但还没有 slow-client 真实进程负向 smoke 和 metrics；不要把当前 in-memory `all` 模式表述为多实例生产方案。
...
- 2026-06-09：已补 push-gateway slow session active close 第一版：session queue full 时 registry 发出 eviction signal，WebSocket writer 尽量发送 `server.resume_hint` 后主动以 policy violation close 连接，客户端随后应重连并 `PullInbox`；同时修复多设备 smoke ACK request_id 可读性，并补 `parseDeviceIDs` 单测。下一步补 Redis route / resume buffer，或增加 slow-client 真实进程负向 smoke 与指标。
