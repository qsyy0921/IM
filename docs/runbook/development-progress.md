...
- `admin-service` product-active：SDD v0.1 和 stage-switch review 已通过，
  ...
  `NEXUSIM_CONTROL_PLANE_GRPC_ADDR` 调用 control-plane public gRPC；本地多进程
  `Create -> approve -> operation-worker -> control-plane snapshot` smoke 已通过；
