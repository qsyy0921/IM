...
1. 做 memory admission hardening、state-diff hardening、runtime-control negative
   fixture 等离线 eval fixture。
...
4. 增加 runtime-control negative fixture pack，覆盖 missing checkpoint、
   cancel propagation incomplete、replay event incomplete。
5. 在上述实验通过后，再决定是否把 Agent Runtime / Harness 提升为 ADR、服务级 SDD
   或实际 runtime module。
