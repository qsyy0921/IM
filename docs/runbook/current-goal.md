...
6. message-service 第一阶段压测报告已整合为 `docs/runbook/loadtest-report-20260609-message-service-consolidated.md`，后续不再围绕 message-service 做大规模硬件矩阵；只在关键机制变更后跑 smoke / 小规模验证。
7. 下一步优先补第二个真实微服务，建议从 `conversation-service` 开始，替换 message-service 的 strict conversation mock，形成真实跨服务 RPC 依赖。
...
- 2026-06-09：根据用户反馈，message-service 不再继续做大规模压测矩阵；已新增 `docs/runbook/loadtest-report-20260609-message-service-consolidated.md`，整合 27 份原始压测报告、瓶颈排查过程和面试可讲要点。下一步转向第二个真实微服务，优先 `conversation-service`。
