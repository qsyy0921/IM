...
4. 当前已补本地 PostgreSQL `repmgr + pgpool` failover smoke；Kafka HA、Redis quorum / 网络分区仍属于生产化后续项，不阻塞当前功能推进。
...
第二层：分布式与可靠性
outbox、Kafka、durable inbox、Redis route、多实例、Win/Mac Docker smoke、基础观测，以及本地 PostgreSQL `repmgr + pgpool` failover smoke。已有小规模 smoke 证据，不是生产 HA。
