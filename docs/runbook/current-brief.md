...
4. api-gateway 已补 first-stage tenant-scoped rate limit、静态 tenant plan override、tenant plan 文件热更新、版本化 quota snapshot 文件、tenant plan source fail-closed guard、tenant quota source ADR、legacy descriptor 显式 opt-in 默认、legacy/facade traffic metrics、legacy descriptor last-seen 迁移观察和单次 migration gate、first-stage Prometheus text `/metrics`、本地 Prometheus scrape / alert rules 原型、本地 Grafana dashboard 原型、first-stage OpenTelemetry 入口 server span 和下游 gRPC client span；contacts-service、identity-service、message-service、conversation-service、delivery-service、receipt-service、policy-service 已开始后端服务 gRPC server span rollout，push-gateway 已补 first-stage WebSocket connection span；本地 OTel collector debug 入口、policy OTLP smoke 脚本、first-stage trace sampling policy / check 和 span attribute guardrail 已补；下一步继续治理 api-gateway / 后端观测：配置中心 quota source adapter 和 legacy opt-in 移除计划。
