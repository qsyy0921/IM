...
| 本地分布式 runbook | `docs/runbook/distributed-local.md` 已新增，定义当前四个真实微服务 / 网关的本地多进程分布式拓扑、`tools/local-distributed-smoke.ps1` 入口、可讲证据和限制 |
...
5. 当前系统可以表述为“本地多进程最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立进程协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route 和 WebSocket notify 串联。下一步优先补真实 Redis 故障 smoke、route cleanup ticker、resume buffer TTL，并单独设计 resume replay smoke。不要把当前本地分布式 smoke 表述为完整生产多实例方案。
