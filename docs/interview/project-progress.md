...
| `agent-service` | 第一版 proposal-only path、真实本地 adapter smoke、MCP prepare、proposal store、approval preflight 和 approval outbox relay 已落；当前 proposal 前调用 `mcp-gateway.PrepareToolCall`，通过 skill catalog / policy precheck / prepare audit 后再消费 EvidencePack 生成可引用 proposal，审批时同事务写 `agent.proposal.approved.v1` 低敏 outbox，并由 relay 发布低敏 `im.agent.events`，同时通过 `VerifyApprovedAgentProposal` 给 executor 做公开校验，不执行真实动作 |
