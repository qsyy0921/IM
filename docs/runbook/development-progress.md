...
- `client-platform`：v0.1 SDD 已冻结浏览器、PC、Android 三端架构；`clients/`
  ...
  且 first-stage contacts workbench 已接 shared BFF / client-core，覆盖好友申请、
  接受 / 拒绝 / 取消、备注、分组、删除和拉黑 / 取消拉黑；
  账号注册和创建群聊 first path 已接入同一套 client BFF / client-core：注册走
  identity-service `RegisterUser`，建群走 conversation-service `CreateConversation`，
  创建 GROUP 时只把当前用户作为 OWNER，并写 member boundary timeline / outbox；
  好友直聊 first path 已接入同一套 client BFF / client-core：Web / PC shell 从
  联系人卡片调用 `/api/conversations/direct`，BFF 先通过 contacts-service
  ...
  群成员添加 / 退群 / 成员列表 / 移除成员 / 角色变更 / owner transfer first path
  已接入同一套 client BFF / client-core：
  ...
  群标题 / 头像 URI / 群公告 read-update 已接入同一套 conversation-service -> api-gateway
  BFF -> client-core -> Web / PC shell 路径；群头像上传 / 展示 first path 已通过
  api-gateway BFF -> media-service 上传会话 / 完成上传 / 短期 download URL ->
  conversation-service profile update 串起来，当前本地路径使用 media-service fake object HTTP adapter，
  ...
  混入后续 shell 包；desktop artifact wrapper 会在已验证 manifest 后跳过 Tauri 内重复
...
- `action-executor` first execution audit path + proposal / approval / prepare audit linkage + local safe adapter + guarded external HTTP provider adapter + external adapter eval + preflight / rate-limit / DLQ-repair safety eval + provider failure retry/DLQ skeleton + bounded retry worker + provider replay controlled redrive operator + redrive result manifest + redrive audit append manifest handoff + external audit append operator + audit append result manifest
- `ai-eval-service` first persistent eval run catalog + low-sensitive `RecordEvalRun` / `GetEvalRun` / `ListEvalRuns` + recorder / policy-driven multi-adapter gate smoke + Python optional adapter path + memory-service / retrieval-gateway optional live adapter first pass + retrieval negative / miss adapter + RAG / Agent service-stack live gate + Summary live negative adapter + CI-safe gate skeleton + first RAG-Agent regression expansion + profile / Agent output safety expansion + service-stack version / hash-only expansion + negative RAG / Agent cases + Python/model-output negative cases + RAG/Summary citation source-ref regression + external MCP recovery eval cases + Agent output regression + action preflight / rate-limit / DLQ-repair safety eval + action provider failure worker / redrive safety eval + memory group source-ref / validity / supersession / cross-group / temporal eval、multi-hop actor chain、workstream / decision dependency edge、reviewed multi-source profile activation、supporting-memory delete -> profile recompute、memory-service public recompute assertion、retrieval smoke、RAG / Summary / Agent stack consumption smoke、40/40 optional stack gate、2026-06-24 retrieval-negative 真实 service-stack gate 51/51 passed、retrieval source-chain rerank live gate 27/27 passed、retrieval graph expansion configurable-depth focused gate，以及 RAG-Agent demo 真实 service-stack gate 27/27 passed
...
- `workflow-service` product-active：SDD v0.1 和 stage-switch review 已通过，
  ...
  record-decision / workflow-list / provider-replay-queue / external-callback-wait /
  instruction-list operator CLI；
  `provider-replay-queue` 默认列出等待 `admin.workflow.provider_replay.v1` 审批的
  action-executor `PROVIDER_REPLAY_REQUEST` workflow，只展示低敏 refs / hash / 状态，
  不执行 redrive、不修改 DLQ row，给 operator UI / ops 管理后续接入；
  `operator-queues` 已提供 action approval、repair approval、provider replay、admin
  operation、compensation request 和 compensation pending 多队列低敏 summary；只读
  `ListWorkflows`，不记录 decision、不执行 replay；
  first-stage external decision manifest binding 已落：`record-decision -decision-manifest`
  使用仓库外低敏 `nexusim.workflow.external_decision_manifest.v1`，记录 decision 前先
  `GetWorkflow` 校验 workflow type / step / target / payload hash / approval policy，
...
- `vector-index-service` product-active：SDD v0.1 和 stage-switch review 已通过，
  ...
  `chunk-consumer` 已支持 protobuf `KnowledgeEvent` 与旧 JSON recovery；已跑通
...
当前 9 个 IM 业务服务已经覆盖 IM 主链路，`timeline-service` 作为第 10 个 noop 运行节点
已进入本地 Docker / 观测链路，为热点群 sequencer 和分区能力预留独立边界：
...
- Redis stop/start fault recovery
- Redis Sentinel discovery / failover / master-stop / quorum-loss recovery
- Redis Sentinel network-partition recovery smoke
...
| `timeline-service` | 已进入本地运行链路、当前 noop | 六层边界、`noop` runtime、Docker runtime、Prometheus rule / Grafana dashboard 已落地；当前不分配 seq block、不写 timeline、不发布 Kafka，后续承接热点群 sequencer、gap marker、epoch fencing 和 timeline 分区 | `service-briefs/timeline-service.md` |
| `delivery-service` | 已落地、已接主链路 | projection / `PullInbox` / `AckDelivery` / hide inbox / delivery outbox、outbox audit / repair / repair audit / cleanup JSON 留存、projection failure audit / checkpoint rewind / failure resolve / cleanup JSON 留存、delivery outbox relay SQL / worker / Kafka batch hardening、2026-06-28 hotgroup QPS step 证明 100 人群 150 QPS 可完成 durable inbox 和 delivery outbox drain，200 QPS 下一瓶颈转移到 projection / `user_inbox` fanout、loadtest `capacity_summary` 口径和本地 seeded 30m 长跑切片 | `service-briefs/delivery-service.md` |
| `push-gateway` | 已落地、已接主链路 | notify / ACK / resume / Redis route / Redis resume negative recovery / Win-Mac / Sentinel / network-partition / Redis Cluster topology / Redis Cluster node-stop recovery / Redis Cluster failover / Redis Cluster 短容量基线 / TLS smoke、stack 短基线、loadtest `capacity_summary` 和 `--duration` / `--vus` full 场景容量循环入口 | `service-briefs/push-gateway.md` |
...
| `search-service` | 已跑通第一轮 projection smoke，并补 PostgreSQL FTS / OpenSearch candidate backend first paths | `search_service.proto`、PostgreSQL core migration、六层 skeleton、`SearchMessages` app / domain / gRPC adapter、projection usecase skeleton、PostgreSQL repository、真实 PG visibility / tombstone 集成测试、PostgreSQL FTS token search、显式 OpenSearch / BM25 candidate backend、OpenSearch opt-in backend smoke 入口、service-owned OpenSearch rebuild operator first path、OpenSearch mapping drift fail-closed hardening、PG candidate hydration visibility / tombstone filter、`grpc` runtime mode、timeline decoder / consumer 和 clean projection smoke 已落地；定位为 search projection / visibility / tombstone / EvidencePack 前置，不做 LLM demo；真实 OpenSearch 进程 smoke 尚未归档通过，provider-grade 运维仍后置 | `service-briefs/search-service.md` |
| `memory-service` | 已跑通第一轮 projection smoke，并补 profile recompute / batch approval operator path、RAG-Agent profile repair gates 和 group-memory answer / proposal gate | `memory_service.proto`、PostgreSQL memory core migration、六层 skeleton、`QueryMemoryEvents` / `GetMemoryEvent` / `ListProfileAggregates` / `RecomputeProfileAggregate` gRPC adapter、domain/types/app validation、PostgreSQL repository first pass、timeline projection usecase、rules-v0.2 group memory extraction cue classifier、profile aggregate recompute / archive、first-stage `loadtest/memoryprofile` profile repair operator、batch manifest plan-only、workflow-service `REPAIR_APPROVAL` 校验后批量执行、未审批 workflow / approval payload hash mismatch fail-closed 负向门禁、既有 subject/type/key profile aggregate 重算更新、`grpc` / `timeline-consumer` runtime、Docker / compose / Prometheus / Grafana wiring、PG integration tests、timeline worker tests、clean projection smoke、`ai-eval-rag-agent-demo-live-20260624-profile-repair-approval-v3`、`ai-eval-rag-agent-demo-live-20260624-profile-repair-negative-v1` 和 `ai-eval-rag-agent-demo-live-20260624-group-memory-answer-proposal-gate-v1` 真实 gates 已落地；group-memory gate 验证 `DECISION` / `BLOCKER` / `FILE` 三类 reviewed events 同时进入 RAG / Agent EvidencePack，并保留 6 个 source refs 与 3 个 cross-group source refs；定位为 group memory / StructuredMemoryEvent / source refs / visibility window / reviewed profile aggregate，不做 LLM demo | `service-briefs/memory-service.md` |
| `retrieval-gateway` | 已跑通 EvidencePack smoke，cross-group / temporal retrieval smoke、source-chain-aware rerank live gate 和 graph expansion configurable-depth focused gate 已落 | `retrieval_gateway.proto`、SDD、六层 skeleton、`RetrieveEvidence` app / gRPC adapter、search / memory RPC clients、可选 policy-service retrieval precheck、EvidencePack `rerank_score` / `dedupe_reason` / `source_coverage`、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring、真实本地 `search + memory -> RetrieveEvidence` smoke，以及 2026-06-20 cross-group source refs / speaker attribution / stale-future memory exclusion smoke 已落地；2026-06-24 source-chain-aware rerank first pass 已让多来源 / 跨群 / actor / graph-edge / profile-supporting evidence 提升 `rerank_score`，并通过 app / loadtest focused checks、retrieval positive adapter 断言和 `ai-eval-service-stack-live-20260624-retrieval-source-chain-rerank-v2` 真实 service-stack gate 覆盖；同日 graph expansion 已从固定 depth=1 升级为可配置 depth 0..3，沿 memory graph edge 通过 memory-service 公开 `GetMemoryEvent` 拉取相邻 memory event，并在非法配置、lookup / visibility / malformed edge 异常时 fail-closed；定位为 EvidencePack 统一检索边界，不读业务库、不调用 LLM、不执行 Agent 动作 | `service-briefs/retrieval-gateway.md` |
| `rag-service` | 已落第一版只读问答路径、真实 adapter smoke、citation verifier、guarded external HTTP LLM boundary、cross-group / temporal stack smoke、RAG-Agent demo runner first path 和 group-memory answer gate | `rag_service.proto`、SDD、六层 skeleton、`AnswerQuestion` app / gRPC adapter、retrieval-gateway RPC client、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring 已落地；第一版 deterministic extractive provider，保留 citations / EvidencePack，`generated_by_llm=false`，无 evidence 时拒答；provider 输出后统一通过 citation verifier；真实本地 adapter smoke 已验证 current-memory stale exclusion 与 cross-group source refs / speaker attribution / future memory exclusion；`loadtest/ragagent` 编排 RAG grounded answer 与 Agent proposal / approval / action-executor audit，并输出低敏总报告；2026-06-24 group-memory answer gate 已验证 `DECISION` / `BLOCKER` / `FILE` 多事件 evidence 进入 RAG EvidencePack 并保留 source refs / cross-group refs；可选 `external-http` provider 只能用 EvidencePack 构造 prompt，provider failure 返回稳定 unavailable，unsafe / malformed output fail closed | `service-briefs/rag-service.md` |
| `summary-service` | 已落第一版只读摘要路径、真实 adapter smoke、Summary negative eval adapter、guarded external HTTP LLM boundary 和 cross-group / temporal stack smoke | `summary_service.proto`、SDD、六层 skeleton、`GenerateConversationSummary` app / gRPC adapter、retrieval-gateway RPC client、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring、`loadtest/summary` 和真实本地 adapter smoke 已落地；第一版 deterministic extractive provider，保留 citations / EvidencePack，`generated_by_llm=false`，无 evidence 时拒绝摘要；真实本地 stack smoke 已验证 cross-group source refs / speaker attribution 和 stale / future memory exclusion；provider 输出后统一通过 citation verifier；可选 `external-http` provider 只能用 EvidencePack 构造 prompt，provider failure 返回稳定 unavailable，unsafe / malformed output fail closed | `service-briefs/summary-service.md` |
| `agent-service` | 已落第一版 proposal store / approval preflight / approval outbox relay / approval operator / planner Python candidate guard / cross-group temporal stack smoke / RAG-Agent demo runner first path / group-memory proposal gate / business proposal source-chain gate | `agent_service.proto`、SDD、六层 skeleton、`CreateAgentProposal` / `ApproveAgentProposal` / `VerifyApprovedAgentProposal` app 和 gRPC adapter、PostgreSQL `agent_proposals` repository、低敏 `agent_approval_outbox`、`approval-outbox-relay`、`proposal-approval-audit` / `proposal-approval-approve` 本地 operator、`im.agent.events` schema、retrieval-gateway RPC client、mcp-gateway prepare RPC client、可选 `python-worker` proposal provider mode、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring、`loadtest/agent` 和真实本地 adapter smoke 已落地；`loadtest/ragagent` 复用 Agent proposal / approval / action-executor audit 校验并与 RAG grounded answer 组合成同一 tenant / conversation 的低敏总报告；当前 proposal path 已验证 cross-group source refs / speaker attribution、stale / future memory exclusion、2026-06-24 group-memory proposal gate 的 `DECISION` / `BLOCKER` / `FILE` 多事件 source-chain，以及 business proposal source-chain gate 的 `DECISION` / `TASK` / `STATUS` 业务证据链；继续只生成 proposal、不直接执行业务 mutation；approval operator 默认 dry-run，reason 走文件且输出不含 proposal 正文 / EvidencePack / reason 原文；默认 deterministic extractive proposal，Python worker 只校验 proposal hash / citation metadata，Go 仍拥有最终 proposal / approval / audit，prepare / policy deny 时不检索证据，不执行工具动作 | `service-briefs/agent-service.md` |
...
| `action-executor` | 已落第一版 approved execution audit + Agent approval preflight + result projection + local safe tool adapter + guarded external HTTP adapter + explicit conversation note / profile business adapters + external adapter eval + preflight / rate-limit / DLQ-repair safety eval + provider failure worker + provider replay admin / workflow handoff + redrive invocation manifest + 受控 redrive execution operator path + redrive result manifest + redrive audit append manifest handoff + external audit append operator path + audit append result manifest | `action_executor_service.proto`、SDD、migration、六层 skeleton、`ExecuteApprovedAction` app / gRPC adapter、agent-service proposal verification RPC client、skill-registry RPC client、policy-service RPC client、PostgreSQL 低敏 execution audit repository、低敏 `action_executor_tool_results` projection、低敏 `action_executor_provider_failures` projection、bounded retry bookkeeping worker、`nexusim.local.echo` 本地安全 adapter、显式 allowlist + `LOW` risk 外部 HTTP provider adapter、显式 `NEXUSIM_ACTION_EXECUTOR_CONVERSATION_GRPC_ADDR` conversation note / profile business adapters、外部 adapter eval / failure smoke、preflight safety smoke、rate-limited action blocked、rate-limiter unavailable fail-closed、repair/DLQ/redrive operator guard smoke、provider replay operator UI first path、`provider-replay-handoff` 低敏 admin / workflow handoff artifact、redrive invocation manifest、`loadtest/actionexecutor` provider replay controlled redrive preflight / explicit execute operator CLI、redrive result manifest writer、redrive audit append manifest writer、`loadtest/actionexecutor` external audit append preflight / explicit execute operator CLI、audit append result manifest writer、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring 已落地；第一版强制 proposal / approval / prepare audit 关联，并通过 `agent-service.VerifyApprovedAgentProposal` 校验 proposal 已批准且字段匹配；2026-06-25 `conversation.note.create` 可在显式配置 conversation-service gRPC 地址后调用公开 `CreateConversationNote` 写真实 note fact；`conversation.profile.update` 可调用公开 `UpdateConversationProfile` 更新会话资料并只输出 profile version / hash metadata；未配置时仍 `executed=false` 且不伪造成功；repair/DLQ/redrive 类动作必须走 operator workflow，provider replay 可以输出低敏 admin / workflow handoff / redrive invocation / result manifest / audit append manifest / audit append result manifest，并由 `loadtest/actionexecutor` 在仓库外校验 raw resource id / new input / reason hash 后显式调用 `RedriveProviderFailure`；result manifest 只绑定低敏 execution summary，不执行 redrive、不追加 audit、不修改 DLQ；redrive audit append manifest 只生成 audit-service append contract，不调用 audit-service；external audit append 只能由 operator 显式 `-execute` 调用 audit-service 公开 `AppendAuditRecord`，默认 preflight 且拒绝 raw provider artifact；audit append result manifest 只绑定 audit-service 返回的 audit id / record hash / idempotency key，不调用 audit-service、不执行 redrive、不修改 DLQ；conversation note / profile / echo / allowlisted HTTP provider tool 可 `SUCCEEDED` 并只记录 output hash，provider failure 只记录 retry/DLQ 状态，不保存 raw input / provider output / provider 原始错误 | `service-briefs/action-executor.md` |
| `ai-eval-service` | 已落第一版持久化 eval run catalog、RecordEvalRun recorder、policy-driven multi-adapter gate smoke、Python optional adapter path、memory-service / retrieval-gateway optional live adapter first pass、retrieval negative / miss adapter、retrieval source-chain rerank live gate、RAG / Agent service-stack live gate、Summary live negative adapter、CI-safe gate skeleton、第一批 RAG-Agent regression expansion、profile / Agent output safety expansion、service-stack version / hash-only expansion、negative RAG / Agent cases、Python/model-output negative cases、RAG/Summary citation source-ref regression、external MCP recovery eval cases、Agent output regression、action preflight / rate-limit / DLQ-repair safety eval、action provider failure worker / redrive safety eval、provider replay admin / workflow handoff eval、provider replay redrive invocation / controlled execution eval、action-executor external audit append eval、provider replay submit operator eval、workflow provider replay queue view eval、workflow external approval binding eval、workflow external callback wait eval、workflow operator queue visibility eval、memory group source-ref / validity / supersession / cross-group / temporal eval、multi-hop actor chain、workstream / decision dependency edge、reviewed multi-source profile activation、supporting-memory delete -> profile recompute、memory-service public recompute assertion、RAG / Summary / Agent current-memory consumption regression、memory extraction confidence / review eval、asker-bound term ambiguity / missing visibility projection fail-closed / audience-language profile negative eval、current-memory service-stack live smoke、cross-group / temporal retrieval smoke、RAG / Summary / Agent stack consumption smoke、40/40 optional stack gate、retrieval negative 51/51 live gate，以及 RAG-Agent demo public candidate / temporal update / profile repair approval / profile repair negative / group-memory answer-proposal / business proposal source-chain 27/27 live gates | `ai_eval_service.proto`、SDD、migration、六层 skeleton、`RecordEvalRun` / `GetEvalRun` / `ListEvalRuns` app / gRPC adapter、PostgreSQL repository、`grpc` runtime mode、本地 Docker / compose / Prometheus / Grafana wiring、`ai-eval-record-smoke`、`run-ai-eval-record-run-smoke.ps1`、`gate-policy.local.json`、`validate-ai-eval-gate-policy.ps1`、`run-ai-eval-regression-gate-smoke.ps1`、`run-ai-eval-service-stack-gate-smoke.ps1`、`run-ai-eval-retrieval-negative-adapter.ps1` 和 `check-ai-eval-regression-gate.ps1` 已落地；case catalog 114 个 active cases，新增 provider replay admin / workflow handoff、provider replay redrive invocation / controlled execution、action-executor external audit append、provider replay submit operator、workflow provider replay queue view、workflow external approval binding、workflow external callback wait 和 workflow operator queue visibility cases，CI-safe fixture adapter 24 cases，memory-service live adapter 已新增 `must_recompute_profile_via_public_api`，retrieval positive adapter 已新增 `must_preserve_source_chain_rerank` 并通过 `ai-eval-service-stack-live-20260624-retrieval-source-chain-rerank-v2` 真实 service-stack gate 4 adapters / 27 cases / 27 passed / 0 failed / 0 skipped；2026-06-24 `ai-eval-service-stack-live-20260624-retrieval-negative` 真实 service-stack gate 9 adapters / 51 cases / 51 passed / 0 failed / 0 skipped；`ai-eval-rag-agent-demo-live-20260624-business-proposal-source-chain-gate-v1` 真实 gate 4 adapters / 27 cases / 27 passed / 0 failed / 0 skipped，验证 RAG grounded answer、Agent approval、profile repair workflow approval、profile repair negative fail-closed、memory-service public recompute、action-executor audit、group-memory 多事件 source-chain 和业务 proposal 审计边界；CI-safe gate 只跑不依赖服务栈的 required adapters；第一版不保存 raw prompt / EvidencePack / model output | `service-briefs/ai-eval-service.md` |
...
| `admin-service` | product-active，第一版 operation / approval 管理入口 + outbox relay + operation worker + operator CLI + control-plane / audit adapter + compensation-request operator / workflow handoff + provider replay workflow handoff / operator bridge 已落 | `admin_service.proto`、PostgreSQL core migration、六层 skeleton、`CreateAdminOperation` / `ApproveAdminOperation` / `GetAdminOperation` / `ListAdminOperations` app + gRPC adapter、PostgreSQL repository、high-risk separation-of-duty、低敏 `admin_outbox`、`grpc` / `operation-worker` / `outbox-relay` / `compensation-request` runtime mode、`im.admin.events` Kafka schema、outbox store、Kafka producer、trigger relay、本地 no-op executor、risk routing、`REPAIR_REQUEST -> workflow-service REPAIR_APPROVAL`、`PROVIDER_REPLAY_REQUEST -> workflow-service REPAIR_APPROVAL -> action-executor`、`CRITICAL -> workflow-service ADMIN_OPERATION`、operation-specific approval policy / target-service routing、非 critical `CONFIG_PUBLISH -> control-plane-service.PublishConfigVersion` adapter、非 critical `CONFIG_ROLLBACK -> control-plane-service.RollbackConfigVersion` adapter、非 critical `TENANT_QUOTA_CHANGE -> control-plane-service.PublishConfigVersion(API_GATEWAY_TENANT_QUOTA)` adapter、非 critical `POLICY_RULE_CHANGE -> control-plane-service.PublishConfigVersion(POLICY_RULESET_REF)` adapter、非 critical `AUDIT_EXPORT_REQUEST -> audit-service.CreateAuditExport` adapter、`admin_operation_results` result 投影、`loadtest/admin` 公开 gRPC create / approve / reject / get / list operator CLI、`provider-replay-submit` / `provider-replay-list` / `provider-replay-approve` / `provider-replay-reject` operator bridge、first-stage `compensation-request` 本地 operator、`COMPENSATION_REQUEST` workflow handoff、本地多进程 config publish / rollback / tenant quota / policy ruleset smoke、Docker / Prometheus / Grafana / worker compose wiring 已落；第一版仍不做其它真实下游 mutation、admin UI 或 provider-grade 运维 | `service-briefs/admin-service.md` |
...
低敏 collaborative-memory eval 已扩展到 multi-hop actor chain、workstream / decision dependency edge、
reviewed multi-source profile activation 和 supporting-memory delete -> profile recompute；
memory-service / retrieval-gateway optional live adapter first pass 已接入，RAG / Summary / Agent
