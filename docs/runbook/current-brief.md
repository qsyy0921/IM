...
11. push-gateway / identity-service 已完成低耦合真实鉴权主线：gateway token 本地验签、HS256/RS256/JWKS、issuer allowlist、RegisterUser / Login / RefreshGatewayToken、refresh rotation / reuse detection、device/session revoke event、push-gateway deny-list projection 和 active close 已落地；已跑通 `IssueGatewayToken -> push auth`、`Login(jwt) -> push auth`、`RegisterUser -> Login(jwt) -> push auth`、device/session revoke active close smoke。identity-service 生产化 hardening 已覆盖：密码 Login 失败计数 / 短锁定、Login 缺失/非 ACTIVE credential 的 dummy verifier timing hardening、email/phone verification 与 password reset challenge hash、challenge webhook sender 第一版及 delivery failure 后过期补偿、challenge delivery debug metrics / 持久状态审计 / encrypted outbox + worker retry-DLQ，并已跑通 `RequestVerificationChallenge(outbox) -> challenge-delivery-worker -> webhook token -> ConfirmVerificationChallenge` 真实进程 smoke；密码重置后撤销 ACTIVE session/refresh token、TOTP MFA 生命周期、Login 强制 MFA、TOTP factor 失败计数 / 短锁定、MFA recovery codes 生成 / 再生成 / 吊销、recovery-code 登录 user-level 失败计数 / 短锁定、`MFA_UNAVAILABLE` 稳定错误、Refresh-token step-up、Refresh 期间直接提交 TOTP / recovery MFA proof、Refresh proof 预校验保持 reuse / expired token 副作用一致、session MFA proof 组合约束、`/debug/metrics` 风险计数，以及 JWKS public-only 边界：HS256 对称密钥不再通过 identity JWKS 暴露，JWKS 只用于 RS256 公钥和旧公钥 overlap。WebAuthn、OIDC federation、自动 key rotation、KMS/HSM、多 issuer 治理、provider-specific email/SMS 模板 / bounce / DLQ repair audit / provider-grade alerting、完整 account-enumeration/timing 防护、更完整风控/限流、mTLS、统一 trace 和告警仍是后续项。详见 `docs/runbook/loadtest/identity-service/README.md`、`docs/runbook/loadtest/push-gateway/README.md` 和 `docs/runbook/loadtest/push-gateway/loadtest-report-20260612-push-gateway-identity-token-smoke.md`。
