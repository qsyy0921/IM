...
| `contacts-service` | 已落地、已接主链路 | contacts grpc / list search / group filter / outbox / audit / repair、first-stage Prometheus text `/metrics`、本地 Prometheus scrape / alert rules 原型、本地 Grafana dashboard 原型、first-stage OTel gRPC server span | 更多隐私策略、生产级 OTel collector / alerting / SLO dashboard |
...
8. `contacts-service`：first-stage `/metrics`、本地 Prometheus alert rules 和 Grafana dashboard 原型已补；`ListContacts.query` 已支持当前用户 ACTIVE edge 的 `contact_user_id / remark` 搜索，`group_name` 已支持联系人分组更新和列表分组过滤；后续继续更多隐私策略。
