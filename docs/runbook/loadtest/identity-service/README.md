...
- email/phone verification 与 password reset challenge，数据库只保存 token hash，并有 target-level active cap + request window throttle；配置 `NEXUSIM_IDENTITY_CHALLENGE_REQUEST_LIMIT_SECRET` 后，password reset 的无效 / 不存在目标也会进入 HMAC target limiter，并可用 one-shot cleanup operator 清理过期 limiter row。
...
- challenge 创建有本地防滥用：同一目标最多 3 个 ACTIVE challenge，默认 15 分钟内最多 5 次创建；password reset 的限流仍返回 neutral accepted，避免暴露账号存在性。无效目标 limiter 只保存 HMAC target key，不保存 raw email/phone；`challenge-request-limit-cleanup` 只删除窗口和锁都过期的 limiter row。
