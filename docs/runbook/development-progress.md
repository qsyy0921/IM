...
- `client-platform`：v0.1 SDD 已冻结浏览器、PC、Android 三端架构；`clients/`
  ...
  store 和 static lifecycle/network runtime adapter；下一步复用同一 core 接
  Tauri runner / local Windows artifact 和 Android native bridge / APK。
...
PC Tauri runner / Android native bridge。
...
baseline 已通过，PC desktop / Android first-stage runtime adapter 已落，下一步接
PC Tauri runner / local Windows artifact 和 Android native bridge / APK。
