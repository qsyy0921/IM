...
当前必要收口已推进到 policy-service tool policy precheck / low-sensitive audit。`search-service v0.1` 已跑通真实 projection smoke；`memory-service v0.1` 已跑通 source-backed group memory projection smoke；`retrieval-gateway` / EvidencePack 已完成第一轮真实本地 smoke，并已补 policy precheck、`rerank_score`、`dedupe_reason`、`source_coverage`；AI eval harness first pass 已新增低敏 case schema / validator；`rag-service` 已完成 first read-only answer path、真实本地 RAG adapter smoke、first-stage provider boundary 和 citation verifier；`summary-service` 已完成 first read-only EvidencePack summary path 和真实本地 `retrieval-gateway -> summary-service` adapter smoke；`agent-service` 已完成 first proposal-only path 和真实本地 `retrieval-gateway -> policy-service -> agent-service` adapter smoke，先通过 policy-service `CheckToolAction` 预检，再消费 retrieval-gateway EvidencePack 生成可引用 proposal，不执行真实动作；`skill-registry` 已完成 first catalog path；`mcp-gateway` 已完成 first prepare path，把 skill catalog、policy precheck 和低敏 audit 串起来但不执行外部工具；`action-executor` 已完成 first execution audit path，强制 proposal / approval / prepare audit 关联并只记录执行边界，`executed=false`。当前下一步推进 approval / real tool adapter 串接或下一项 AI 主线。完整系统测试、生产级 HA、长压和 sizing 后置为 hardening backlog。
