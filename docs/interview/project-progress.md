...
| Conversation-local fanout buckets | 已实现 registry 内部按 stable `session_id` bucket 并行 fanout，同一 session 仍由单 bucket 保序，queue full 仍显式 slow-session eviction；本地 Docker `push-gateway-ws` 配置 8 bucket | 这是代码实现和架构收口，不是压测结论。下一步需要 clean commit 镜像重建 / 归档 / redeploy，并用同一 400 subscriber coordinator + 4 shard 场景复压，比较 worker fanout p95 / p99 和 drain rate 是否突破 `fedb5f43`。 |
