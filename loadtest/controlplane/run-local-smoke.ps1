...
if (-not $RunName) {
    $RunName = "control-plane-grpc-smoke-" + (Get-Date -Format "yyyyMMdd-HHmmss")
}
...
if (-not $SkipBuild) {
    ...
    go build -o (Join-Path $repoRoot "bin\control-plane-smoke.exe") ./loadtest/controlplane
}
...
$port = [int]($ControlPlaneGrpcAddr.Split(":")[-1])

$processes = @()
try {
    $processes += Start-NexusProcess -Name "control-plane-service" -FilePath (Join-Path $repoRoot "bin\control-plane-service.exe") -Port $port -Env @{
    ...
    & (Join-Path $repoRoot "bin\control-plane-smoke.exe") `
        --pg-dsn $PgDsn `
        --target $ControlPlaneGrpcAddr `
        --result-root $ResultRoot `
        --run-name $RunName
    if ($LASTEXITCODE -ne 0) {
        throw "control-plane smoke failed with exit code $LASTEXITCODE"
    }
