...
| contacts-service SDD | 已新增 `docs/sdd/contacts-service.md` v0.1 Draft，定位为联系人 / 好友关系事实源；第一阶段只做 `SendContactRequest`、`RespondContactRequest(ACCEPT/DECLINE)`、`ListContacts`、`GetContactState`，并通过 `contacts_outbox -> im.contact.events` 发布联系人事件；明确不写 `conversation_members`、不自动创建会话、message-service 不同步依赖 contacts-service。下一步先做 SDD 评审和 P1 修正，再进入 proto / migration / 六层骨架。 |
...
- 2026-06-10：已新增 contacts-service SDD v0.1 Draft，作为第三层联系人 / 好友关系事实源。设计刻意和 conversation-service 解耦：好友关系不写 `conversation_members`，接受好友不自动创建会话，后续如需创建 direct conversation 必须通过显式 app port 或 saga；第一阶段只做好友申请、接受 / 拒绝、联系人列表和 contacts outbox。下一步先让评审线程复核 SDD，再进入 `contacts_service.proto`、`000001_contacts_core.sql`、六层骨架和最小 smoke。
