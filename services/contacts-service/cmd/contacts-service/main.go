...
func run() error {
	...
	switch mode {
	case "", "noop":
		log.Println("contacts-service runtime wiring is idle; set NEXUSIM_CONTACTS_SERVICE_MODE=grpc, outbox-relay, outbox-repair, outbox-repair-audit, or outbox-repair-cleanup")
		return nil
	case "grpc":
		return runGRPC()
	case "outbox-relay":
