...
type AnswerQuestionExecutor interface {
	Execute(context.Context, types.AnswerQuestionCommand) (types.AnswerQuestionResult, error)
}

type Server struct {
	ragv1.UnimplementedRagServiceServer
	answerQuestion AnswerQuestionExecutor
}

func NewServer(answer AnswerQuestionExecutor) *Server {
	return &Server{answerQuestion: answer}
}

func Register(registrar grpcgo.ServiceRegistrar, server *Server) {
	ragv1.RegisterRagServiceServer(registrar, server)
}

func (server *Server) AnswerQuestion(
	ctx context.Context,
	request *ragv1.AnswerQuestionRequest,
) (*ragv1.AnswerQuestionResponse, error) {
	if request == nil {
		return nil, status.Error(codes.InvalidArgument, "request is required")
	}
	if server == nil || server.answerQuestion == nil {
		return nil, publicError(types.ErrRAGUnavailable)
	}
	...
	return answerQuestionResultToProto(result), nil
}
...
func publicError(err error) error {
	...
	case errors.Is(err, types.ErrInvalidArgument):
		return status.Error(codes.InvalidArgument, "invalid rag request")
...
func answerQuestionResultToProto(result types.AnswerQuestionResult) *ragv1.AnswerQuestionResponse {
	citations := make([]*ragv1.Citation, 0, len(result.Citations))
	for _, citation := range result.Citations {
		citations = append(citations, &ragv1.Citation{
	...
	return &ragv1.AnswerQuestionResponse{
		AnswerId:       result.AnswerID,
		Status:         answerStatusToProto(result.Status),
		AnswerText:     result.AnswerText,
		Confidence:     result.Confidence,
		Citations:      citations,
		EvidencePack:   evidencePackToProto(result.EvidencePack),
		RagVersion:     result.RAGVersion,
...
func evidenceItemToProto(item types.EvidenceItem) *retrievalv1.EvidenceItem {
	...
	return &retrievalv1.EvidenceItem{
		...
		ProfileSubjectUserId:     string(item.ProfileSubjectUserID),
		ProfileAggregateType:     item.ProfileAggregateType,
		ProfileAggregateKey:      item.ProfileAggregateKey,
...
func answerStatusToProto(status string) ragv1.AnswerStatus {
	switch status {
	case types.AnswerStatusGrounded:
		return ragv1.AnswerStatus_ANSWER_STATUS_GROUNDED
	case types.AnswerStatusInsufficientEvidence:
		return ragv1.AnswerStatus_ANSWER_STATUS_INSUFFICIENT_EVIDENCE
	default:
		return ragv1.AnswerStatus_ANSWER_STATUS_UNSPECIFIED
	}
}
