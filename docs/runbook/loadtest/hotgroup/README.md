...
| `hotgroup-message-cdc-wal-shadow-plan-20260702.md` | Backend Lab 已实现 DB-first CDC/WAL shadow export：`cdc_shadow` 保留旧 `message_outbox` 主路径并把 `conversation_timeline_events` 通过 Debezium + `message-service cdc-bridge` 发布到 `conversation.timeline.events.cdc`；`cdc_only` 显式跳过 `message_outbox` 写入，只允许在 CDC health 和 shadow 对账通过后进入压测。该文件是实现 / 切换 runbook，不是正式容量结果。 |
