...
- 已支持 unread、pinned、muted、tag、draft-only 过滤，`UPDATED_AT` / `PINNED_UPDATED_AT` / `UNREAD_UPDATED_AT` 排序，以及 archive / pin / mute / tags / draft 的最小会话列表能力；分页 cursor 会绑定 sort / archived / unread / pinned / muted / tag / draft-only 过滤条件和排序边界，避免串页。
