...
func TestWriteOutboxAuditOutput(t *testing.T) {
	...
	outputPath := filepath.Join(t.TempDir(), "delivery-outbox-audit.json")
	...
	err := writeOutboxAuditOutput(outputPath, []postgresinfra.OutboxAuditRow{
		{
			...
			Status:           "DLQ",
			RetryCount:       5,
			LastError:        "delivery outbox publish failed",
	...
	if row.ID != 42 ||
		...
		row.EventType != "delivery.inbox_item.created.v1" ||
		row.Status != "DLQ" ||
		row.RetryCount != 5 ||
		row.LastError != "delivery outbox publish failed" ||
		row.AvailableAt == "" ||
		row.NextRetryAt == "" ||
		row.PublishedAt == "" ||
		row.DeadLetteredAt == "" ||
