...
- 已补 `projection-failure-audit` 只读模式：可直接列出 unresolved projection failure，减少手写 SQL 排障。
- 已补 `projection-failure-cleanup` operator：只删除超过保留期的 resolved failure 审计行，不会碰 unresolved blocker。
