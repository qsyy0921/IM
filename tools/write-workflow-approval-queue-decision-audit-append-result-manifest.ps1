...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path (Split-Path -Parent ([System.IO.Path]::GetFullPath($AuditAppendResultPath))) "workflow-approval-queue-decision-audit-append-result-manifest.json"
}
Assert-ExternalRepairOutputPath -Value $OutputPath -FieldName "OutputPath"

if ([string]::IsNullOrWhiteSpace($ResultManifestID)) {
    $ResultManifestID = "workflow-approval-decision-audit-append-result-" + [System.Guid]::NewGuid().ToString("N")
}
...
function Assert-NoRawText {
    ...
    if ($Value -match "(?i)(password|passwd|secret|token|bearer|credential|api[_-]?key|access[_-]?key|refresh|session|cookie|sk-|eyJ|postgres://|mysql://|mongodb://|raw:|payload_body|message_body|provider_body|provider_error|callback_body|decision_body|reason_text|EvidencePack|prompt|local_path|filesystem_path|input_json)") {
...
$auditManifest = Get-JsonDocument -Path $AuditManifestPath -Label "Workflow approval decision audit append manifest"
$auditResult = Get-JsonDocument -Path $AuditAppendResultPath -Label "Workflow approval decision audit append execution summary"
...
Assert-Same -Actual $recordType -Expected "WORKFLOW_APPROVAL_BATCH_DECISION" -FieldName "audit_manifest.record_type"
Assert-Same -Actual $action -Expected "RECORD_WORKFLOW_DECISIONS" -FieldName "audit_manifest.action"
Assert-Same -Actual $outcome -Expected "RECORDED" -FieldName "audit_manifest.outcome"
Assert-Same -Actual $reasonCode -Expected "WORKFLOW_APPROVAL_QUEUE_BATCH_DECISION_RECORDED" -FieldName "audit_manifest.reason_code"
...
$manifest = [ordered]@{
    schema_version = "nexusim.workflow.approval_queue_decision_audit_append_result.v1"
    ...
    executes_target_action = $false
    mutates_workflow_fact = $false
    ...
    note = "Low-sensitive workflow approval decision audit append result manifest. It binds an executed external audit append summary to its manifest but does not append audit, record workflow decisions, call action-executor, execute target actions, mutate workflow facts, or embed raw payloads."
}

$encoded = $manifest | ConvertTo-Json -Depth 30 -Compress
Assert-NoRawDocumentText -Value $encoded -FieldName "workflow approval decision audit append result manifest"
...
Write-Host "OK   workflow approval decision audit append result manifest written: $OutputPath"
