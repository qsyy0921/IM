...
| `contacts-service` | SDD v0.1 Draft 已新增；`contacts_service.proto`、`im.contact.events` schema、`000001_contacts_core.sql`、生成代码、六层骨架和 PostgreSQL repository 真实事务已落地；定位为联系人 / 好友关系事实源，第一阶段只做好友申请、接受 / 拒绝和当前联系人列表 | 下一步实现 contacts outbox relay 和真实进程 smoke；不得把好友关系写入 `conversation_members`，也不得自动创建会话 |
