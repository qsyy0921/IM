...
11. push-gateway 已新增第一版低耦合真实鉴权入口，并在 clean commit `8aa414c` 跑通 HMAC auth full smoke：`NEXUSIM_PUSH_AUTH_MODE=hmac` 时校验短期 signed gateway token 的 HMAC 签名、`aud=push-gateway`、过期时间和 device 绑定；runner 使用 `Authorization: Bearer`，`push_auth_query_identity_sent=false`，随后完整通过 `delivery.notify -> PullInbox -> delivery.ack.ok`。当前支持 current + previous secrets 的最小密钥轮换，smoke runner 已能用 previous secret 签发 token 验证兼容窗口。identity-service 已完成最小独立边界：proto / migration / 六层骨架、users/devices/sessions、短期 gateway token 签发、device/session revoke 状态，并已跑通 `IssueGatewayToken -> push-gateway HMAC auth -> delivery.notify -> PullInbox -> AckDelivery` smoke；push-gateway 仍本地验签，不在 WebSocket 热路径同步 RPC 依赖 identity-service。identity-service 已补第一版 `/healthz`、`/readyz`、`/debug/metrics`、pgx pool/identity count/gRPC method/code/latency 指标、JSON gRPC 请求日志、`NEXUSIM_IDENTITY_ADMIN_AUTH_MODE=metadata` 的 admin/read-state RPC gateway verified metadata 模式，以及 `identity_outbox -> im.identity.events` 的 device/session revoke 异步事件发布链路。clean commit `148d959` 已补 push-gateway identity revoke projection：HMAC token 解析 `session_id`，`identity-consumer` 消费 `im.identity.events` 写入 in-memory / Redis deny-list；Redis 分进程 smoke 证明 `RevokeDevice -> identity_outbox -> im.identity.events -> push-gateway identity-consumer -> Redis deny-list` 生效后，旧 gateway token 重连返回 `PERMISSION_DENIED`。clean commit `4d38a50` 已补 device revoke active close：旧 WebSocket 在线时会收到 `server.resume_hint(reason=identity_revoked)` 并以 `StatusPolicyViolation` 关闭，随后旧 token 重连仍返回 `PERMISSION_DENIED`。clean commit `37dd031` 已补 session revoke active close：同一 device 的两条 session 中，被吊销 session 收到 `server.resume_hint(reason=identity_revoked)` 并关闭，survivor session 仍可 `server.pong`，旧被吊销 token 重连返回 `PERMISSION_DENIED`。clean commit `dbca0e6` 已补标准三段 JWT HS256 gateway token 兼容和 identity debug JWKS，并跑通 `IssueGatewayToken(jwt) -> push-gateway local verify -> delivery.notify -> PullInbox -> AckDelivery` smoke；当前 HS256 JWK 仅用于本地 / 内部调试，生产仍需非对称 JWK key ring。当前已补 RegisterUser / Login / RefreshGatewayToken 最小实现：RegisterUser 创建 ACTIVE 用户凭据，Login 校验 `identity_users.password_hash`，创建 session 和 opaque refresh token；RefreshGatewayToken 执行 refresh token rotation，旧 token 标记 USED，新 token ACTIVE，过期 token REVOKED，重复使用旧 token 会吊销 session 并写 `identity.session.revoked.v1` outbox。clean commit `a5386aa` 已跑通 `Login(jwt) -> push-gateway local verify -> delivery.notify -> PullInbox -> AckDelivery` smoke；clean commit `b2136b6` 已跑通 `RegisterUser -> Login(jwt) -> push-gateway local verify -> delivery.notify -> PullInbox -> AckDelivery` smoke，summary 在 `H:\NexusIM\loadtest-results\push-gateway-identity-register-login-jwt-20260612-201330\pushgateway-summary.json`，`git_dirty=false`。后续仍需邮箱 / 手机验证、密码重置、MFA、OIDC federation、多 issuer、非对称 JWK key ring、登录风控、mTLS、统一 trace 和告警。报告见 `docs/runbook/loadtest/push-gateway/loadtest-report-20260612-push-gateway-identity-token-smoke.md` 和 `docs/runbook/loadtest/push-gateway/loadtest-report-20260612-push-gateway-identity-revoke-smoke.md`。
