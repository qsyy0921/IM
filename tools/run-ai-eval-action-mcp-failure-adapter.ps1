...
function Get-SmokeCase {
    ...
    throw "external MCP failure smoke case missing: $CaseID"
}

function Test-MCPFailureAssertion {
    param(
        $SmokeSummary,
        $EvalCase,
        $Assertion
    ...
    switch ($type) {
        "must_classify_mcp_failure" {
            ...
            Assert-Condition ($expected.Length -gt 0) "must_classify_mcp_failure requires expected_classification"
            return `
        ...
        default {
            throw "unsupported external MCP failure eval assertion type: $type"
        }
    }
}
...
if ([string]::IsNullOrWhiteSpace($RunName)) {
    $RunName = "action-external-mcp-failure-eval-" + (Get-Date).ToUniversalTime().ToString("yyyyMMdd-HHmmss")
}
...
$smokeSummaryPath = Join-Path $resultDir "action-external-mcp-unavailable-smoke.json"

Push-Location $repoRoot
try {
    & go run ./services/action-executor/cmd/action-external-mcp-unavailable-smoke -output $smokeSummaryPath
    if ($LASTEXITCODE -ne 0) {
        throw "action external MCP failure smoke failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
...
foreach ($case in @($caseDocument.cases)) {
    ...
    if ($stage -ne "action-executor-mcp-failure" -or $status -ne "active") {
        continue
    }
    ...
    foreach ($assertion in @($case.required_assertions)) {
        $type = Get-JsonPropertyString -Object $assertion -Name "type"
        $passed = Test-MCPFailureAssertion -SmokeSummary $smokeSummary -EvalCase $case -Assertion $assertion
        $assertionResults.Add([pscustomobject]@{
            type = $type
            passed = $passed
        })
        Assert-Condition $passed "External MCP failure eval assertion failed for case $($case.id): $type"
    }
...
Assert-Condition ($caseResults.Count -gt 0) "No active action-executor MCP failure eval cases found in $resolvedCasePath"

$adapterSummary = [pscustomobject]@{
    schema_version = 1
    adapter = "action-external-mcp-failure"
    generated_at = (Get-Date).ToUniversalTime().ToString("o")
    scope = "first-stage action-executor external MCP failure eval; local failure executor only, no external network and not a production benchmark"
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $resultDir "action-external-mcp-failure-eval-summary.json"
}
...
Write-Host "OK   action external MCP failure eval summary written: $resolvedOutputPath"
