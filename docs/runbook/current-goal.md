...
当前优先级：
1. 先治理已有 9 个微服务，新增服务后置；api-gateway 已补 first-stage tenant-scoped rate limit 和静态 tenant plan override，下一步继续 legacy descriptor 显式 opt-in、OpenTelemetry trace 和运行时动态 quota 配置。`search-service` 只保留 SDD draft，不进入 proto / migration / skeleton。
