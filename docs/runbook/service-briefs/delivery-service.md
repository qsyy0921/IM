...
- 已补 `outbox-audit` / `outbox-repair` / `outbox-repair-audit` / `outbox-repair-cleanup`，可审计、redrive `delivery_outbox` DLQ 并清理 repair audit 历史；`outbox-audit` / `outbox-repair` / `outbox-repair-audit` / `outbox-repair-cleanup` 可通过 `NEXUSIM_DELIVERY_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_DELIVERY_OUTBOX_REPAIR_OUTPUT` / `NEXUSIM_DELIVERY_OUTBOX_REPAIR_AUDIT_OUTPUT` / `NEXUSIM_DELIVERY_OUTBOX_REPAIR_CLEANUP_OUTPUT` 写低敏 JSON 结果或 cleanup summary，便于 operator 留存证据；`outbox-audit` 会在 JSON 输出当前 compacted filters；`outbox-repair-audit` 支持按 outbox / event / tenant / conversation / mode / outcome / `repaired_at` RFC3339 时间窗口过滤，并在 JSON 输出 compacted filters。
