...
后续按 real tool adapter / tool result projection / approval operator 串接继续推进；
...
| `action-executor` | 第一版 approved execution audit 边界已落；强制 proposal / approval / prepare audit 关联，重新做 skill execute contract check 和 policy execute precheck，只记录低敏审计，`executed=false` |
