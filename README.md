...
| 向量索引 | vector-index-service；PostgreSQL metadata、pgvector optional adapter、embedding queue；pgvector / OpenSearch vector / Milvus provider preflight gates、provider readiness matrix、retrieval `provider_coverage` summary contract | Qdrant、Weaviate、真实 pgvector / Milvus / OpenSearch vector provider smoke | 向量是可重建 projection，不是消息事实源；preflight / readiness / coverage contract 只证明 runtime / mapping / reporting contract，不代表真实 provider smoke 已完成。 |
