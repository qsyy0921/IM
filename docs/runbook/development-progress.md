...
| `search-service` | 已开始第一实现切片 | `search_service.proto`、PostgreSQL core migration、六层 skeleton、`SearchMessages` app / domain / gRPC adapter、projection usecase skeleton、PostgreSQL repository、真实 PG visibility / tombstone 集成测试和 `grpc` runtime mode 已落地；下一步是 timeline decoder / consumer 和 projection smoke；定位为 search projection / visibility / tombstone / EvidencePack 前置，不做 LLM demo | `service-briefs/search-service.md` |
...
search-service v0.1 第一实现切片已继续推进到 PG repository / SearchMessages / grpc runtime，后续是 timeline consumer、memory-service / retrieval-gateway / RAG / summary-service / Agent / skill-registry / mcp-gateway / action-executor。
