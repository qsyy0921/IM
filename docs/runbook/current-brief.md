...
11. conversation-service 已补第三层群管理最小读能力：`ListConversationMembers` 只返回当前 ACTIVE 成员，调用者必须是 ACTIVE 成员；clean commit `99aacc6` 的 `JOIN` roster smoke、clean commit `14ffedc` 的 `LEAVE` roster smoke、clean commit `be2e039` 的 `REMOVE` roster smoke 和 clean commit `7150944` 的 `ROLE_CHANGED` roster smoke 均通过，证明 JOIN 后出现、LEAVE/REMOVE 后消失、ROLE_CHANGED 后当前 role 更新。它是低耦合 roster API，不让其它服务跨表读取 `conversation_members`，也不把成员历史 / 审计视图塞进普通列表。下一条群管理切片已先冻结 owner transfer SDD：必须用专用 `TransferConversationOwner` / `conversation.member.owner_transferred.v1`，不能复用 `ROLE_CHANGED`，实现要分 proto/schema/migration/relay/projection 与 repository/RPC/smoke 多阶段推进。
