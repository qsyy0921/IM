...
| `contacts-service` | 隐私例外查询 / 清理 / 管理面、陌生人策略、后续接入 admin/config service 正式权限面；当前已有 first-stage ALLOW-DENY 隐私例外、字段级 profile 可见性、第一阶段 Prometheus text `/metrics`、本地 alert rules 和本地 Grafana dashboard，但还不是生产观测平台 |
