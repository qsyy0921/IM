...
- RAG / Agent live gate 19 cases；Summary live 2 cases；profile / group-memory fixture 9 cases；Python worker 5 cases；RAG/Summary citation、external MCP fallback、Agent output regression、action preflight / rate-limit / DLQ-repair safety eval 和 action provider failure worker / redrive safety eval 已落。
...
下一步：memory-service runtime source-ref / validity / supersession projection/query smoke。
