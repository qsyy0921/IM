...
2. 对 outbox 候选 `batch_size=100/workers=8` 和 `batch_size=500/workers=8` 各重复 2 轮，确认 1200/1600 VU 下的波动范围。
3. 补 active/idle relay metrics 或 `outbox_fetched_per_call`，避免 adaptive limit 使用被 `stats_wait` idle 样本稀释的 `outbox_process_ready_latency_ms`。
4. 设计更细的 adaptive limit，把 outbox pending、relay active process ready、Kafka records/call 和 PG pool acquire 纳入输入，而不是只看瞬时 acquired conns。
...
7. 当前 PublishBatch 开关和正式矩阵已跨并发/消息发布语义，下一次较大代码或压测报告完成后再统一邀请评审线程，避免过于频繁。
