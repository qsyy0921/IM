...
if (-not $RunName) {
    $RunName = "redis-sentinel-network-partition-smoke-" + (Get-Date -Format "yyyyMMdd-HHmmss")
}
...
function Apply-CoreMigrations {
    ...
    foreach ($dir in $migrationSets) {
        foreach ($migration in Get-ChildItem -LiteralPath (Join-Path $repo $dir) -Filter "*.sql" | Sort-Object Name) {
            Apply-PostgresMigration -Path $migration.FullName -Name ("redis-network-partition-" + $migration.Name)
        }
    }
}
...
$summaryPath = Join-Path $resultDir "redis-sentinel-network-partition-summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "redis_sentinel_network_partition_summary=$summaryPath"
