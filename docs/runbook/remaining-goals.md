...
- `admin-service`：`REPAIR_REQUEST -> workflow-service REPAIR_APPROVAL`、
  ...
  更多补偿 adapter、provider-grade compensation instruction 审批 / UI 管理和运维。
...
更多 compensation adapter、provider-grade instruction UI / external approval binding、external callback
wait、outbox relay、workflow repair operators 和完整 action / repair / admin operation
smoke。
