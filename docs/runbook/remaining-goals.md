...
2. Memory / retrieval 深化：
   ...
   Summary / Agent API 显式透传 `at_conversation_seq` 已落；下一步补
   current-memory regression / eval，验证 RAG、Summary、Agent 不引用过期或
   superseded memory，继续保留 source-ref / visibility / review 边界。
