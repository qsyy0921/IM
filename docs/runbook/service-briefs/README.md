...
- 短期不以生产级完整系统测试或生产级 HA 作为转进阻塞；后续顺序是 memory / retrieval / RAG / summary / Agent / skill-registry / MCP gateway / action-executor。
- `memory-service` 第一版必须按 group memory 设计，而不是普通摘要缓存：source refs、speaker / audience、validity window、supersedes、confidence、review state 是基本字段。
