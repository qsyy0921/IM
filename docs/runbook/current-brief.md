...
4. api-gateway 已补 first-stage tenant-scoped rate limit、静态 tenant plan override 和 legacy descriptor 显式 opt-in 默认；下一步继续治理 api-gateway：OpenTelemetry trace、运行时动态 quota 配置和 legacy opt-in 使用面迁移审计。
