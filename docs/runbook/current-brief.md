...
12. contacts-service 已完成好友申请、接受、拒绝、删除、拉黑、备注名最小闭环。clean commit `c1893c1` 的 `delete` / `block` / `remark` 三条 smoke 均通过，报告见 `docs/runbook/loadtest/contacts-service/loadtest-report-20260611-contacts-edge-management-smoke.md`。contacts-service 仍保持独立事实源，不写 `conversation_members`，不自动创建会话，message-service 不同步依赖 contacts-service。
