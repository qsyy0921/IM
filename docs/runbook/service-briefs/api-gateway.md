...
- `/debug/metrics` 已暴露低敏 gRPC、auth JWK、rate-limit key scope / tenant plan count / identity error count、runtime config 和 OTel trace config 快照，其中包含 legacy descriptor 是否仍注册。
...
- 运行时动态 quota 配置、OTel collector / alerting / 跨服务 rollout、legacy descriptor opt-in 使用面的后续迁移审计。
