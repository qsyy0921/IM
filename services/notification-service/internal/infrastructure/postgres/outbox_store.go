...
func (store *OutboxStore) ProcessReadyBatch(
	...
	if store == nil || store.pool == nil {
		return types.OutboxRelayStats{}, errors.New("notification outbox store is not configured")
	}
	if publish == nil {
		return types.OutboxRelayStats{}, errors.New("notification outbox publish callback is not configured")
	}
	if limit <= 0 {
		limit = 500
	}
	...
	if len(publishErrors) != len(messages) {
		return types.OutboxRelayStats{}, errors.New("notification outbox batch publish result count mismatch")
	}
	...
	for index, message := range messages {
		if err := publishErrors[index]; err != nil {
			lastError := sanitizeNotificationOutboxPublishError(err)
...
SELECT
    ...
    retry_count,
    created_at
FROM notification_outbox current
WHERE status = 'PENDING'
  ...
  AND NOT EXISTS (
      ...
      WHERE previous.tenant_id = current.tenant_id
        AND previous.request_id = current.request_id
...
`, limit)
	...
	for rows.Next() {
		...
		message.Producer = "notification-service"
...
WHERE notification_outbox.tenant_id = keys.tenant_id
  AND notification_outbox.event_id = keys.event_id
`, tenantIDs, eventIDs, types.OutboxStatusPublished, publishedAt)
	if err != nil {
		return types.NewDBWriteFailed(err.Error())
	}
	if tag.RowsAffected() != int64(len(messages)) {
		return types.NewDBWriteFailed("notification outbox published row count mismatch")
	}
	return nil
}
...
func sanitizeNotificationOutboxPublishError(err error) string {
	if err == nil {
		return ""
	}
	if errors.Is(err, context.Canceled) {
		return "notification outbox publish canceled"
	}
	if errors.Is(err, context.DeadlineExceeded) {
		return "notification outbox publish timeout"
	}
	return sanitizeNotificationOutboxErrorText(err.Error())
}

func sanitizeNotificationOutboxErrorText(value string) string {
	message := strings.ToLower(strings.TrimSpace(value))
	switch {
	case message == "":
		return "notification outbox publish failed"
	case strings.Contains(message, "cancel"):
		return "notification outbox publish canceled"
	case strings.Contains(message, "timeout") || strings.Contains(message, "deadline exceeded"):
		return "notification outbox publish timeout"
	case strings.Contains(message, "unsupported"):
		return "notification outbox publish unsupported event"
	case strings.Contains(message, "malformed") ||
		...
		return "notification outbox publish invalid payload"
	case strings.Contains(message, "kafka") ||
		...
		return "notification outbox publish broker unavailable"
	default:
		return "notification outbox publish failed"
	}
}
