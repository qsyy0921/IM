...
- contacts-service、identity-service、message-service、conversation-service、delivery-service、receipt-service、policy-service 已开始后端服务 gRPC server span rollout；本地 OTel collector debug 入口、policy OTLP smoke 脚本和 first-stage trace sampling policy / check 已补；后续继续补其它后端服务 server span rollout 和采样治理 hardening；
