...
- `/debug/metrics` 已暴露低敏 gRPC、auth JWK、rate-limit 和 runtime config 快照，其中包含 legacy descriptor 是否仍注册。
...
- 统一 OpenTelemetry、租户级配额治理、逐步把 legacy descriptors 从兼容默认迁移到显式 opt-in。
