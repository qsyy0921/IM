...
- challenge delivery repair / audit：一次性 operator mode 支持 `audit`、`redrive-active-pending`、`cancel-inactive`；另有只读 `challenge-delivery-repair-audit` 直接查询 repair 历史，以及 `challenge-delivery-repair-cleanup` 按 retention / scope 清理旧 audit 行。DLQ row 不会复活旧 challenge token，必须走正常 API 重新申请 challenge。
...
| `loadtest-report-20260614-identity-api-gateway-facade-smoke.md` | `RegisterUser -> RequestVerificationChallenge(outbox) -> ConfirmVerificationChallenge -> Login -> RefreshGatewayToken -> RequestPasswordReset -> ConfirmPasswordReset -> post-reset Login -> MFA lifecycle -> Refresh step-up` 经 api-gateway `GatewayService` facade 的真实进程 smoke |
