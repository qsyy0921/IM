...
| `contacts-service` | SDD v0.1 Draft 已新增；`contacts_service.proto`、`im.contact.events` schema、`000001_contacts_core.sql`、生成代码和六层最小可编译骨架已落地；定位为联系人 / 好友关系事实源，第一阶段只做好友申请、接受 / 拒绝和当前联系人列表 | 下一步实现 PostgreSQL repository/usecase 和 contacts outbox relay；不得把好友关系写入 `conversation_members`，也不得自动创建会话 |
