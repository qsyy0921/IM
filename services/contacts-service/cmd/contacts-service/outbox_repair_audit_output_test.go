...
func TestWriteOutboxRepairAuditOutput(t *testing.T) {
	...
	outputPath := filepath.Join(t.TempDir(), "contacts-outbox-repair-audit.json")
	...
	err := writeOutboxRepairAuditOutput(outputPath, []postgresinfra.OutboxRepairAuditRow{
		{
			...
			PreviousLastError:      "contacts outbox publish failed",
	...
	if row.EventID != "event-1" ||
		...
		row.PreviousLastError != "contacts outbox publish failed" ||
