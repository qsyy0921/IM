...
2. 分布式 HA / 故障演练深化：
   继续补更长时间 Kafka ISR flapping、consumer rebalance、producer retry budget；继续完善 Redis Cluster / 网络分区组合、PostgreSQL quorum / split-brain fencing 和服务发现 / 部署编排。PostgreSQL 生产 quorum 边界以 ADR-034 为准。
...
| `receipt-service` | 送达回执扩展；会话列表更多产品化能力。 |
