...
func TestAuthHelpersUseExpectedIdentities(t *testing.T) {
	...
	if got := conversationAuth(auth); got.GetTenantId() != "tenant-1" || got.GetUserId() != "user-1" || got.GetDeviceId() != "device-1" {
		t.Fatalf("unexpected conversation auth: %+v", got)
	}
	if got := deliveryAuth(auth); got.GetTenantId() != "tenant-1" || got.GetUserId() != "user-1" || got.GetDeviceId() != "device-1" {
