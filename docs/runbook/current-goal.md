...
| 本地双机网络 | Win-Mac 压测网络已改为有线直连：Windows `172.31.50.1/24`，Mac `172.31.50.2/24`，链路 `2.5Gbps`；两端 Wi-Fi 继续负责上网。后续双机压测、SSH 和 callback/mock receiver 优先使用 `172.31.50.*`，历史 `192.168.0.*` 地址只作为 Wi-Fi fallback 和旧报告事实 |
| 本地分布式 runbook | `docs/runbook/distributed-local.md` 已新增，定义当前四个真实微服务 / 网关的本地多进程分布式拓扑、`tools/local-distributed-smoke.ps1` 入口、可讲证据和限制；已补 Win/Mac 双机 Docker 节点模拟计划：Windows `172.31.50.1/24`、Mac `172.31.50.2/24`，后续优先用有线直连承载服务间通信 |
...
5. 当前系统可以表述为“本地多进程最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立进程协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route 和 WebSocket notify 串联。下一步优先配置 Mac Docker / Go / repo 环境，跑 Win-Mac 有线直连 smoke；同时补 resume buffer TTL，并单独设计 resume replay smoke。不要把当前本地分布式 smoke 表述为完整生产多实例方案。
...
- 2026-06-09：已检测 Win-Mac 有线 / Wi-Fi SSH 入口：`172.31.50.2:22` 与 `192.168.0.182:22` 均可达，但 Windows -> Mac 免密 SSH 当前返回 `Permission denied (publickey,password,keyboard-interactive)`；Mac Docker 配置和双机 Docker smoke 暂时阻塞于 Mac `authorized_keys` 未接受 Windows 当前公钥。下一步先恢复免密，再验证 Mac `docker version/docker info`。
