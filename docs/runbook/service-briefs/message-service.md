...
- 已补 first-stage compliance delete approval：`message_compliance_delete_approvals` 只保存低敏 external proof ref，不保存 proof 正文；`compliance-approval-audit` / `compliance-approval-create` / `compliance-approval-cancel` operator 可写低敏 JSON 结果，输出只暴露 reason-present 和 proof ref。
...
- 会话级删除策略深化、外部 proof 链路、长时间容量曲线和生产观测；用户私有隐藏已由 delivery-service `HideInboxItem` 承担，图片 / 文件 / 语音二进制上传和处理属于后续 media 能力。
