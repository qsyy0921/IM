...
| 本地分布式 runbook | `docs/runbook/distributed-local.md` 已新增，定义当前四个真实微服务 / 网关的本地多进程分布式拓扑、`tools/local-distributed-smoke.ps1` 入口、可讲证据和限制；已补 Win/Mac 双机 Docker 节点模拟计划：Windows `172.31.50.1/24`、Mac `172.31.50.2/24`，后续优先用有线直连承载服务间通信；Windows -> Mac 免密 SSH 已恢复，Mac Docker CLI 可用，Docker Desktop 29.5.2，当前 Docker 资源池约 8 CPU / 8GB；`tools/sync-mac-distributed-smoke.ps1` 可通过有线 SSH/scp 传 Git bundle 和 darwin/arm64 二进制到 Mac 专用 checkout |
