...
| `workflow-service` | workflow creation / decision、approval 状态机、`timer-worker` 显式 approval timeout -> `TIMED_OUT`、`ListWorkflows` 低敏队列视图、provider replay queue operator view、operator queues、external decision manifest binding、compensation request / instruction registry / rollback adapter first path。 |
