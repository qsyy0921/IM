...
```text
push-gateway multi-device notify smoke
...
5. push-gateway 后续优先补 Redis route、resume buffer 和慢连接主动关闭；不要把当前 in-memory `all` 模式表述为多实例生产方案。
