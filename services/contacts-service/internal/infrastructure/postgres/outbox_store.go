...
func (store *OutboxStore) ProcessReadyBatch(
	...
	if store == nil || store.pool == nil {
		return types.OutboxRelayStats{}, errors.New("contacts outbox store is not configured")
	}
	if publish == nil {
		return types.OutboxRelayStats{}, errors.New("contacts outbox publish callback is not configured")
	}
	if limit <= 0 {
		limit = 500
	}
	...
	if len(publishErrors) != len(messages) {
		return types.OutboxRelayStats{}, errors.New("contacts outbox batch publish result count mismatch")
	}
...
func (store *OutboxStore) RepairDLQEvents(ctx context.Context, eventIDs []string, reason string) (types.OutboxRepairStats, error) {
	if store == nil || store.pool == nil {
		return types.OutboxRepairStats{}, errors.New("contacts outbox store is not configured")
	}
	...
	if strings.TrimSpace(reason) == "" {
		reason = "manual contacts outbox repair"
	}
...
func (store *OutboxStore) CleanupOutboxRepairs(ctx context.Context, options OutboxRepairCleanupOptions) (OutboxRepairCleanupStats, error) {
	if store == nil || store.pool == nil {
		return OutboxRepairCleanupStats{}, errors.New("contacts outbox store is not configured")
	}
	if options.Limit <= 0 {
...
DELETE FROM contacts_outbox_repair_audit target
USING doomed
...
SELECT
    ...
    retry_count,
    created_at
FROM contacts_outbox current
WHERE status = 'PENDING'
  AND published_at IS NULL
  AND COALESCE(next_retry_at, available_at) <= now()
  AND NOT EXISTS (
      SELECT 1
      FROM contacts_outbox previous
...
`, ids, types.OutboxStatusPublished, publishedAt)
	...
	if tag.RowsAffected() != int64(len(ids)) {
		return types.NewDBWriteFailed("contacts outbox published row count mismatch")
	}
	return nil
}
