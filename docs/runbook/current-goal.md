...
- Web / PC shell 已接账号密码登录、注册、好友工作台、点击好友打开私聊、
  ...
  owner transfer 第一路径、群成员搜索 / 角色过滤 / 分页、群资料卡、邀请来源提示、群设置操作区、消息列表、发送后本地状态刷新、
  PullInbox 和 ACK。
...
- Windows desktop artifact collector 已能在 collected package 中写入
  ...
  Windows Authenticode public status，不签名、不安装、不启动、不下载；2026-06-23 已重新
  collect 新格式 `desktop-executable` artifact，manifest 为
  `clients/artifacts/2026-06-22T214826Z/manifest.json`，git commit
  `65dd5f3c`，签名状态为 `NotSigned`。desktop signing / signing execution /
...
2. 若继续客户端产品能力，优先补群标题 / 头像 read model 编辑能力、完整群设置和更丰富的真实多用户 UI smoke。
