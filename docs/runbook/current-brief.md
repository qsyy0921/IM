...
4. 当前先治理已有 9 个微服务；identity repository、repository test、`loadtest/pushgateway/main.go` 和长目标架构文档已完成拆分，delivery-service 已补 `/debug/metrics`、只读 `outbox-audit`、`outbox-repair`、只读 `outbox-repair-audit`、按 retention/scope 清理 outbox repair audit 历史、`projection-checkpoint-repair`、只读 `projection-checkpoint-repair-audit`、按 retention/class/scope 清理 repair audit 历史、projection failure audit、resolved 标记、按 unresolved failure 定点 replay、按最早 unresolved failure 自动 rewind、只读 `projection-failure-audit` 及其按 offset/event/class 的定点过滤、以及按 class/scope 过滤的 resolved failure cleanup operator；identity-service 已补低敏 identity / MFA / challenge-delivery 聚合观测；policy-service 已补低敏 gRPC / decision / rule-store / projection / audit-outbox 聚合观测；conversation-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 gRPC/PG/member-change 聚合观测；contacts-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 contact request / edge / outbox 聚合观测；push-gateway 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 session / resume / Redis route / auth JWK 聚合观测，并区分 Redis publish error 与 0-subscriber stale-route 场景；receipt-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 receipt projection / outbox 聚合观测；message-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 send / repository / outbox relay 聚合观测，后续继续处理各服务 P2 hardening。
