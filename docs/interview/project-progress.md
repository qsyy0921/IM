...
| `contacts-service` | 字段级 profile 可见性、黑白名单、陌生人策略、后续接入 admin/config service 正式权限面；当前已有第一阶段 Prometheus text `/metrics`、本地 alert rules 和本地 Grafana dashboard，但还不是生产观测平台 |
