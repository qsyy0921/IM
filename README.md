...
| 工作流 / 审批 | workflow-service 内部状态机、compensation instruction registry、external callback delivery persistent worker first path | Temporal、Cadence、Argo Workflows | 审批和补偿审计归 workflow / audit 所有；callback worker 只投递低敏 refs，不记录 decision、不执行 target。 |
...
| `workflow-service` | workflow creation / decision、approval 状态机、`timer-worker` 显式 approval timeout -> `TIMED_OUT`、`ListWorkflows` 低敏队列视图、provider replay queue operator view、operator queues、external decision manifest binding、external callback wait、external callback delivery plan、delivery status / redrive plan、external callback delivery persistent worker first path、compensation request / instruction registry / rollback adapter first path、compensation review page 和 execution readiness manifest。 |
