...
| `hotgroup-metrics-window-20260701-routecache-400sub-5000msg.md` | route cache 复压窗口显示核心 target up，`delivery_outbox_pending` 峰值 1985 后归零，WebSocket `delivery_notify` write p95 / p99 约 0.241ms / 0.433ms，queue p95 / p99 约 1.928ms / 2.529ms，Redis subscriber fanout p95 / p99 约 1.96ms / 6.25ms。当前 Prometheus 只 scrape 4 个 ws debug target，delivery-consumer 未暴露 debug 端口，因此 route cache hit / miss 在窗口中仍为 0，这是观测缺口。 |
