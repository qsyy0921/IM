...
| 搜索 | search-service projection；PostgreSQL FTS lexical first path；显式 OpenSearch / BM25 candidate backend first path | OpenSearch 集群 smoke、Elasticsearch、Meilisearch、provider-grade BM25 运维 | 只有 search-service 写搜索索引；外部索引只召回候选，最终 visibility / tombstone 仍由 PostgreSQL projection 过滤。 |
...
| 服务 / 模块 | 当前状态 |
| --- | --- |
| `search-service` | 搜索 projection、visibility / tombstone、PostgreSQL FTS lexical `SearchMessages`、显式 OpenSearch / BM25 candidate backend、timeline consumer、projection smoke；外部索引只召回候选，PG projection 负责最终过滤。 |
