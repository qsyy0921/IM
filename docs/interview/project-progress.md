...
| `workflow-service` | Agent / repair / retention 的长事务、审批等待、补偿和 operator workflow；当前已支撑 action / repair / admin operation approval first path、显式 approval timeout -> `TIMED_OUT` 和 provider replay queue 低敏队列视图 |
