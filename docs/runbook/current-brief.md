...
4. api-gateway 已补 first-stage tenant-scoped rate limit、静态 tenant plan override、tenant plan 文件热更新、legacy descriptor 显式 opt-in 默认和 first-stage OpenTelemetry gRPC server span；下一步继续治理 api-gateway：OTel collector / alerting / 跨服务 rollout、legacy opt-in 使用面迁移审计和配置中心 / DB-backed quota hardening。
