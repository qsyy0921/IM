...
- `admin-service`：`REPAIR_REQUEST -> workflow-service REPAIR_APPROVAL` 和
  `CRITICAL -> workflow-service ADMIN_OPERATION` 第一版已接；后续仍需更多
  operation 专用 workflow adapter、audit ingestion / export、operator approval CLI、
  admin UI 和下游公开 admin API adapter。
