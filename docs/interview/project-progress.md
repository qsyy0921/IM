...
| 三机资源曲线 | `tools/record-lab-resource-window.ps1` 通过 Windows 本机采样和 SSH 采样 Ubuntu / Mac，输出 H 盘 CSV、Markdown summary 和 SVG 曲线 | 后续正式热点群压测必须附 Windows / Ubuntu / Mac CPU 与内存时间序列。工具已改为远端脚本 base64 执行，避免 Windows / SSH / bash / zsh 多层引号问题；远端机器采样全失败会直接失败，不能生成缺机器的误导性曲线。 |
...
| Hotgroup 并发发压修正 | 当前 runner 已新增 `--send-concurrency` / `NEXUSIM_HOTGROUP_SEND_CONCURRENCY`，默认使用 `sender-count`，并把 `send_concurrency`、`send_duration_seconds`、`achieved_send_rate` 写入 summary 和分析报告 | 旧 runner 是单 goroutine 同步调用 SendMessage，256 sender 并没有形成 256 路并发，因此上一轮 66.741 msg/s 不能代表服务端上限。这个修正把压测工具从“功能回归 runner”推进为更接近真实多用户同时发消息的压力源。下一步要用 clean commit 复跑 total-subscriber 场景，观察瓶颈是否迁移到 message-service、Kafka、delivery projection 或 push signal drain。 |
