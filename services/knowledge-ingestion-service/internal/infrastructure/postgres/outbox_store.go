...
func (store *OutboxStore) ProcessReadyBatch(
	...
	if store == nil || store.pool == nil {
		return types.OutboxRelayStats{}, errors.New("knowledge outbox store is not configured")
	}
	if publish == nil {
		return types.OutboxRelayStats{}, errors.New("knowledge outbox publish callback is not configured")
	}
	if limit <= 0 {
		limit = 500
	}
	...
	if len(publishErrors) != len(messages) {
		return types.OutboxRelayStats{}, errors.New("knowledge outbox batch publish result count mismatch")
	}
...
SELECT
    ...
    retry_count,
    created_at
FROM knowledge_outbox current
WHERE status = 'PENDING'
  AND published_at IS NULL
  AND COALESCE(next_retry_at, available_at) <= now()
  AND NOT EXISTS (
      SELECT 1
      FROM knowledge_outbox previous
...
`, limit)
	...
	for rows.Next() {
		...
		message.Producer = "knowledge-ingestion-service"
...
func (store *OutboxStore) markPublishedBatch(ctx context.Context, tx pgx.Tx, messages []types.OutboxMessage, publishedAt time.Time) error {
	eventIDs := outboxEventIDs(messages)
	tag, err := tx.Exec(ctx, `
...
`, eventIDs, types.OutboxStatusPublished, publishedAt)
	if err != nil {
		return types.NewDBWriteFailed(err.Error())
	}
	if tag.RowsAffected() != int64(len(messages)) {
		return types.NewDBWriteFailed("knowledge outbox published row count mismatch")
	}
	return nil
}
...
SET retry_count = $2,
    ...
    updated_at = now()
WHERE event_id = $1
`, message.EventID, retryCount, nextRetryAt)
	if err != nil {
		return types.NewDBWriteFailed(err.Error())
	}
	return nil
...
SET status = $2,
    ...
    updated_at = now()
WHERE event_id = $1
`, message.EventID, types.OutboxStatusDLQ, retryCount, deadLetteredAt)
	if err != nil {
		return types.NewDBWriteFailed(err.Error())
	}
	return nil
...
func outboxEventIDs(messages []types.OutboxMessage) []string {
