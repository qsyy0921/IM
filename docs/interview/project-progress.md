...
- 各核心服务的 `/healthz`、`/readyz`、`/debug/metrics`，以及 api-gateway 第一阶段 Prometheus text `/metrics`；
...
| 服务 | 待开发 / 待完善功能 |
| --- | --- |
| `api-gateway` | alerting / dashboard / 生产采样治理、后端服务 server span rollout、legacy opt-in 实际迁移观察和移除计划、配置中心 / DB-backed quota hardening、生产部署治理；当前已有第一阶段 Prometheus text `/metrics`，但还不是生产告警平台 |
