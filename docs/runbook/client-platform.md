...
- `npm --prefix clients run check:no-toolchain` runs the no-toolchain client
  ...
  clientweb smoke hook contract, desktop / Android action assets, desktop
  WebView metadata / login dry-run contracts, Android login smoke plan, Android
  login smoke dry-run runner contract and Android platform readiness. It does
  not build native artifacts or APKs, launch Docker, install APKs, start
  activities, open `adb reverse` or install toolchains; it does read
