...
`, cfg.tenantID, memoryEventID, sourceRefID, messageID, sourceEventID, cfg.conversationID, seq, crossGroupSourceRefID, crossGroupConversationID, now, crossGroupMessageID, crossGroupSourceEventID, seq+1); err != nil {
	...
	staleMemories := []struct {
	...
	}{
		{
			...
			status:     "ACTIVE",
			factText:   "Expired phoenix launch summary reminder is outside the requested conversation sequence window.",
			validFrom:  seq,
			validTo:    seq + 1,
			confidence: 0.8800,
		},
		{
			...
			status:     "SUPERSEDED",
			factText:   "Superseded phoenix launch summary reminder should not appear as current evidence.",
			validFrom:  seq,
			validTo:    seq + 10,
			confidence: 0.8700,
		},
