...
第六阶段：9 个服务做必要收口后，以 search-service v0.1 作为向 AI 大模型应用后端转进的第一步；当前 search-service 第一实现切片已推进到 PG repository、真实 SearchMessages 查询、grpc runtime、timeline consumer，并已跑通 projection smoke；memory-service 已开始落 SDD / proto / migration contract，后续继续 memory skeleton、retrieval、RAG、summary、Agent、skill-registry、MCP gateway 和 action-executor。
```
...
search-service v0.1 第一实现切片已推进到 PG repository、真实 SearchMessages 查询、grpc runtime 和 timeline consumer，并已跑通 projection smoke；memory-service 已开始落 SDD / proto / migration contract；
...
search-service v0.1 / group memory / retrieval 是下一阶段基础能力，其中 search-service 第一实现切片已跑通 projection smoke；RAG / summary / Agent / skill-registry / MCP gateway / action-executor 后置到 EvidencePack 和权限过滤稳定之后；
...
3. 基于已通过的 `search-service` v0.1 projection smoke，设计并实现 memory / group memory、EvidencePack 和 retrieval-gateway 最小链路；
...
后续我会先把 9 个服务做必要收口，补齐消息变更、成员窗口、群管理、回执、联系人和策略这些 AI 会依赖的 IM 语义；短期不把生产级完整系统测试作为转进阻塞，而是用切片级本地检查和最小 smoke 守住事实、权限和证据边界。当前 search-service v0.1 已推进到 PG repository、真实 SearchMessages 查询、grpc runtime、timeline consumer，并已跑通 projection smoke；memory-service 已开始落 SDD / proto / migration contract；后续再做 memory skeleton、retrieval-gateway、RAG、summary、Agent、skill-registry、MCP gateway 和 action-executor。大模型只能通过权限过滤后的 EvidencePack 访问聊天记录，Agent 写动作必须走 proposal、approval、executor 和 audit。
```
