...
2. AI 大模型应用底座架构深化：
   需要把 `target-architecture-ai.md` 中的 group memory、StructuredMemoryEvent、Memory Graph、ProfileAggregate、EvidencePack、ActionProposal、Tool Policy、skill registry、MCP gateway 和 action executor 落成可执行 SDD / ADR。`memory-service` 已从 SDD / proto / migration contract 切到 foundation-active implementation slice，并开始落六层 skeleton、domain/types/app validation、PostgreSQL repository first pass、timeline projection usecase 和 runtime/observability wiring；下一步要补真实 PostgreSQL repository integration tests、timeline worker 单测和 focused projection smoke。2025/2026 memory 论文和 benchmark 的设计启发必须继续进入 `memory-service`：source refs、speaker / audience scope、valid_from / valid_to、supersedes / contradicts、confidence、PENDING / ACTIVE / SUPERSEDED / REJECTED 状态、profile 不能从单条群消息直接升级。Agent 可以接入真实业务，但写动作必须走 policy precheck、tool policy、proposal / approval / executor / audit；高风险动作第一阶段禁止自动执行。后续需要 AI eval harness 覆盖 retrieval miss、temporal version、attribution、permission leak、profile overgeneralization、tool policy violation 和 action execution safety。
...
- `memory-service` / group memory projection：当前已从 SDD / proto / migration contract 切到 foundation-active implementation slice，六层 skeleton、repository first pass、projection usecase 和 runtime/observability wiring 已开始落；下一步补真实 PG integration tests、timeline worker 单测和 focused projection smoke。在 search-service 边界稳定后做结构化协作记忆、版本语义、speaker / audience 归因、Memory Graph 和画像聚合；无 source ref 的长期 memory 不能进入 ACTIVE。
...
- `ai-eval-service` / AI eval harness：伴随 search / retrieval / Agent 一起建设。
