...
| 服务 | 当前状态 | 已有证据 | 主要剩余工作 |
| --- | --- | --- | --- |
| `api-gateway` | 已落地、已接主链路 | gateway auth / downstream trusted metadata smoke、token / tenant scope rate limit | OpenTelemetry、动态 tenant plan / quota、legacy descriptor 收敛、生产部署治理 |
...
1. `api-gateway`：first-stage tenant-scoped rate limit 已补；下一步继续动态 tenant plan / quota、legacy descriptor 显式 opt-in 和 OpenTelemetry trace，不先扩新 facade。
...
1. 继续做 `api-gateway` 入口治理：动态 tenant plan / quota、legacy descriptor 显式 opt-in 和 OpenTelemetry trace。
2. 继续把现有 9 个服务做干净。
3. 继续补分布式故障恢复 smoke。
