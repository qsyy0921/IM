...
- 除非用户明确要求，不再把流量诊断、代理用量归因、外网消耗排查列为当前任务；开发时仅保留低流量约束：复用已有镜像/依赖，避免不必要下载，Win/Mac 服务间流量走本地有线 `172.31.50.*`。
...
| 本地双机网络 | Win-Mac 压测网络已改为有线直连：Windows `172.31.50.1/24`，Mac `172.31.50.2/24`，链路 `2.5Gbps`；两端 Wi-Fi 继续负责上网；两端各自的 `127.0.0.1:7890` 只给本机访问外网使用，不作为 Win-Mac 服务间代理。后续双机压测、SSH 和 callback/mock receiver 优先使用 `172.31.50.*`，历史 `192.168.0.*` 地址只作为 Wi-Fi recovery 和旧报告事实 |
...
| contacts-service SDD / implementation | 已新增 `docs/sdd/contacts-service.md` v0.1 Draft，定位为联系人 / 好友关系事实源；第一阶段只做 `SendContactRequest`、`RespondContactRequest(ACCEPT/DECLINE)`、`ListContacts`、`GetContactState`，并通过 `contacts_outbox -> im.contact.events` 发布联系人事件；明确不写 `conversation_members`、不自动创建会话、message-service 不同步依赖 contacts-service。当前已完成 `contacts_service.proto`、`im.contact.events` Kafka schema、`000001_contacts_core.sql`、生成代码、contacts-service 六层骨架、`cmd/contacts-service` noop/grpc/outbox-relay 入口、PostgreSQL repository 真实事务、contacts outbox store、Kafka producer、trigger relay 和 ACCEPT / DECLINE 两条真实进程 smoke；Send 幂等只以 `contact_requests` 唯一键 + `command_hash` 为事实源，Respond 幂等使用 `contact_command_idempotency`，所有关系事件按 canonical user pair 分区；真实 PG 测试覆盖 request/accept/decline/list/get/pagination/idempotent replay/concurrent first send/opposite terminal decision，outbox 测试覆盖 retry/DLQ、mark PUBLISHED 和按 `partition_key + aggregate_version` 阻塞低版本 PENDING/DLQ；clean commit `584017f` 的 ACCEPT smoke 证明 `SendContactRequest -> RespondContactRequest(ACCEPT) -> contacts_outbox -> Kafka im.contact.events -> ListContacts / GetContactState`，outbox `PUBLISHED=2/PENDING=0/DLQ=0`，Kafka 读回 `contact.request.created.v1` 与 `contact.request.accepted.v1`；clean commit `3a392df` 的 DECLINE smoke 证明不创建联系人 edge、双方 `ListContacts` 为空、`GetContactState` 返回 NotFound，Kafka 读回 `contact.request.created.v1` 与 `contact.request.declined.v1`。好友删除 / 拉黑 / 备注名 v0.2 设计已冻结并完成代码切片：三者均为当前 owner 的单向 `contact_edges` 操作，只写 contacts-service 自己的 edge / idempotency / outbox，不写 `conversation_members`、不直接改变 message-service 发送权限；已新增 `DeleteContact` / `BlockContact` / `UpdateContactRemark` proto、`contact.edge.deleted/blocked/remark_updated` schema、`000002_contact_edge_management.sql`、repository 事务、relay builder 和 loadtest runner 场景；真实 PG 集成测试覆盖 remark 返回、单向 delete/block、幂等 replay 和 outbox 事件。下一步跑 `delete` / `block` / `remark` 三条真实进程 smoke 并归档报告。 |
...
- 2026-06-09：独立评审复核 push-gateway slow-client smoke，结论无 P0/P1，不阻塞继续 Redis route / cross-instance route。P2 已记录：`NEXUSIM_PUSH_TEST_WRITE_DELAY` 仍是生产二进制可见的测试 knob，生产必须 unset/0；本轮 slow-client smoke 证明的是 durable `PullInbox` recovery，不证明 resume buffer replay，后续需单独设计 resume replay smoke。
...
- 2026-06-10：已新增 contacts-service SDD v0.1 Draft，作为第三层联系人 / 好友关系事实源。设计刻意和 conversation-service 解耦：好友关系不写 `conversation_members`，接受好友不自动创建会话，后续如需创建 direct conversation 必须通过显式 app port 或 saga；第一阶段只做好友申请、接受 / 拒绝、联系人列表和 contacts outbox。当前已完成 `contacts_service.proto`、`im.contact.events` Kafka schema、`000001_contacts_core.sql`、生成代码、contacts-service 六层骨架、noop/grpc/outbox-relay 入口、PostgreSQL repository 真实事务、contacts outbox store、Kafka producer 和 trigger relay；本轮保持低耦合和复杂度控制，不改 message/conversation/delivery/push 的内部表或同步依赖，也不新增跨服务同步调用。真实 PG 集成测试覆盖申请、接受、拒绝、列表分页、幂等 replay、并发首次申请、终态相反 decision、outbox retry/DLQ、mark PUBLISHED 和 canonical user pair 顺序阻塞。clean commit `584017f` 已跑通 contacts ACCEPT 真实进程 smoke：`SendContactRequest -> RespondContactRequest(ACCEPT) -> contacts_outbox -> Kafka im.contact.events -> ListContacts / GetContactState`，结果 `send_status=PENDING`、`respond_status=ACCEPTED`、双向 `CONTACT_EDGE_STATUS_ACTIVE`、outbox `PUBLISHED=2/PENDING=0/DLQ=0`，Kafka 读回 `contact.request.created.v1` 和 `contact.request.accepted.v1`；clean commit `3a392df` 已跑通 contacts DECLINE 真实进程 smoke：`respond_status=DECLINED`、双方 `ListContacts` 为空、双向 `GetContactState` 返回 NotFound、outbox `PUBLISHED=2/PENDING=0/DLQ=0`，Kafka 读回 `contact.request.created.v1` 和 `contact.request.declined.v1`。报告见 `docs/runbook/loadtest/contacts-service/`。
