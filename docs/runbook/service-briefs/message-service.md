...
- 已补 `outbox-audit`、`outbox-repair`、只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式，可直接审计、redrive 和清理 `message_outbox` repair 历史；`outbox-audit` / `outbox-repair-audit` 可通过 `NEXUSIM_MESSAGE_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_MESSAGE_OUTBOX_REPAIR_AUDIT_OUTPUT` 写低敏 JSON 结果，便于 operator 留存证据。
