...
| `retrieval-gateway` | EvidencePack 统一边界，聚合 search / memory / policy precheck，并通过 memory-service 公开 API 扩展 current memory graph edges 和当前用户 profile aggregate evidence；source-chain-aware rerank first pass 已让多来源 / 跨群 / graph-edge / profile-supporting evidence 提升 `rerank_score`，并已通过 2026-06-24 retrieval source-chain rerank service-stack gate；不直接调用 LLM。 |
