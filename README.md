...
| 服务 / 模块 | 当前状态 |
| --- | --- |
| `search-service` | 搜索 projection、visibility / tombstone、PostgreSQL FTS lexical `SearchMessages`、显式 OpenSearch / BM25 candidate backend、OpenSearch opt-in smoke 入口、service-owned OpenSearch rebuild operator first path、mapping drift fail-closed hardening、timeline consumer、projection smoke；外部索引只召回候选，PG projection 负责最终过滤。 |
