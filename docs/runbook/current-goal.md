...
5. 当前系统可以表述为“本地多进程 + Win/Mac 双机 Docker 最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route、Redis-backed best-effort resume buffer 和 WebSocket notify 串联；已在 clean commit `8c322fc` 跑通 Windows -> Mac Docker WebSocket gateway 的有线直连 smoke。下一步优先跑 cross-instance resume 真实进程 smoke，并继续保持：不要把当前本地分布式 smoke 表述为完整生产多实例方案。
