...
func TestWriteOutboxAuditOutput(t *testing.T) {
	...
	outputPath := filepath.Join(t.TempDir(), "message-outbox-audit.json")
	...
	err := writeOutboxAuditOutput(outputPath, []postgresinfra.OutboxAuditRow{
		{
			...
			Status:           "DLQ",
			RetryCount:       5,
			LastError:        "message outbox publish failed",
	...
	if row.ID != 42 ||
		...
		row.EventType != "message.persisted.v1" ||
		row.Status != "DLQ" ||
		row.RetryCount != 5 ||
		row.LastError != "message outbox publish failed" ||
		row.AvailableAt == "" ||
		row.NextRetryAt == "" ||
		row.PublishedAt == "" ||
		row.DeadLetteredAt == "" ||
