...
- 已有 health / ready / metrics、correlation propagation、rate limiter、RetryInfo、W3C `traceparent` 输入桥接、legacy/facade traffic audit counters 和默认关闭的 first-stage OpenTelemetry gRPC server span。
...
- OTel collector / alerting / 跨服务 rollout、legacy descriptor opt-in 实际迁移观察和移除计划、配置中心 / DB-backed quota hardening。
