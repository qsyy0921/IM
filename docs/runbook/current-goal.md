...
| conversation-service member roster | 已新增最小 `ListConversationMembers` 读接口并跑通 clean commit `99aacc6` 真实进程 smoke：3 条 `JOIN` 后 roster 返回 4 个 ACTIVE 成员；clean commit `14ffedc` 的 `LEAVE` roster smoke 和 clean commit `be2e039` 的 `REMOVE` roster smoke 均通过，目标成员状态变为 `LEFT` 且普通 roster 不再返回该用户；只列当前 ACTIVE 成员，调用者必须是该会话 ACTIVE 成员；分页 token opaque，当前按 `user_id ASC` keyset；该接口用于降低耦合，避免其它服务跨表读取 `conversation_members`，不承担成员历史 / 审计视图 |
