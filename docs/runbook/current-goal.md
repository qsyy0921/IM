...
3. 优先降低微服务耦合、控制代码复杂度：不跨服务读取内部表，不引入网状依赖，不为短期功能增加不必要同步 RPC、公共包或抽象层；实现方案明显变复杂时，先拆小切片或补 SDD/契约再编码。
...
- 2026-06-10：已为 push-gateway 增加第一版低耦合真实鉴权入口：`NEXUSIM_PUSH_AUTH_MODE=hmac` 时 WebSocket 建连必须提供短期 signed gateway token，服务端校验 HMAC 签名、`aud=push-gateway`、`exp` 和 device 绑定；裸 `tenant_id/user_id` query 只保留在 `mock` 模式用于本地 smoke。该切片不新增 identity-service、不跨服务读取用户/设备表，后续仍需 device revoke、session revoke、refresh token、key rotation、多 issuer 和 HMAC smoke runner 支持。
