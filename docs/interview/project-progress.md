...
| `agent-service` | 第一版 proposal-only path、真实本地 adapter smoke、MCP prepare、proposal store 和 approval preflight 已落；当前 proposal 前调用 `mcp-gateway.PrepareToolCall`，通过 skill catalog / policy precheck / prepare audit 后再消费 EvidencePack 生成可引用 proposal，并通过 `VerifyApprovedAgentProposal` 给 executor 做公开校验，不执行真实动作 |
...
| `action-executor` | 第一版 approved execution audit 和 low-sensitive tool result projection 已落；强制 proposal / approval / prepare audit 关联，重新做 skill execute contract check 和 policy execute precheck，只记录低敏审计和结果引用，`executed=false` |
