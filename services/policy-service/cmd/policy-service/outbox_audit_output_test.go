...
func TestWriteOutboxAuditOutput(t *testing.T) {
	...
	outputPath := filepath.Join(t.TempDir(), "policy-outbox-audit.json")
	...
	err := writeOutboxAuditOutput(outputPath, []postgresinfra.OutboxAuditRow{
		{
			...
			AggregateVersion: 7,
			EventType:        "policy.decision.recorded.v1",
			Status:           "DLQ",
			RetryCount:       5,
			LastError:        "policy outbox publish failed",
	...
	if row.ID != 42 ||
		...
		row.AggregateType != "policy_decision" ||
		row.AggregateID != "decision-1" ||
		row.AggregateVersion != 7 ||
		row.EventType != "policy.decision.recorded.v1" ||
		row.Status != "DLQ" ||
		row.RetryCount != 5 ||
		row.LastError != "policy outbox publish failed" ||
		row.AvailableAt == "" ||
		row.NextRetryAt == "" ||
		row.PublishedAt == "" ||
		row.DeadLetteredAt == "" ||
