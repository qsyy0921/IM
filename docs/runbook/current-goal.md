...
持续推进 `E:\development\IM` 的 NexusIM 项目落地。`message-service`、`conversation-service`、`delivery-service` 已分别完成最小真实闭环，`push-gateway` 也已跑通在线通知、慢连接和 Redis route smoke；当前阶段切换到“把已落地链路提升为可说明、可运行、可验证的最小分布式系统”：

```text
local distributed smoke
-> Redis route failure semantics
...
| 本地双机网络 | Win-Mac 压测网络已改为有线直连：Windows `172.31.50.1/24`，Mac `172.31.50.2/24`，链路 `2.5Gbps`；两端 Wi-Fi 继续负责上网；两端对外代理端口统一为 `127.0.0.1:7890`。后续双机压测、SSH 和 callback/mock receiver 优先使用 `172.31.50.*`，历史 `192.168.0.*` 地址只作为 Wi-Fi fallback 和旧报告事实 |
| 本地分布式 runbook | `docs/runbook/distributed-local.md` 已新增，定义当前四个真实微服务 / 网关的本地多进程分布式拓扑、`tools/local-distributed-smoke.ps1` 入口、可讲证据和限制；已补 Win/Mac 双机 Docker 节点模拟计划：Windows `172.31.50.1/24`、Mac `172.31.50.2/24`，后续优先用有线直连承载服务间通信；Windows -> Mac 免密 SSH 已恢复，Mac Docker CLI 可用，Docker Desktop 29.5.2，当前 Docker 资源池为 8 CPU / 8192MiB；`tools/check-mac-docker-desktop.ps1` 可复查 CPU / memory / proxy；`tools/sync-mac-distributed-smoke.ps1` 可通过有线 SSH/scp 传 Git bundle 和 darwin/arm64 二进制到 Mac 专用 checkout |
...
5. 当前系统可以表述为“本地多进程最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立进程协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route 和 WebSocket notify 串联。下一步优先处理 Mac repo 同步策略并跑 Win-Mac 有线直连 smoke；同时补 resume buffer TTL，并单独设计 resume replay smoke。不要把当前本地分布式 smoke 表述为完整生产多实例方案。
...
- 2026-06-09：已为 push-gateway Redis route 增加 per-session TTL 续期：connect 写 route 后由 gateway 进程按 TTL 比例周期性刷新 session route / user set，disconnect 时取消续期并 best-effort 删除 route；miniredis 测试已覆盖 route 在 TTL 前被续期、unregister 后被删除。当前可把能力表述为“最小分布式在线路由模拟”，但 Redis 故障语义、cleanup ticker、跨实例 resume 仍待补。
...
- 2026-06-09：已恢复 Windows -> Mac 免密 SSH：`172.31.50.2:22` 与 `192.168.0.182:22` 均可达，Windows 当前公钥已加入 Mac `authorized_keys`；Mac Docker CLI 已配置到用户级 PATH，`docker --version` 返回 Docker `29.5.2`，Docker Desktop 当前资源池约 8 CPU / 8GB。两端 Git 代理统一为 `http://127.0.0.1:7890`。Mac `/Users/qsyy0921/Desktop/IM` 存在但有本地 ahead / untracked 文件，后续不要强行 reset；优先 `fetch`，不能 fast-forward 时新建干净 smoke checkout。
- 2026-06-09：已将 Windows `main` 推送到 GitHub，`origin/main` 更新到 `a839e61`，用于后续 Mac fetch / 双机分布式 smoke。
- 2026-06-09：为节省外网流量，已改为通过有线 `172.31.50.2` 同步 Mac：新增 `tools/sync-mac-distributed-smoke.ps1`，Windows 生成 Git bundle 并 scp 到 Mac，再从 Windows 交叉编译 `darwin/arm64` 的 `push-gateway` / `pushgateway-smoke` 传入 `/Users/qsyy0921/Desktop/IM-distributed-smoke/bin/darwin-arm64`；Mac 专用 checkout 已验证 `NEXUSIM_PUSH_GATEWAY_MODE=noop` 可启动。
- 2026-06-09：已用 `tools/sync-mac-distributed-smoke.ps1 -SkipBuild` 通过有线重新同步 Mac 专用 checkout 到 Windows HEAD `d514390`，Mac `/Users/qsyy0921/Desktop/IM-distributed-smoke` 显示 `## main`，并再次验证 `bin/darwin-arm64/push-gateway` 的 `noop` 模式可启动。
- 2026-06-09：已新增并执行 `tools/check-mac-docker-desktop.ps1`，通过有线 SSH 复查 Mac Docker Desktop：Docker `29.5.2`，context `desktop-linux`，`Cpus=8`，`MemoryMiB=8192`，`SwapMiB=1024`，Docker proxy `http/https=http://127.0.0.1:7890`，proxy exclude 包含 `172.16.0.0/12`，结果 `mac_docker_desktop_config=OK`。后续 Mac 两节点模拟使用容器级 `--cpus 4 --memory 4g`，不再调大 Docker Desktop 全局池。
