...
- 已补 `projection-checkpoint-repair` / audit / cleanup，当前只允许带审计地回调 checkpoint 做 replay，不允许前跳跳过事件；`projection-checkpoint-repair-audit` / cleanup 可选 `NEXUSIM_DELIVERY_PROJECTION_REPAIR_AUDIT_OUTPUT` / `NEXUSIM_DELIVERY_PROJECTION_REPAIR_CLEANUP_OUTPUT` 写低敏 JSON 结果或 cleanup summary，便于 CI / operator 留存结构化证据。
