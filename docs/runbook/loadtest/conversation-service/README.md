...
- `ListConversationMembers` 最小 roster smoke 已通过：3 条 `JOIN` 后，真实 gRPC 读取到 `member_list_count=4`，成员为 seed owner + 3 个 active target；该接口只返回当前 ACTIVE 成员，不承担成员历史 / 审计视图。
