...
- Invalid MFA recovery-code attempts are counted on the user row, independent from password and TOTP factor counters. By default they inherit `NEXUSIM_IDENTITY_MFA_MAX_FAILED_ATTEMPTS`, `NEXUSIM_IDENTITY_MFA_FAILURE_WINDOW` and `NEXUSIM_IDENTITY_MFA_LOCK_DURATION`, but deployments can override recovery-code risk separately with `NEXUSIM_IDENTITY_MFA_RECOVERY_MAX_FAILED_ATTEMPTS`, `NEXUSIM_IDENTITY_MFA_RECOVERY_FAILURE_WINDOW` and `NEXUSIM_IDENTITY_MFA_RECOVERY_LOCK_DURATION`; a locked recovery-code path returns stable `mfa temporarily locked`.
