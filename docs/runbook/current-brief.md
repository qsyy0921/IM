...
11. conversation-service 已补第三层群管理最小读能力：`ListConversationMembers` 只返回当前 ACTIVE 成员，调用者必须是 ACTIVE 成员；clean commit `99aacc6` 的 `JOIN` roster smoke、clean commit `14ffedc` 的 `LEAVE` roster smoke、clean commit `be2e039` 的 `REMOVE` roster smoke 和 clean commit `7150944` 的 `ROLE_CHANGED` roster smoke 均通过，证明 JOIN 后出现、LEAVE/REMOVE 后消失、ROLE_CHANGED 后当前 role 更新。它是低耦合 roster API，不让其它服务跨表读取 `conversation_members`，也不把成员历史 / 审计视图塞进普通列表。owner transfer 已完成专用 RPC、Kafka schema、migration、relay/projection、conversation-service 写路径和 clean commit `490db1a` 的真实进程 smoke：旧 owner 降级为 ACTIVE ADMIN，新 owner 成为唯一 ACTIVE OWNER，saga DONE，outbox `PUBLISHED=1/PENDING=0`。下一步可转向联系人 / 好友关系、群管理 hardening，或继续补真实鉴权的 device/session 管理；仍要保持低耦合和小切片。
