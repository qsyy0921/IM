...
func TestWriteOutboxRepairAuditOutput(t *testing.T) {
	...
	outputPath := filepath.Join(t.TempDir(), "receipt-outbox-repair-audit.json")
	...
	err := writeOutboxRepairAuditOutput(outputPath, []postgresinfra.OutboxRepairAuditRow{
		{
			...
			PreviousLastError:      "receipt outbox publish failed",
	...
	if row.EventID != "event-1" ||
		...
		row.PreviousLastError != "receipt outbox publish failed" ||
