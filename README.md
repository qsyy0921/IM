...
| IM 核心层 | 10 个本地运行链路服务：9 个已运行 IM 服务分别拥有身份、策略、联系人、会话、消息、投递、回执等事实和读模型；`timeline-service` 作为 noop runtime 进入 Docker / 观测链路，承接后续热点会话 sequencer / 分区 / gap marker，但不伪装成当前已完成写路径。 |
...
| 搜索 | search-service projection；PostgreSQL FTS lexical first path；显式 OpenSearch / BM25 candidate backend first path；OpenSearch opt-in smoke 入口、preflight-opensearch gate、service-owned rebuild operator first path 和 mapping drift hardening | OpenSearch 真实进程 smoke 归档、Elasticsearch、Meilisearch、provider-grade BM25 运维 | 只有 search-service 写搜索索引；外部索引只召回候选，最终 visibility / tombstone 仍由 PostgreSQL projection 过滤；OpenSearch opt-in smoke 先用显式 request-timeout 校验 endpoint / index，再启动 service / Kafka 链路。 |
| 向量索引 | vector-index-service；PostgreSQL metadata、pgvector optional adapter、embedding queue；pgvector / OpenSearch vector / Milvus provider preflight gates、provider readiness matrix、显式 provider request-timeout、search-rag provider runtime readiness bootstrap、retrieval `provider_coverage` summary contract | Qdrant、Weaviate、真实 pgvector / Milvus / OpenSearch vector provider smoke | 向量是可重建 projection，不是消息事实源；runtime readiness / preflight / coverage contract 只证明 runtime / mapping / reporting contract，不代表真实 provider smoke 已完成。 |
...
| 工作流 / 审批 | workflow-service 内部状态机、compensation instruction registry、external callback delivery persistent worker + redrive operator first path、compensation instruction approval page、compensation execution readiness / invocation / result / audit append / audit append result manifest | Temporal、Cadence、Argo Workflows | 审批和补偿审计归 workflow / audit 所有；callback worker / redrive / compensation instruction approval / invocation / result / audit append / audit append result 都只处理低敏 refs，不记录 decision、不执行 target。 |
...
| `timeline-service` | 当前为 noop runtime 和观测节点；后续承接热点群 sequencer、seq block、gap marker 和 timeline 分区。 |
...
| 服务 / 模块 | 当前状态 |
| --- | --- |
| `search-service` | 搜索 projection、visibility / tombstone、PostgreSQL FTS lexical `SearchMessages`、显式 OpenSearch / BM25 candidate backend、OpenSearch opt-in smoke 入口、preflight-opensearch gate、service-owned OpenSearch rebuild operator first path、mapping drift fail-closed hardening、timeline consumer、projection smoke；外部索引只召回候选，PG projection 负责最终过滤。 |
...
| `retrieval-gateway` | EvidencePack 统一边界，聚合 search / memory / vector / policy precheck，并通过 memory-service 公开 API 扩展 current memory graph edges、可配置 depth 0..3 的相邻 memory 和当前用户 profile aggregate evidence；当前策略版本 `retrieval-gateway.v1.hybrid-source-vector-rrf-graph-depth<N>` 会按 lexical search、vector item、memory event、profile aggregate、source chain、memory graph、actor attribution 和 profile support lane 做 RRF 风格融合，再叠加 source-chain 信号，`<N>` 记录本次运行实际 graph expansion depth；vector source 只消费 vector-index-service 公开 `SearchVectors` 返回的低敏引用 / hash / visibility metadata，不传 raw text 或 embedding vector；已通过 opt-in vector backend live smoke，search-service 已具备 PostgreSQL FTS lexical first path、显式 OpenSearch / BM25 candidate backend first path、service-owned rebuild operator first path 和 mapping drift hardening；retrieval positive smoke / adapter 已门禁低敏 `source_coverage`，并可读取 pgvector / OpenSearch vector / Milvus provider readiness summary 输出 `provider_coverage`，避免 RAG / Agent 把缺失 source lane 或未就绪 provider 误当成可用事实；下一步做真实 OpenSearch 进程 smoke、pgvector / Milvus / OpenSearch provider smoke；不直接调用 LLM。 |
| `rag-service` | 只读问答 first path、EvidencePack citation verifier、guarded external HTTP LLM boundary，并保留 EvidencePack memory graph edges 和 profile aggregate evidence；grounded-answer anchor gate 只把有文本且可追踪的 evidence 交给 provider，ref-only evidence 只保留作审计；`loadtest/ragagent` 会把 RAG grounded answer 与 Agent approval / action audit 汇总成低敏演示报告。 |
| `summary-service` | 只读摘要 first path、EvidencePack citation verifier、guarded external HTTP LLM boundary，并保留 EvidencePack memory graph edges 和 profile aggregate evidence；grounded-summary anchor gate 只把有文本且可追踪的 evidence 交给 provider，ref-only evidence 只保留作审计。 |
| `agent-service` | proposal-only path、mcp-gateway prepare、approval workflow、approval outbox relay、planner Python candidate guard，并保留 EvidencePack memory graph edges 和 profile aggregate evidence；`loadtest/ragagent` 复用 Agent proposal / approval / action-executor audit 校验，并已验证 EvidencePack-backed business proposal 经 approval 后通过 action-executor 执行 conversation note / profile mutation。 |
...
| `action-executor` | approved execution audit、proposal / approval / prepare audit 校验、本地安全 adapter、guarded external HTTP provider adapter、conversation note / profile opt-in business adapters、provider failure lifecycle、provider failure metrics、batch redrive operator handoff、provider replay operator UI first path、provider replay admin / workflow handoff、handoff review page、execution readiness page、redrive invocation manifest、受控 redrive execution operator path、redrive result manifest、redrive audit append manifest handoff、external audit append operator、audit append result manifest、`RedriveProviderFailure` 受控 redrive API first path 和 eval smoke；operator UI / handoff / readiness / invocation manifest / result manifest / audit append manifest / audit append result manifest 只输出低敏 batch / candidate / workflow / admin / fresh proof / command / result / audit metadata，不执行 tool、不修改 failure row、不复用旧 approval、不携带 raw resource id / input / reason / attributes JSON / provider artifacts；result-to-audit handoff 只生成 external audit append manifest，不调用 audit-service；audit append result manifest 只绑定 audit-service 返回的 audit id / record hash / idempotency key，不调用 audit-service；`loadtest/actionexecutor` 默认 preflight，显式执行时才通过 action-executor 公开 gRPC 调用 `RedriveProviderFailure`，或通过 audit-service 公开 gRPC 调用 `AppendAuditRecord`。 |
| `ai-eval-service` | 低敏 eval catalog / recorder / gate；case catalog 覆盖 profile-Agent safety、RAG / Summary grounding safety、LLM boundary safety、action-executor preflight safety、business proposal source-chain audit boundary、approved note / profile mutation、provider failure low-sensitive metrics、batch redrive handoff、provider replay operator UI first path、provider replay admin / workflow handoff、handoff review page、execution readiness page、redrive invocation manifest、external audit append operator、provider replay submit operator bridge、workflow provider replay queue view、workflow external approval binding、workflow external callback wait、workflow operator queue visibility、workflow compensation review 和 execution readiness boundary；profile-agent safety 本地 adapter 已扩到 24 cases，新增 asker-bound term ambiguity、visible-chain incomplete abstention、missing visibility projection fail-closed 和 audience-language profile negative checks，并显式禁止 unsupported memory hidden path / raw prompt persistence；memory-service / retrieval-gateway / RAG / Summary / Agent live adapters 已完成第一轮 service-stack gate，覆盖 collaborative memory、profile aggregation、public candidate review / temporal update、profile repair approval、EvidencePack、Agent output 和 action safety；retrieval positive adapter 已通过 source-chain rerank live gate，确认 multi-source memory evidence rerank 优先级；retrieval opt-in vector backend smoke 已证明 refs-only `VECTOR_ITEM` 能经真实 vector-index-service gRPC 进入 EvidencePack；`rag-agent-demo` 已通过 optional service-stack live gate，确认 RAG grounded answer、Agent approval、profile repair workflow approval、group-memory answer / proposal、business proposal source-chain、action-executor audit 和 opt-in `conversation.note.create` / `conversation.profile.update` 真实业务 mutation。 |
...
| `admin-service` | 管理操作创建 / 审批 / 查询、operation worker、outbox relay、control-plane / audit adapter、workflow compensation request handoff、provider replay handoff submit / list / approve operator bridge，以及 provider replay readiness / invocation manifest 所需 approved operation 低敏证据。 |
...
| `workflow-service` | workflow creation / decision、approval 状态机、`timer-worker` 显式 approval timeout -> `TIMED_OUT`、`ListWorkflows` 低敏队列视图、provider replay queue operator view、operator queues、external decision manifest binding、external callback wait、external callback delivery plan、delivery status / redrive plan、external callback delivery persistent worker first path、external callback delivery redrive operator path、compensation request / instruction registry / rollback adapter first path、`ListWorkflowCompensations` 低敏执行结果查询、compensation review page、compensation instruction approval page、execution readiness / invocation / result manifest、compensation execution audit append manifest handoff、audit append result manifest、workflow outbox relay first path。 |
...
`check-local` 会把当前运行 / 失败的子门禁记录到 `.git\nexusim-check-local-state.json`。
下一次默认从该子门禁继续，避免失败后从头重跑；需要强制完整重跑时使用
`.\tools\check-local.ps1 -NoResume`，需要清理旧状态时使用
`.\tools\check-local.ps1 -ResetResume`。
...
```text
NexusIM 已完成 9 个 IM 后端服务的主链路，并把 `timeline-service` 作为第 10 个 noop 运行节点纳入本地链路和观测；一批本地 / 双机分布式 smoke 已落地，
