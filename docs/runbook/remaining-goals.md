...
2. AI 大模型应用底座架构深化：
   需要把 `target-architecture-ai.md` 中的 group memory、StructuredMemoryEvent、Memory Graph、ProfileAggregate、EvidencePack、ActionProposal、Tool Policy、skill registry、MCP gateway 和 action executor 落成可执行 SDD / ADR。`memory-service` 已从 SDD / proto / migration contract 切到 foundation-active implementation 并跑通 clean projection smoke；`retrieval-gateway` 已完成 first-stage EvidencePack 字段 hardening（rerank score、source coverage、dedupe reason）；`rag-service` 已完成 first-stage read-only answer path，后续需要真实 `retrieval -> rag` smoke、LLM provider 边界设计和 citation verifier。AI eval harness first pass 已落低敏 case schema / validator，当前覆盖 retrieval miss、temporal version、attribution、permission leak 和 tool policy violation；后续需要执行 adapter 覆盖 profile overgeneralization、action execution safety 和真实 EvidencePack / RAG 输出。2025/2026 memory 论文和 benchmark 的设计启发必须继续进入 `memory-service`：source refs、speaker / audience scope、valid_from / valid_to、supersedes / contradicts、confidence、PENDING / ACTIVE / SUPERSEDED / REJECTED 状态、profile 不能从单条群消息直接升级。Agent 可以接入真实业务，但写动作必须走 policy precheck、tool policy、proposal / approval / executor / audit；高风险动作第一阶段禁止自动执行。
...
- `rag-service`：first-stage read-only answer path 已落；后续补真实 retrieval->rag smoke、RAG eval execution adapter、LLM provider 边界和 citation verifier。
