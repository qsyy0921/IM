...
if (-not $RunName) {
    $RunName = "message-delete-smoke-" + (Get-Date -Format "yyyyMMdd-HHmmss")
}
...
if (-not $SkipBuild) {
    ...
    go build -o bin\messagedelete-smoke.exe ./loadtest/messagedelete
}
...
WHERE status IN ('PENDING', 'DLQ')
  AND (
    ...
    OR tenant_id = 'tenant-message-delete-smoke'
  );
"@ | Out-Null
}
...
try {
    ...
    $runner = Join-Path $repo "bin\messagedelete-smoke.exe"
    ...
    & $runner `
        ...
        --tenant-id "tenant-message-delete-smoke" `
        --conversation-id "conv-message-delete-smoke" `
        --owner-user-id "owner-1" `
        --receiver-user-id "delete-user-1" `
