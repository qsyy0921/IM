...
- `/debug/metrics` exposes only aggregate challenge delivery counters and identity risk counters. For password reset hashed-target limiting it reports only total limiter rows and currently locked rows; it must not expose raw challenge tokens, HMAC target keys, user IDs, destinations, template data or provider error bodies.
