...
1. 当前已完成：identity-service 已补只读 `session-mfa-proof-audit` 和 `challenge-delivery-repair-audit` 运维模式，分别用于发现历史 session MFA proof 脏数据和直接审计 challenge delivery repair 历史。
