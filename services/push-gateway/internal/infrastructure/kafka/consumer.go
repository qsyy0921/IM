...
func (consumer *ReaderConsumer) Fetch(ctx context.Context) (types.DeliveryEventMessage, error) {
	if consumer == nil || consumer.reader == nil {
		return types.DeliveryEventMessage{}, errors.New("kafka reader consumer is not configured")
	}
	message, err := consumer.reader.FetchMessage(ctx)
	if err != nil {
		return types.DeliveryEventMessage{}, err
	}
	return types.DeliveryEventMessage{
		Topic:     message.Topic,
...
func (consumer *ReaderConsumer) Commit(ctx context.Context, message types.DeliveryEventMessage) error {
	if consumer == nil || consumer.reader == nil {
