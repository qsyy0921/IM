...
- 已补只读 `outbox-audit`，以及 `outbox-repair` / 只读 `outbox-repair-audit` / `outbox-repair-cleanup` 运维模式，可直接审计、redrive 和清理 `receipt_outbox` repair 历史；`outbox-repair-audit` 支持按 event / tenant / `repaired_at` RFC3339 时间窗口缩小范围并输出 compacted filters；`outbox-audit` / `outbox-repair` / `outbox-repair-audit` / `outbox-repair-cleanup` 可通过 `NEXUSIM_RECEIPT_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_AUDIT_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_CLEANUP_OUTPUT` 写低敏 JSON 结果或 cleanup summary，便于 operator 留存证据。
