...
5. push-gateway slow-client 真实进程负向 smoke 已通过；该 smoke 证明 durable `PullInbox` fallback，不证明 resume buffer replay。Redis route 最小 adapter 和真实跨实例 route smoke 已完成，下一步优先补 route TTL 续期 / cleanup、Redis 故障语义、resume buffer TTL，并单独设计 resume replay smoke。不要把当前 `all` 模式或 Redis route adapter 表述为完整多实例生产方案。
