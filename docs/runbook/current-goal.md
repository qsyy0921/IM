...
| contacts-service SDD / baseline | 已新增 `docs/sdd/contacts-service.md` v0.1 Draft，定位为联系人 / 好友关系事实源；第一阶段只做 `SendContactRequest`、`RespondContactRequest(ACCEPT/DECLINE)`、`ListContacts`、`GetContactState`，并通过 `contacts_outbox -> im.contact.events` 发布联系人事件；明确不写 `conversation_members`、不自动创建会话、message-service 不同步依赖 contacts-service。当前已完成 `contacts_service.proto`、`im.contact.events` Kafka schema、`000001_contacts_core.sql`、生成代码、contacts-service 六层最小可编译骨架和 `cmd/contacts-service` noop/grpc 入口；repository 仍是显式未实现 stub，下一步进入 PostgreSQL repository/usecase 真实事务、contacts outbox relay 和最小 `SendContactRequest -> RespondContactRequest(ACCEPT) -> ListContacts` smoke。 |
...
- 2026-06-10：已新增 contacts-service SDD v0.1 Draft，作为第三层联系人 / 好友关系事实源。设计刻意和 conversation-service 解耦：好友关系不写 `conversation_members`，接受好友不自动创建会话，后续如需创建 direct conversation 必须通过显式 app port 或 saga；第一阶段只做好友申请、接受 / 拒绝、联系人列表和 contacts outbox。当前已完成 `contacts_service.proto`、`im.contact.events` Kafka schema、`000001_contacts_core.sql`、生成代码、contacts-service 六层最小可编译骨架和 noop/grpc 入口；本轮保持低耦合和复杂度控制，不改 message/conversation/delivery/push 的内部表或同步依赖。下一步做 PostgreSQL repository/usecase 真实事务、contacts outbox relay 和最小 smoke。
