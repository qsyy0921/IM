...
const (
	...
	eventMessageDeleted   = "message.deleted.v1"
	originalMessageText   = "message delete smoke original"
...
type summary struct {
	...
	DeleteMessage           deleteMessage    `json:"delete_message"`
	PullDeleted             pullResult       `json:"pull_deleted"`
	...
	MessageLogDeletedAt     string           `json:"message_log_deleted_at,omitempty"`
...
func parseConfig() config {
	...
	flag.StringVar(&cfg.resultDir, "result-dir", filepath.Join(`H:\NexusIM\loadtest-results`, "message-delete-smoke-"+now), "result directory")
	...
	flag.StringVar(&cfg.tenantID, "tenant-id", "tenant-message-delete-smoke", "tenant id")
	flag.StringVar(&cfg.conversationID, "conversation-id", "conv-message-delete-smoke", "conversation id")
	flag.StringVar(&cfg.ownerUserID, "owner-user-id", "owner-1", "owner user id")
	flag.StringVar(&cfg.receiverUserID, "receiver-user-id", "delete-user-1", "receiver user id")
	flag.StringVar(&cfg.receiverDeviceID, "receiver-device-id", "delete-device-1", "receiver device id")
	flag.StringVar(&cfg.deliveryGroup, "delivery-consumer-group", "", "optional delivery timeline consumer group for checkpoint stats")
	flag.BoolVar(&cfg.verifiedAuthMetadata, "verified-auth-metadata", envBool(false, "NEXUSIM_MESSAGE_DELETE_VERIFIED_AUTH_METADATA", "NEXUSIM_MESSAGE_MUTATION_VERIFIED_AUTH_METADATA"), "send gateway verified identity through user-facing gRPC metadata")
...
func run(cfg config) error {
	...
	delete, err := deleteMessageOnce(ctx, cfg, messageClient, send.GetMessageId())
	if err != nil {
		runErr = fmt.Errorf("delete message: %w", err)
		return runErr
	}
	result.DeleteMessage = deleteMessage{
		MessageID:        delete.GetMessageId(),
		ConversationSeq:  delete.GetConversationSeq(),
		ChangeVersion:    delete.GetChangeVersion(),
		IdempotentReplay: delete.GetIdempotentReplay(),
	}
	deletedPull, err := pullInboxUntil(ctx, cfg, deliveryClient, send.GetConversationSeq(), delete.GetConversationSeq(), eventMessageDeleted, send.GetMessageId())
	if err != nil {
		runErr = fmt.Errorf("pull deleted inbox: %w", err)
		return runErr
	}
	result.PullDeleted = deletedPull

	ack, err := ackDeliveryOnce(ctx, cfg, deliveryClient, delete.GetConversationSeq())
	if err != nil {
		runErr = fmt.Errorf("ack delete seq: %w", err)
		return runErr
	}
...
func createReceiverJoin(ctx context.Context, cfg config, client conversationv1.ConversationServiceClient) (*conversationv1.CreateMemberChangeResponse, error) {
	...
	auth := verifiedAuthIdentity{
		...
		deviceID:  "message-delete-owner-device",
		sessionID: "message-delete-owner-session",
		traceID:   "message-delete-join",
		requestID: "message-delete-join",
	}
...
func sendMessageOnce(ctx context.Context, cfg config, client messagev1.MessageServiceClient) (*messagev1.SendMessageResponse, error) {
	...
	auth := verifiedAuthIdentity{
		...
		deviceID:  "message-delete-owner-device",
		sessionID: "message-delete-owner-session",
		traceID:   "message-delete-send",
		requestID: "message-delete-send",
	}
	requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
	return client.SendMessage(requestCtx, &messagev1.SendMessageRequest{
		...
		ConversationId: cfg.conversationID,
		ClientMsgId:    "message-delete-client-message-1",
		MessageType:    "TEXT",
		Payload:        payload,
	})
}

func deleteMessageOnce(ctx context.Context, cfg config, client messagev1.MessageServiceClient, messageID string) (*messagev1.MessageChangeResponse, error) {
	requestCtx, cancel := context.WithTimeout(ctx, cfg.requestTimeout)
	defer cancel()
	auth := verifiedAuthIdentity{
		tenantID:  cfg.tenantID,
		...
		deviceID:  "message-delete-owner-device",
		sessionID: "message-delete-owner-session",
		traceID:   "message-delete-delete",
		requestID: "message-delete-delete",
	}
	requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
	return client.DeleteMessage(requestCtx, &messagev1.DeleteMessageRequest{
		...
		IdempotencyKey: "message-delete-delete-1",
		Reason:         "message delete smoke",
	})
}
...
func pullInboxUntil(
	...
	for {
		...
		auth := verifiedAuthIdentity{
			...
			sessionID: "message-delete-pull",
			traceID:   "message-delete-pull",
			requestID: "message-delete-pull",
		}
		requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
...
func ackDeliveryOnce(ctx context.Context, cfg config, client deliveryv1.DeliveryServiceClient, seq int64) (*deliveryv1.AckDeliveryResponse, error) {
	...
	auth := verifiedAuthIdentity{
		...
		userID:    cfg.receiverUserID,
		deviceID:  cfg.receiverDeviceID,
		sessionID: "message-delete-ack",
		traceID:   "message-delete-ack",
		requestID: "message-delete-ack",
	}
	requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
...
SELECT
    ...
    mch.after_payload_json IS NULL,
    mch.before_status,
    mch.after_status,
    mch.change_version
FROM message_log ml
...
`, cfg.tenantID, cfg.conversationID, messageID).Scan(
		...
		return fmt.Errorf("query deleted message facts: %w", err)
	}
	result.MessageLogStatus = status
	result.MessageLogPayload = payload
	if deletedAt != nil {
		result.MessageLogDeletedAt = deletedAt.UTC().Format(time.RFC3339Nano)
	}
	...
	if status != "DELETED" {
		return fmt.Errorf("message status = %s, want DELETED", status)
	}
	if deletedAt == nil {
		return errors.New("message deleted_at is null")
	}
	if changeType != "DELETE" || beforeStatus != "NORMAL" || afterStatus != "DELETED" {
		return fmt.Errorf("unexpected change history: type=%s before_status=%s after_status=%s", changeType, beforeStatus, afterStatus)
	}
	if !jsonTextPayloadEquals(payload, originalMessageText) {
		return fmt.Errorf("message payload = %s, want original text=%q", payload, originalMessageText)
	}
	...
	if !afterPayloadIsNull {
		return errors.New("after payload is not null for delete")
	}
...
func finish(cfg config, result *summary, runErr error) error {
	...
	path := filepath.Join(cfg.resultDir, "message-delete-summary.json")
