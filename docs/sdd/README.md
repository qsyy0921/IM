...
| `delivery-service` | SDD v0.1 已存在，最小 projection / PullInbox / AckDelivery / delivery outbox relay、第一阶段 gRPC TLS / mTLS server config、`loadtest/delivery` 和 `loadtest/deliveryvisibility` client TLS config 已落地 | 可以支撑 push-gateway 第一阶段，只要 push-gateway 不绕过 durable inbox / ACK；server / loadtest client TLS 和 push-gateway delivery client TLS 均默认 plaintext、可静态配置，证书治理和全服务 mTLS rollout 仍是后续项 |
