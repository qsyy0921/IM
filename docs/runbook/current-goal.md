...
- RAG / Agent 正式业务能力。
- 热点会话 sequencer 生产逻辑。
- `DeleteMessage` 业务实现；`RevokeMessage` 和 `EditMessage` 已完成最小真实进程 smoke，并已收口第一阶段可见性：撤回 tombstone / 编辑 changed item 只投给已收到原消息的用户，乱序变更 fail-closed；并发变更、管理员权限真实 policy、push-gateway 在线通知消息变更和搜索/回执修正仍未覆盖。
