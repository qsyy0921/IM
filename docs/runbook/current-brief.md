...
4. 当前先治理已有 9 个微服务；identity repository、repository test、`loadtest/pushgateway/main.go` 和长目标架构文档已完成拆分，identity-service 已补低敏 identity / MFA / challenge-delivery 聚合观测，`outbox-relay` 和 `challenge-delivery-worker` 对非取消运行时错误已改为退避重试，并分别在 relay / worker 模式暴露低敏 retry 快照；malformed payload / unsupported event 仍保持 fail-closed，decrypt failure / incomplete message / notifier error 仍保持 store 驱动的 retry / expire / DLQ 语义。delivery-service 已补 `/debug/metrics`、只读 `outbox-audit`、`outbox-repair`、只读 `outbox-repair-audit`、按 retention/scope 清理 outbox repair audit 历史、`projection-checkpoint-repair`、只读 `projection-checkpoint-repair-audit`、按 retention/class/scope 清理 repair audit 历史、projection failure audit、resolved 标记、按 unresolved failure 定点 replay、按最早 unresolved failure 自动 rewind、只读 `projection-failure-audit` 及其按 offset/event/class 的定点过滤、以及按 class/scope 过滤的 resolved failure cleanup operator，`timeline-consumer` 与 `outbox-relay` 现已对运行时 `Fetch` / `Commit` 或 relay runtime 错误做退避重试，并分别在 worker / relay 模式暴露 retry 快照；malformed event、projection failure、failure recorder 异常和 malformed payload / unsupported event 仍保持持久审计或 outbox fail-closed 语义；policy-service 已补低敏 gRPC / decision / rule-store / projection / audit-outbox 聚合观测，`contact-consumer`、`timeline-consumer` 和 `outbox-relay` 对非取消运行时错误已改为退避重试，并分别在 worker / relay 模式暴露 retry 快照；conversation-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 gRPC/PG/member-change 聚合观测，`member-change-worker` 对非取消错误已改为退避重试，并在 worker 模式暴露 retry 快照；contacts-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 contact request / edge / outbox 聚合观测，`outbox-relay` 对非取消运行时错误已改为退避重试，并在 relay 模式暴露 outbox relay retry 快照；push-gateway 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 session / resume / Redis route / auth JWK 聚合观测，并区分 Redis publish error 与 0-subscriber stale-route 场景，`delivery-consumer`、`identity-consumer` 和 Redis route subscriber 仅对运行时错误做退避重试；Redis route 续约连续失败达到阈值后会主动踢掉本地 session，避免 route TTL 失效后仍长期假装在线；`invalid frame` / `unsupported event` 和 malformed subscriber payload 仍保持 fail-closed 或仅记聚合计数，并在 worker / subscriber 模式暴露 retry 快照；receipt-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 receipt projection / outbox 聚合观测，`delivery-consumer` 与 `outbox-relay` 对非取消运行时错误已改为退避重试，并分别在 worker / relay 模式暴露 retry 快照；message-service 已补 `/healthz`、`/readyz`、`/debug/metrics` 和低敏 send / repository / outbox relay 聚合观测，`outbox-relay` 对非取消运行时错误已改为退避重试，并在 relay 模式暴露 retry 快照；api-gateway 已补 trusted metadata 启动门禁：当下游服务启用 `metadata` / `verified-metadata` auth 时，公网地址 + 无 mTLS client cert 的危险组合会在启动前直接失败；私网 / loopback 仍保留第一阶段 trusted metadata 直连；malformed event / payload 仍保持 fail-closed，后续继续处理各服务 P2 hardening。
