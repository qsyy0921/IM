...
func cleanupTenant(ctx context.Context, pool *pgxpool.Pool, tenantID string) error {
	statements := []string{
		...
		`DELETE FROM timeline_seq_gap_markers WHERE tenant_id = $1`,
		`DELETE FROM timeline_seq_block_leases WHERE tenant_id = $1`,
		`DELETE FROM timeline_sequence_state WHERE tenant_id = $1`,
