...
3. Repair / DLQ / audit 产品化：
   多数服务已有本地 operator / audit / cleanup，且已有统一 operator 索引、机器可读 `repair-operators.catalog.json` 和只生成低敏 JSON 的 `write-repair-operator-plan.ps1` 计划入口；后续要补真正跨服务执行编排、批量 repair、审批边界、外部审计 sink 和运维 UI，不把手写 SQL 当作长期方案。
