...
- 已补 `outbox-audit` / `outbox-repair` / `outbox-repair-audit` / `outbox-repair-cleanup`，可审计、redrive `delivery_outbox` DLQ 并清理 repair audit 历史；`outbox-audit` / `outbox-repair-audit` 可通过 `NEXUSIM_DELIVERY_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_DELIVERY_OUTBOX_REPAIR_AUDIT_OUTPUT` 写低敏 JSON 结果，便于 operator 留存证据。
