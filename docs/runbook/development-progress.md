...
- future service boundary 门禁仍保护未授权服务目录；`search-service v0.1` 已作为 AI 底座第一服务落地基础目录和 timeline consumer，后续 `memory-service` / `retrieval-gateway` / RAG / `summary-service` / Agent / `skill-registry` / `mcp-gateway` / `action-executor` 不能绕过 search / memory / retrieval / policy 直接落 demo
...
| `search-service` | 已开始第一实现切片 | `search_service.proto`、PostgreSQL core migration、六层 skeleton、`SearchMessages` app / domain / gRPC adapter、projection usecase skeleton、PostgreSQL repository、真实 PG visibility / tombstone 集成测试、`grpc` runtime mode 和 timeline decoder / consumer 已落地；下一步是 projection smoke；定位为 search projection / visibility / tombstone / EvidencePack 前置，不做 LLM demo | `service-briefs/search-service.md` |
...
search-service v0.1 第一实现切片已继续推进到 PG repository / SearchMessages / grpc runtime / timeline consumer，后续是 projection smoke、memory-service / retrieval-gateway / RAG / summary-service / Agent / skill-registry / mcp-gateway / action-executor。
