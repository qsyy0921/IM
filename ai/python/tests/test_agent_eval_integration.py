...
STATE_DIFF_DEEPER_HARDENING_PATH = (
    REPO_ROOT
    / "ai"
    / "python"
    / "fixtures"
...
class AgentEvalIntegrationTests(unittest.TestCase):
    ...
    def test_cli_outputs_pass_report_for_state_diff_deeper_hardening_scenarios(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "ai/python/scripts/run_agent_eval_fixture.py",
                str(STATE_DIFF_DEEPER_HARDENING_PATH),
            ],
            check=True,
            capture_output=True,
            cwd=REPO_ROOT,
