...
func TestPolicyIntegrationAuthHelpersUseExpectedIdentities(t *testing.T) {
	cfg := config{
		tenantID:        "tenant-1",
		userID:          "sender-1",
		deviceID:        "device-1",
		sessionID:       "session-1",
		changeUserID:    "editor-1",
		changeDeviceID:  "device-2",
		changeSessionID: "session-2",
	}
	if got := messageAuth(sendAuth(cfg, "send-1")); got.GetUserId() != "sender-1" || got.GetDeviceId() != "device-1" || got.GetRequestId() != "send-1" {
		t.Fatalf("unexpected send auth: %+v", got)
	}
	if got := messageAuth(changeAuth(cfg, "edit-1")); got.GetUserId() != "editor-1" || got.GetDeviceId() != "device-2" || got.GetRequestId() != "edit-1" {
		t.Fatalf("unexpected change auth: %+v", got)
	}
}
