...
- Windows desktop 已有 artifact / signing / installer plan first paths；签名 / installer
  ...
  中的公开 signer subject policy，但不会使用证书源签名或修改 artifact；signing executor
  的低敏 execution policy 会声明 profile 读取和 `--require-valid` 下的 signer subject enforcement；release readiness report 也会对已收集的
  ...
  的低敏 execution policy 也会声明 signing profile 读取、signer subject policy 检查、artifact
