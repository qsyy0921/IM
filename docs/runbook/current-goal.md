...
- RAG / Agent 正式业务能力。
- 热点会话 sequencer 生产逻辑。
- 用户私有删除 / 合规物理删除；`RevokeMessage`、`EditMessage` 和第一阶段 `DeleteMessage(CONVERSATION_VIEW)` 已完成最小真实进程 smoke，并已收口第一阶段可见性：撤回 tombstone / 编辑 changed item / 删除 tombstone 只投给已收到原消息的用户，乱序变更 fail-closed；并发变更、管理员权限真实 policy、push-gateway 在线通知消息变更和搜索/回执修正仍未覆盖。
...
| push-gateway message-change notify runner | `loadtest/pushgateway` 已新增 `message-change-notify` 场景和 `--message-change-action edit|revoke|delete`，标准脚本 `loadtest/pushgateway/run-local-smoke.ps1` 已支持该场景；runner 会验证 WebSocket `delivery.notify.source_event_type` 与 durable `PullInbox` 的 `event_type + message_id + conversation_seq` 一致。当前只是 runner 能力，三类真实 smoke 报告尚未归档。 |
