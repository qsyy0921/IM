...
第一版会话展示标题、空态、常见错误中文文案和显式本地启动脚本。
...
群成员列表 / 移除 / 角色 / owner transfer 的真实多用户 smoke、成员搜索 / 分页、群标题 /
头像 read model 和更完整群设置。Android 真机 WebView login smoke 和正式移动端发布链路后置到用户明确切回。
...
- 完整 Web / App / 桌面客户端；当前 Web / Windows PC shell 已有账号登录、注册、好友、
  ...
  Windows installer、Android 真机 smoke、正式移动端发布链路以及成员搜索 / 分页、
