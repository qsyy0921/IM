...
5. 当前系统可以表述为“本地多进程 + Win/Mac 双机 Docker 最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route 和 WebSocket notify 串联；已跑通 Windows -> Mac Docker WebSocket gateway 的有线直连 smoke。下一步优先把 Win-Mac smoke 在 clean commit 上重跑归档，再继续 resume buffer TTL / resume replay smoke。不要把当前本地分布式 smoke 表述为完整生产多实例方案。
