...
- 分布式 smoke 证据低敏 manifest：集中索引 Redis / PostgreSQL / Kafka 本地 summary 路径，并支持 schema-only / H 盘真实文件复核和 Markdown report
- 安全启动门禁 catalog：集中索引 DDD / cross-service table / debug listener / public listener auth / TLS / gateway / repair operator safety gates，并校验已接入 `check-local`
