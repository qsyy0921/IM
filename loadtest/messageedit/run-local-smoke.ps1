...
$timelineTopic = "conversation.timeline.edit." + (Get-Date -Format "yyyyMMdd-HHmmss")
$deliveryTopic = "im.delivery.events"
$deliveryConsumerGroup = "nexusim-delivery-edit-smoke-" + (Get-Date -Format "yyyyMMddHHmmss")
...
if (-not $SkipBuild) {
    ...
    go build -o bin\messageedit-smoke.exe ./loadtest/messageedit
}
...
WHERE status IN ('PENDING', 'DLQ')
  AND (
    ...
    OR tenant_id = 'tenant-message-edit-smoke'
  );
"@ | Out-Null
}
...
try {
    ...
    $runner = Join-Path $repo "bin\messageedit-smoke.exe"
    ...
    & $runner `
        ...
        --tenant-id "tenant-message-edit-smoke" `
        --conversation-id "conv-message-edit-smoke" `
        --owner-user-id "owner-1" `
        --receiver-user-id "edit-user-1" `
        --receiver-device-id "edit-device-1" `
        --delivery-consumer-group $deliveryConsumerGroup `
        --wait-timeout "25s" `
        --request-timeout "5s" `
        --verified-auth-metadata=$VerifiedAuthMetadata
    if ($LASTEXITCODE -ne 0) {
        throw "messageedit smoke runner failed with exit code $LASTEXITCODE"
    }
} finally {
