...
| `identity-service` | SDD v0.1 Draft、proto / migration / 六层骨架、gateway token 签发、refresh rotation、邮箱/手机验证、密码重置 challenge、TOTP MFA 因子生命周期、Login 强制校验、factor 级 MFA 失败锁定、`MFA_UNAVAILABLE` 稳定错误和 `/debug/metrics` MFA 风险计数已落地 | 支撑 push-gateway 本地 token 验签；Refresh step-up、MFA recovery / WebAuthn、OIDC federation 和 KMS/HSM 仍是后续项 |
