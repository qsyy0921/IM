...
| `conversation-service / member_change_saga` | SDD 已冻结 v1.0；proto / schema / migration v2 / relay builder / 最小 `CreateMemberChange` 写路径、saga publish 状态推进、full smoke 和当前 ACTIVE 成员 `ListConversationMembers` 读接口已落地；`LEAVE / REMOVE` 后 roster 过滤 smoke 已覆盖 | 后续补 ROLE_CHANGED、DLQ repair、admin-only 成员历史查询和生产韧性 |
...
- 已实现当前 ACTIVE 成员 `ListConversationMembers` 读接口，并已验证 `JOIN` 后返回 active roster、`LEAVE / REMOVE` 后不再返回目标成员；它只暴露会话当前 roster，不承担成员历史 / 审计查询。
