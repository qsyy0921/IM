...
- `memory-service` / group memory projection：当前已落 SDD / proto / migration contract；下一步补六层 skeleton、repository、projection usecase 和 focused tests。在 search-service 边界稳定后做结构化协作记忆、版本语义、speaker / audience 归因、Memory Graph 和画像聚合；无 source ref 的长期 memory 不能进入 ACTIVE。
