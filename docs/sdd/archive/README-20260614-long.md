...
| 服务 / 设计 | 状态 | 对编码的影响 |
| --- | --- | --- |
| `message-service` | SDD 已冻结 v1.0；Send/Edit/Revoke/Delete、adaptive admission、policy/conversation RPC adapter、outbox relay、gateway verified metadata auth mode、第一阶段 gRPC TLS / mTLS server config、`loadtest/sendmessage` client config 和消息变更 smoke runner client config 已落地 | 继续保持消息事实源边界；auth mode 默认 body 兼容历史 smoke，metadata 模式只消费 gateway verified gRPC metadata，不等同完整 API gateway；server / loadtest client TLS 默认 plaintext，证书签发 / 轮换 / 分发、动态服务身份和全服务 mTLS rollout 仍是后续项 |
| `timeline-service / sequencer` | SDD 未完成 | 不阻塞 `LOCAL_ROW_LOCK`；阻塞热点会话生产实现 |
| `conversation-service / send context` | SDD v0.1 已存在；`GetSendContext` 服务间 read path、第一阶段 gRPC TLS / mTLS server config 和 message-service client TLS config 已落地 | 继续作为 message-service 的会话上下文查询端口，不读取其它服务内部表 |
| `conversation-service / member_change_saga` | SDD 已冻结 v1.0；proto / schema / migration v2 / relay builder / 最小 `CreateMemberChange` 写路径、saga publish 状态推进、full smoke、当前 ACTIVE 成员 `ListConversationMembers` 读接口、gateway verified metadata auth mode、`loadtest/memberchange` client TLS config 和 verified metadata auth smoke 开关已落地；`LEAVE / REMOVE` 后 roster 过滤和 `ROLE_CHANGED` 后 role 更新 smoke 已覆盖 | 后续补 DLQ repair、admin-only 成员历史查询、更完整权限负例和生产韧性；auth mode 默认 body 兼容历史 smoke，metadata 模式只消费 gateway verified gRPC metadata，`GetSendContext` 仍保持服务间 request 契约 |
| `push-gateway` | SDD v0.1 Draft、WebSocket frame、`im.delivery.events` consumer、ACK 转发、HMAC auth、Redis route / resume、slow session close、多实例 smoke、入站 WebSocket TLS / mTLS 静态配置、`loadtest/pushgateway` conversation / message / delivery / identity / WebSocket client TLS config 和 verified metadata auth smoke 开关已落地 | 只做在线唤醒和回源协调；push-gateway 进程自身 delivery RPC client TLS、WebSocket listener TLS 和 smoke runner client TLS 均默认 plaintext、可静态配置；ACK 转发会把 WebSocket auth 派生出的身份写入 delivery-service metadata，后续补证书治理、Redis route hardening 和生产指标 |
| `identity-service` | SDD v0.1 Draft、proto / migration / 六层骨架、gateway token 签发、refresh rotation、邮箱/手机验证、密码重置 challenge、TOTP MFA 因子生命周期、Login 强制校验、factor 级 MFA 失败锁定、MFA recovery codes 生成 / 再生成 / 吊销、`MFA_UNAVAILABLE` 稳定错误、`/debug/metrics` MFA 风险计数和第一阶段 gRPC TLS / mTLS server + smoke client config 已落地 | 支撑 push-gateway 本地 token 验签；Refresh step-up、WebAuthn、OIDC federation、backup factor policy、KMS/HSM 和全服务 mTLS rollout 仍是后续项 |
| `policy-service` | SDD v0.1 Draft、proto / 六层骨架、静态 message action policy、message-service 可选 RPC adapter、第一阶段 gRPC TLS / mTLS server config、direct policy smoke client config、policy/message integration smoke message-service client config 和 verified metadata auth smoke 开关已落地 | 已抽出 message-service policy 边界；contacts / conversation / tenant / risk projection 和真实策略引擎仍是后续项；auth mode 仍在 message-service public entrypoint 消费 gateway verified metadata，direct policy smokes 是服务级契约检查；全服务 mTLS rollout 仍是后续项 |
...
| `api-gateway` | SDD v0.1 Draft、第一版 gRPC proxy skeleton、`nexusim.gateway.v1.GatewayService` public facade proto、gateway token 验证、trusted metadata 注入、`AuthContext` 重写、入口静态 TLS / mTLS、下游静态 TLS / mTLS client 配置、低敏 gRPC access log、`/debug/metrics`、默认 `api-gateway` audience、第一阶段 local / Redis rate limiter、first-stage correlation propagation 已落地；facade 已覆盖 conversation / message / delivery / receipt / contacts user-facing RPC；clean commit `bb13300` 已跑通 secure demo 经 `GatewayService` facade 的真实进程 smoke；clean commit `c1328ca` 已跑通 legacy descriptor disabled 的 facade-only smoke；clean commit `f3b290f` 已跑通 contacts 经 `GatewayService` facade 的真实进程 smoke；clean commit `ccae224` 已跑通 contacts facade correlation smoke | 只做 user-facing 入口鉴权与转发，不拥有业务事实；facade 不包含 `GetSendContext`，legacy service descriptor 默认保留用于兼容但可通过 `NEXUSIM_API_GATEWAY_REGISTER_LEGACY_DESCRIPTORS=false` 关闭；contacts 事实仍归 contacts-service，message-service 不同步依赖 contacts-service；启用后端 verified-metadata auth 时必须配合 loopback / 内网隔离或 mTLS peer allowlist；push-gateway token audience 与 api-gateway token audience 默认分离；debug metrics 是进程内聚合，不包含 token / user / body；correlation 只覆盖 `trace_id/request_id` 生成、metadata/header/log 传播，不是完整 OpenTelemetry；Redis limiter 是第一版跨实例 fixed-window 入口保护，不是完整 tenant quota / WAF / 风控系统；仍不包含证书生命周期、OpenTelemetry collector / alerting 或生产级 API gateway 部署 |
