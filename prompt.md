...
它不保存 Codex UI 目标正文、临时粘贴内容、历史副本或一次性提示词。
...
2. 模块级切片闭环：优先按一个完整可感知功能模块推进，避免只改字段 / 文档碎片；
   每个模块先做架构分析，再把代码、必要测试和文档一起收；默认跑相关局部门禁，
   不频繁跑完整 `check-local`。完整门禁失败后优先从记录的失败子门禁继续，
   不要无脑从头重跑。
   Codex 目标应指向完整功能模块，严禁把单个小切片、小字段或单句文档作为目标。
   当前已有未提交改动时，先把它收口、验证、提交，再进入下一个功能模块。
...
5. 不引入隐藏业务兜底；编码时不要用假数据、默认成功、本地缓存、静默降级或旧接口冒充真实结果。
   依赖、权限、事实源、投影或 provider 不确定时必须 fail-closed、显式 retry / repair / redrive，
   或返回稳定错误；开发相关模块时持续删除旧隐藏兜底 / fallback 代码。
...
11. 文档只在阶段变化、公开能力变化、架构边界变化或新增服务 / 中间件 / provider 时同步；
    不因单个内部字段或实现细节扩大同步所有文档。
12. 汇报只说做了什么、验证了什么、还剩什么；新发现待办写入
    `docs/runbook/remaining-goals.md`；不回滚用户已有修改。
