param(
    ...
    [ValidateSet("full", "message-change-notify", "resume-replay", "redis-resume-negative", "cross-instance-resume", "slow-client", "redis-fault", "redis-cluster-node-stop", "redis-cluster-failover", "redis-sentinel-failover", "redis-sentinel-master-stop", "redis-sentinel-quorum-loss", "redis-sentinel-network-partition", "identity-revoke")]
...
. (Join-Path $PSScriptRoot "run-local-smoke.helpers.ps1")
. (Join-Path $PSScriptRoot "run-local-smoke.redis-faults.ps1")

$redisFaultSetup = Resolve-PushGatewayRedisFaultSetup `
    -Scenario $Scenario `
    ...
    -ResultDir $resultDir
$RedisFaultCommand = $redisFaultSetup.RedisFaultCommand
$RedisRestoreCommand = $redisFaultSetup.RedisRestoreCommand
$runnerRequestTimeout = $redisFaultSetup.RunnerRequestTimeout
