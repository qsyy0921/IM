...
当前优先级：
1. 先治理已有 9 个微服务，新增服务后置；api-gateway 已补 first-stage tenant-scoped rate limit，下一步继续动态 tenant plan / quota、legacy descriptor 显式 opt-in 和 OpenTelemetry trace。`search-service` 只保留 SDD draft，不进入 proto / migration / skeleton。
