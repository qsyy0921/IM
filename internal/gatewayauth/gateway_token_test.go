...
func TestAuthenticatorJWTRejectsHS256GatewayToken(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), ErrPermissionDenied.Error()) {
...
func TestAuthenticatorHMACRejectsExpiredToken(t *testing.T) {
	...
	_, err = authenticator.Authenticate(httptest.NewRequest("GET", "/ws?token="+token, nil))
	if err == nil || !strings.Contains(err.Error(), ErrAuthExpired.Error()) {
...
func (checker *fakeRevocationChecker) IsRevoked(ctx context.Context, auth AuthContext) (bool, error) {
	checker.auth = auth
	if checker.err != nil {
