...
1. 当前已完成：identity-service 已补只读 `session-mfa-proof-audit`、只读 `challenge-delivery-repair-audit` 和 `challenge-delivery-repair-cleanup` operator；contacts-service 和 policy-service 已分别补只读 `outbox-repair-audit` 运维模式，用于直接审计各自 outbox repair 历史。
