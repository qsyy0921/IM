...
const (
	...
	eventMessageRevoked   = "message.revoked.v1"
...
type summary struct {
	...
	RevokeMessage           revokeMessage    `json:"revoke_message"`
	PullRevoked             pullResult       `json:"pull_revoked"`
...
func parseConfig() config {
	...
	flag.StringVar(&cfg.tenantID, "tenant-id", "tenant-message-revoke-smoke", "tenant id")
	flag.StringVar(&cfg.conversationID, "conversation-id", "conv-message-revoke-smoke", "conversation id")
	flag.StringVar(&cfg.ownerUserID, "owner-user-id", "owner-1", "owner user id")
	flag.StringVar(&cfg.receiverUserID, "receiver-user-id", "revoke-user-1", "receiver user id")
	flag.StringVar(&cfg.receiverDeviceID, "receiver-device-id", "revoke-device-1", "receiver device id")
	flag.StringVar(&cfg.deliveryGroup, "delivery-consumer-group", "", "optional delivery timeline consumer group for checkpoint stats")
	flag.BoolVar(&cfg.verifiedAuthMetadata, "verified-auth-metadata", envBool(false, "NEXUSIM_MESSAGE_REVOKE_VERIFIED_AUTH_METADATA", "NEXUSIM_MESSAGE_MUTATION_VERIFIED_AUTH_METADATA"), "send gateway verified identity through user-facing gRPC metadata")
...
func run(cfg config) error {
	...
	revoke, err := revokeMessageOnce(ctx, cfg, messageClient, send.GetMessageId())
	if err != nil {
		runErr = fmt.Errorf("revoke message: %w", err)
		return runErr
	}
	result.RevokeMessage = revokeMessage{
		MessageID:        revoke.GetMessageId(),
		ConversationSeq:  revoke.GetConversationSeq(),
		ChangeVersion:    revoke.GetChangeVersion(),
		IdempotentReplay: revoke.GetIdempotentReplay(),
	}
	revokedPull, err := pullInboxUntil(ctx, cfg, deliveryClient, send.GetConversationSeq(), revoke.GetConversationSeq(), eventMessageRevoked, send.GetMessageId())
	if err != nil {
		runErr = fmt.Errorf("pull revoked inbox: %w", err)
		return runErr
	}
	result.PullRevoked = revokedPull

	ack, err := ackDeliveryOnce(ctx, cfg, deliveryClient, revoke.GetConversationSeq())
	if err != nil {
		runErr = fmt.Errorf("ack revoke seq: %w", err)
		return runErr
	}
...
func createReceiverJoin(ctx context.Context, cfg config, client conversationv1.ConversationServiceClient) (*conversationv1.CreateMemberChangeResponse, error) {
	...
	auth := verifiedAuthIdentity{
		...
		deviceID:  "message-revoke-owner-device",
		sessionID: "message-revoke-owner-session",
		traceID:   "message-revoke-join",
		requestID: "message-revoke-join",
	}
...
func sendMessageOnce(ctx context.Context, cfg config, client messagev1.MessageServiceClient) (*messagev1.SendMessageResponse, error) {
	payload, err := structpb.NewStruct(map[string]any{"text": "message revoke smoke"})
	if err != nil {
		return nil, err
	}
	requestCtx, cancel := context.WithTimeout(ctx, cfg.requestTimeout)
	...
	auth := verifiedAuthIdentity{
		...
		deviceID:  "message-revoke-owner-device",
		sessionID: "message-revoke-owner-session",
		traceID:   "message-revoke-send",
		requestID: "message-revoke-send",
	}
	requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
	return client.SendMessage(requestCtx, &messagev1.SendMessageRequest{
		...
		ConversationId: cfg.conversationID,
		ClientMsgId:    "message-revoke-client-message-1",
		MessageType:    "TEXT",
		Payload:        payload,
	})
}

func revokeMessageOnce(ctx context.Context, cfg config, client messagev1.MessageServiceClient, messageID string) (*messagev1.MessageChangeResponse, error) {
	requestCtx, cancel := context.WithTimeout(ctx, cfg.requestTimeout)
	defer cancel()
	auth := verifiedAuthIdentity{
		tenantID:  cfg.tenantID,
		...
		deviceID:  "message-revoke-owner-device",
		sessionID: "message-revoke-owner-session",
		traceID:   "message-revoke-revoke",
		requestID: "message-revoke-revoke",
	}
	requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
	return client.RevokeMessage(requestCtx, &messagev1.RevokeMessageRequest{
		...
		ConversationId: cfg.conversationID,
		MessageId:      messageID,
		IdempotencyKey: "message-revoke-revoke-1",
		Reason:         "message revoke smoke",
	})
}
...
func pullInboxUntil(
	...
	for {
		...
		auth := verifiedAuthIdentity{
			...
			sessionID: "message-revoke-pull",
			traceID:   "message-revoke-pull",
			requestID: "message-revoke-pull",
		}
		requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
...
func ackDeliveryOnce(ctx context.Context, cfg config, client deliveryv1.DeliveryServiceClient, seq int64) (*deliveryv1.AckDeliveryResponse, error) {
	...
	auth := verifiedAuthIdentity{
		...
		userID:    cfg.receiverUserID,
		deviceID:  cfg.receiverDeviceID,
		sessionID: "message-revoke-ack",
		traceID:   "message-revoke-ack",
		requestID: "message-revoke-ack",
	}
	requestCtx = withVerifiedAuthMetadata(requestCtx, cfg, auth)
...
`, cfg.tenantID, cfg.conversationID, messageID).Scan(&status); err != nil {
		return fmt.Errorf("query message status: %w", err)
	}
...
func finish(cfg config, result *summary, runErr error) error {
	...
	path := filepath.Join(cfg.resultDir, "message-revoke-summary.json")
