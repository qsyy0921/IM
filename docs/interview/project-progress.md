...
| `identity-service` | 注册、登录、Refresh Token、MFA TOTP、recovery codes、JWKS、opt-in OIDC discovery、session/device revoke、verification/password reset challenge、webhook / SMTP email challenge sender，first-stage OTel gRPC server span | 身份认证、MFA、token 轮换、JWKS、公私钥边界、issuer discovery 边界、通知投递可靠性，身份服务已进入 trace rollout |
...
| `identity-service` | WebAuthn / passkeys、外部 OIDC federation / OAuth client flows、多 issuer 治理、KMS / HSM key management、完整登录风控、SMS provider、bounce handling、多租户通知模板 |
