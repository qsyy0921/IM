...
| `search-service` | 已跑通第一轮 projection smoke，并补 PostgreSQL FTS / OpenSearch candidate backend first paths | `search_service.proto`、PostgreSQL core migration、六层 skeleton、`SearchMessages` app / domain / gRPC adapter、projection usecase skeleton、PostgreSQL repository、真实 PG visibility / tombstone 集成测试、PostgreSQL FTS token search、显式 OpenSearch / BM25 candidate backend、PG candidate hydration visibility / tombstone filter、`grpc` runtime mode、timeline decoder / consumer 和 clean projection smoke 已落地；定位为 search projection / visibility / tombstone / EvidencePack 前置，不做 LLM demo；OpenSearch 进程 smoke、mapping / rebuild operator 和 provider-grade 运维仍后置 | `service-briefs/search-service.md` |
