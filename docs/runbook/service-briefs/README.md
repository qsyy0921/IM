...
- `search-service` / `memory-service` / `retrieval-gateway` / `rag-service` / `summary-service` / `agent-service` / `skill-registry` 是第一组 foundation-active AI 服务；后续顺序是 MCP gateway / action-executor。
