...
func (store *OutboxStore) ProcessReadyBatch(
	...
	if store == nil || store.pool == nil {
		return types.OutboxRelayStats{}, errors.New("admin outbox store is not configured")
	}
	if publish == nil {
		return types.OutboxRelayStats{}, errors.New("admin outbox publish callback is not configured")
	}
	if limit <= 0 {
		limit = 500
	}
	...
	if len(publishErrors) != len(messages) {
		return types.OutboxRelayStats{}, errors.New("admin outbox batch publish result count mismatch")
	}
	...
	for index, message := range messages {
		if err := publishErrors[index]; err != nil {
			lastError := sanitizeAdminOutboxPublishError(err)
...
SELECT
    ...
    retry_count,
    created_at
FROM admin_outbox current
WHERE status = 'PENDING'
  AND published_at IS NULL
  AND COALESCE(next_retry_at, available_at) <= now()
  AND NOT EXISTS (
      SELECT 1
      FROM admin_outbox previous
...
`, limit)
	...
	for rows.Next() {
		...
		message.Producer = "admin-service"
...
WHERE admin_outbox.tenant_id = keys.tenant_id
  AND admin_outbox.event_id = keys.event_id
`, tenantIDs, eventIDs, types.OutboxStatusPublished, publishedAt)
	if err != nil {
		return types.NewDBWriteFailed(err.Error())
	}
	if tag.RowsAffected() != int64(len(messages)) {
		return types.NewDBWriteFailed("admin outbox published row count mismatch")
	}
	return nil
}
...
func sanitizeAdminOutboxPublishError(err error) string {
	if err == nil {
		return ""
	}
	if errors.Is(err, context.Canceled) {
		return "admin outbox publish canceled"
	}
	if errors.Is(err, context.DeadlineExceeded) {
		return "admin outbox publish timeout"
	}
	return sanitizeAdminOutboxErrorText(err.Error())
}

func sanitizeAdminOutboxErrorText(value string) string {
	message := strings.ToLower(strings.TrimSpace(value))
	switch {
	case message == "":
		return "admin outbox publish failed"
	case strings.Contains(message, "cancel"):
		return "admin outbox publish canceled"
	case strings.Contains(message, "timeout") || strings.Contains(message, "deadline exceeded"):
		return "admin outbox publish timeout"
	case strings.Contains(message, "unsupported"):
		return "admin outbox publish unsupported event"
	case strings.Contains(message, "malformed") ||
		...
		return "admin outbox publish invalid payload"
	case strings.Contains(message, "kafka") ||
		...
		return "admin outbox publish broker unavailable"
	default:
		return "admin outbox publish failed"
	}
}
