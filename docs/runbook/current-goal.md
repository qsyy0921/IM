...
当前优先级：
1. 先治理已有 9 个微服务，新增服务后置；api-gateway 已补 first-stage tenant-scoped rate limit、静态 tenant plan override 和 legacy descriptor 显式 opt-in 默认，下一步继续 OpenTelemetry trace、运行时动态 quota 配置和 legacy opt-in 使用面迁移审计。`search-service` 只保留 SDD draft，不进入 proto / migration / skeleton。
