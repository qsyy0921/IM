...
2. 跑 outbox batch size 100/500/1000 与 relay workers 8/12/16 的联合矩阵，继续观察 pending、`outbox_process_ready_latency_ms`、`kafka_publish_records_per_call` 和 success p99。
3. 设计更细的 adaptive limit，把 outbox pending、relay process ready latency 和 Kafka batch record 数纳入输入，而不是只看瞬时 acquired conns。
