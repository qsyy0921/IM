...
| `receipt-service` | SDD v0.1 Draft、proto、Kafka schema、migration、六层骨架草案已新增 | 下一步实现 PostgreSQL repository / delivery event consumer / MarkRead 事务；不得直接读取 delivery-service 内部表 |
...
1. `receipt-service` PostgreSQL repository / delivery consumer / MarkRead 事务
