...
5. push-gateway slow-client 真实进程负向 smoke 已通过；该 smoke 证明 durable `PullInbox` fallback，不证明 resume buffer replay。Redis route 最小 adapter 已落地，下一步优先做真实跨实例 route smoke、route TTL 续期 / cleanup、resume buffer TTL，并单独设计 resume replay smoke。不要把当前 `all` 模式或 Redis route adapter 表述为完整多实例生产方案。
