...
- `admin-service` product-active：SDD v0.1 和 stage-switch review 已通过，
  ...
  `ADMIN_OPERATION`；`loadtest/admin` operator CLI 已支持通过公开 gRPC approve /
  reject / get / list；非 `CRITICAL` 的 `CONFIG_PUBLISH` 已可通过
  `NEXUSIM_CONTROL_PLANE_GRPC_ADDR` 调用 control-plane public gRPC；不宣称其它
  真实下游 mutation、admin UI 或 provider-grade 运维。
