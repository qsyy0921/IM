...
```text
push-gateway full smoke review
-> multi-device notify smoke
-> Redis route / resume buffer / slow session close
```
...
4. `delivery_outbox -> im.delivery.events -> push-gateway all mode -> online WebSocket client delivery.notify -> PullInbox -> delivery.ack -> AckDelivery -> delivery.ack.ok` 已通过 clean commit smoke；下一步邀请独立评审复核。
5. push-gateway 后续优先补同 user 多 device notify smoke、Redis route、resume buffer 和慢连接主动关闭；不要把当前 in-memory `all` 模式表述为多实例生产方案。
