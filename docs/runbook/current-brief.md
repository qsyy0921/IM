...
10. push-gateway 已新增第一版低耦合真实鉴权入口：`NEXUSIM_PUSH_AUTH_MODE=hmac` 时校验短期 signed gateway token 的 HMAC 签名、`aud=push-gateway`、过期时间和 device 绑定；`mock` auth 只保留给本地 smoke。它不是完整 identity-service，后续仍需 device revoke、session revoke、key rotation、refresh token 和多 issuer。
