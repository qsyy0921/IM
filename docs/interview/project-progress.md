...
后续按服务级 Python candidate integration 继续推进；
...
| Python AI Worker | ADR-036 已固定边界，`ai/python` 目录、`IM` conda toolchain、candidate contract helpers、低敏 safety guard、contract validator、candidate-only worker CLI、output-safety eval adapter、第一条 worker smoke 和 Go-side adapter smoke 已落；后续只作为 LLM / embedding / rerank / memory extraction / planner / eval 候选层接入，最终校验、权限、审批、审计和持久化仍由 Go 服务完成 |
