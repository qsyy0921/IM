...
| `policy-service` | SDD v0.1 Draft、proto / 六层骨架、静态 message action policy、message-service 可选 RPC adapter、第一阶段 gRPC TLS / mTLS server config 和 direct policy smoke client config 已落地 | 已抽出 message-service policy 边界；contacts / conversation / tenant / risk projection 和真实策略引擎仍是后续项；全服务 mTLS rollout 仍是后续项 |
