...
search-service v0.1 第一实现切片已推进到 PG repository、真实 SearchMessages 查询、grpc runtime 和 timeline consumer，并已跑通 projection smoke；memory-service 已跑通 source-backed projection smoke；retrieval-gateway / EvidencePack 第一轮真实 smoke 已通过；
...
search-service v0.1 / group memory / retrieval 是下一阶段基础能力，其中 search-service 第一实现切片、memory-service source-backed projection smoke 和 retrieval-gateway EvidencePack smoke 均已跑通；RAG / summary / Agent / skill-registry / MCP gateway / action-executor 后置到 EvidencePack 字段 / policy hardening 之后；
...
3. 基于已通过的 `search-service` v0.1 projection smoke、`memory-service` source-backed projection smoke 和 `retrieval-gateway` EvidencePack smoke，继续补 EvidencePack 字段 / policy hardening；
