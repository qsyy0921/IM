...
- 已有 health / ready / metrics、correlation propagation、rate limiter、RetryInfo、W3C `traceparent` 输入桥接、legacy/facade traffic audit counters 和默认关闭的 first-stage OpenTelemetry 入口 server span / 下游 gRPC client span。
...
- `/debug/metrics` 已暴露低敏 gRPC、facade / legacy descriptor / other request counters、auth JWK、rate-limit key scope / tenant plan count / tenant plan reload count / reload error count / identity error count、runtime config 和 OTel trace config 快照，其中包含 legacy descriptor 是否仍注册。
...
- OTel collector / alerting / 后端服务 server span rollout、legacy descriptor opt-in 实际迁移观察和移除计划、配置中心 / DB-backed quota hardening。
