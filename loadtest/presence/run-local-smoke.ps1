...
if (-not $RunName) {
    $RunName = "presence-grpc-smoke-" + (Get-Date -Format "yyyyMMdd-HHmmss")
}
...
if (-not $SkipBuild) {
    go build -o (Join-Path $repoRoot "bin\presence-service.exe") ./services/presence-service/cmd/presence-service
    go build -o (Join-Path $repoRoot "bin\presence-smoke.exe") ./loadtest/presence
}
...
if (-not $PresenceGrpcAddr) {
    $PresenceGrpcAddr = "127.0.0.1:" + (Get-FreeTcpPort)
}
$port = [int]($PresenceGrpcAddr.Split(":")[-1])

$processes = @()
try {
    $processes += Start-NexusProcess -Name "presence-service" -FilePath (Join-Path $repoRoot "bin\presence-service.exe") -Port $port -Env @{
        NEXUSIM_PRESENCE_SERVICE_MODE = "grpc"
        NEXUSIM_PRESENCE_GRPC_ADDR = $PresenceGrpcAddr
        NEXUSIM_PRESENCE_DEBUG_ADDR = ""
        NEXUSIM_PG_DSN = $PgDsn
    }

    & (Join-Path $repoRoot "bin\presence-smoke.exe") `
        --pg-dsn $PgDsn `
        --target $PresenceGrpcAddr `
        --result-root $ResultRoot `
        --run-name $RunName
    if ($LASTEXITCODE -ne 0) {
        throw "presence smoke failed with exit code $LASTEXITCODE"
    }
    Write-Host "run_name=$RunName"
    Write-Host "presence_grpc_addr=$PresenceGrpcAddr"
