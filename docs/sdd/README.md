...
| `api-gateway` | SDD v0.1 Draft、第一版 gRPC proxy skeleton、`nexusim.gateway.v1.GatewayService` public facade proto、gateway token 验证、trusted metadata 注入、`AuthContext` 重写、入口静态 TLS / mTLS、下游静态 TLS / mTLS client 配置、低敏 gRPC access log、`/debug/metrics`、默认 `api-gateway` audience、第一阶段 local / Redis rate limiter 已落地；clean commit `bb13300` 已跑通 secure demo 经 `GatewayService` facade 的真实进程 smoke；clean commit `c1328ca` 已跑通 legacy descriptor disabled 的 facade-only smoke | 只做 user-facing 入口鉴权与转发，不拥有业务事实；facade 不包含 `GetSendContext`，legacy service descriptor 默认保留用于兼容但可通过 `NEXUSIM_API_GATEWAY_REGISTER_LEGACY_DESCRIPTORS=false` 关闭；启用后端 verified-metadata auth 时必须配合 loopback / 内网隔离或 mTLS peer allowlist；push-gateway token audience 与 api-gateway token audience 默认分离；debug metrics 是进程内聚合，不包含 token / user / body；Redis limiter 是第一版跨实例 fixed-window 入口保护，不是完整 tenant quota / WAF / 风控系统；仍不包含证书生命周期、统一 trace 或生产级 API gateway 部署 |
