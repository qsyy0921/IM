...
4. 会话列表 / 未读数最小 `ListConversations` 已在 receipt-service 内落地并跑通真实进程 smoke：`unread_count` 从投递后的 `1` 在 `MarkRead` 后变为 `0`；当前已补 `source_event_type` 口径，edit/revoke/delete 可推进会话 `last_visible_seq`，但不作为新未读消息计数。该能力复用 `im.delivery.events`、`receipt_inbox_projection` 和 `user_read_cursors`，没有新增独立服务，也不读取其它服务内部表。
...
8. push-gateway 在线 `delivery.notify` 保持轻量通知边界，同时需要透传 `source_event_type`，让客户端能区分新增 / 编辑 / 撤回 / 删除唤醒；展示事实仍以 `PullInbox` 为准。
