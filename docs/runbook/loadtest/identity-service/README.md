...
| `loadtest-report-20260614-identity-api-gateway-facade-smoke.md` | `RegisterUser -> RequestVerificationChallenge(outbox) -> ConfirmVerificationChallenge -> Login -> RefreshGatewayToken -> RequestPasswordReset -> ConfirmPasswordReset -> post-reset Login -> MFA lifecycle` 经 api-gateway `GatewayService` facade 的真实进程 smoke |
