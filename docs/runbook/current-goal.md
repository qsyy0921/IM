...
- api-gateway 补充：第一版 gRPC 入口 skeleton 已落地，复用共享 `internal/gatewayauth` 验证 gateway token，代理 conversation / message / delivery / receipt user-facing RPC，重写 body `AuthContext` 并注入 trusted metadata；下游 client 已支持静态 TLS / mTLS。后端启用 verified-metadata auth 时必须配合 loopback / 内网隔离或 mTLS peer allowlist，下一步跑 demo 经 api-gateway 的 secure smoke。
