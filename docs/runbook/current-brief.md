...
12. contacts-service 已完成好友申请、申请列表、接受、拒绝、删除、拉黑、解除拉黑、备注名和删除后重新申请恢复的最小闭环。clean commit `eecdccb` 跑通 `ListContactRequests` smoke：`SendContactRequest -> ListContactRequests(INCOMING,PENDING)=1 -> RespondContactRequest(ACCEPT) -> ListContactRequests(INCOMING,PENDING)=0 -> ListContactRequests(INCOMING,ACCEPTED)=1 -> ListContacts`，outbox `PUBLISHED=2/PENDING=0/DLQ=0`；报告见 `docs/runbook/loadtest/contacts-service/loadtest-report-20260611-contacts-request-list-smoke.md`。contacts-service 仍保持独立事实源，不写 `conversation_members`，不自动创建会话，message-service 不同步依赖 contacts-service。
