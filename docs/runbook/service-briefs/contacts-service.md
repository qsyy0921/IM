...
- 已补只读 `outbox-audit`、只读 `outbox-repair-audit` 和 `outbox-repair-cleanup` 运维模式，可直接审计当前 `contacts_outbox`，并按 retention 清理 contacts outbox repair 历史；`outbox-audit` / `outbox-repair-audit` 可通过 `NEXUSIM_CONTACTS_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_CONTACTS_OUTBOX_REPAIR_AUDIT_OUTPUT` 写低敏 JSON 结果；租户隐私默认值 / 来源策略 audit 可通过 `NEXUSIM_CONTACTS_TENANT_PRIVACY_AUDIT_OUTPUT` / `NEXUSIM_CONTACTS_SOURCE_POLICY_AUDIT_OUTPUT` 写低敏策略 JSON，便于 operator 留存证据。
