...
5. 当前系统可以表述为“本地多进程 + Win/Mac 双机 Docker 最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route 和 WebSocket notify 串联；已在 clean commit `8c322fc` 跑通 Windows -> Mac Docker WebSocket gateway 的有线直连 smoke。下一步优先做跨实例 resume 设计或 Redis-backed resume buffer；不要把当前本地分布式 smoke 表述为完整生产多实例方案。
...
- 2026-06-09：已为 push-gateway 单实例 in-memory resume buffer 增加 TTL：`NEXUSIM_PUSH_RESUME_BUFFER_TTL` 默认 `10m`，registry 会在注册、通知入队和 `/debug/metrics` 读取时清理过期 token；过期 token 视为 `buffer_miss` 并签发新 token，不会重放旧通知；`/debug/metrics` 已新增 resume token count / expired count。当前仍未做跨实例 resume buffer，可靠恢复继续依赖 durable `PullInbox`。
