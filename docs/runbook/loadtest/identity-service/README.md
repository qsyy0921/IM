...
- challenge delivery repair / audit：一次性 operator mode 支持 `audit`、`redrive-active-pending`、`cancel-inactive`；另有只读 `challenge-delivery-repair-audit` 直接查询 repair 历史。DLQ row 不会复活旧 challenge token，必须走正常 API 重新申请 challenge。
