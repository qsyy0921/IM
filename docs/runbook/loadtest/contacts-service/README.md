...
`contacts-service` 已完成联系人 / 好友关系第一条真实小闭环，并补齐好友申请列表、接受 / 拒绝 / 删除 / 拉黑 / 解除拉黑 / 备注名路径：
...
- 当前 smoke 已验证 ACCEPT 后双向 ACTIVE edge、DECLINE 后不创建 edge、Delete/Block/Unblock/Remark 只修改当前 owner 视角 edge、删除后重新申请可以恢复联系人关系、outbox 清空、Kafka 读回对应 contact event。
