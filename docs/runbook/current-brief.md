...
1. 收口 Web / Windows PC 客户端：账号登录 / 注册、好友列表、好友申请、点击好友发起
   ...
   Tauri installer 和显式签名输入 readiness；`build:desktop-installer` 已提供显式
   `--execute` 门控的 installer build 包装器，但 readiness 不满足时仍 fail-closed，
   不执行 bundling 或签名；下一步继续 MSI / NSIS 和真实 code-signing execution pipeline。
