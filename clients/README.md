...
- `check:no-toolchain` runs the fast client shell guard set without launching
  ...
  `adb reverse` or installing toolchains. It includes desktop WebView metadata /
  login dry-run contracts, the Android login-level WebView smoke plan-only
  contract, the Android dry-run runner contract and reads low-sensitive ADB /
