...
状态：foundation-active / approved execution + guarded adapters + provider replay redrive result path。
...
- Provider replay result manifest 只是执行后证据绑定，不是 redrive 执行器，也不是 audit append。
