...
5. 当前系统可以表述为“本地多进程最小分布式 IM 链路”：`conversation-service / message-service / delivery-service / push-gateway` 独立进程协作，通过 PostgreSQL outbox、Kafka、durable inbox、Redis route 和 WebSocket notify 串联。下一步优先补真实 Redis 故障 smoke、resume buffer TTL，并单独设计 resume replay smoke。不要把当前本地分布式 smoke 表述为完整生产多实例方案。
