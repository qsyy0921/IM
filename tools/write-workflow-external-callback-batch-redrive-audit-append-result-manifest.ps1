...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path (Split-Path -Parent ([System.IO.Path]::GetFullPath($AuditAppendResultPath))) "workflow-external-callback-batch-redrive-audit-append-result-manifest.json"
}
Assert-ExternalRepairOutputPath -Value $OutputPath -FieldName "OutputPath"

if ([string]::IsNullOrWhiteSpace($ResultManifestID)) {
    $ResultManifestID = "workflow-callback-redrive-audit-append-result-" + [System.Guid]::NewGuid().ToString("N")
}
...
$auditManifest = Get-JsonDocument -Path $AuditManifestPath -Label "Workflow external callback batch redrive audit append manifest"
$auditResult = Get-JsonDocument -Path $AuditAppendResultPath -Label "Workflow external callback batch redrive audit append execution summary"
...
Assert-Same -Actual $recordType -Expected "WORKFLOW_EXTERNAL_CALLBACK_BATCH_REDRIVE" -FieldName "audit_manifest.record_type"
Assert-Same -Actual $action -Expected "REDRIVE_EXTERNAL_CALLBACK_DELIVERIES" -FieldName "audit_manifest.action"
Assert-Same -Actual $outcome -Expected "REDRIVEN" -FieldName "audit_manifest.outcome"
Assert-Same -Actual $reasonCode -Expected "WORKFLOW_EXTERNAL_CALLBACK_BATCH_REDRIVEN" -FieldName "audit_manifest.reason_code"
...
$manifest = [ordered]@{
    schema_version = "nexusim.workflow.external_callback_batch_redrive_audit_append_result.v1"
    ...
    redrives_external_callback = $false
    ...
    note = "Low-sensitive workflow external callback batch redrive audit append result manifest. It binds an executed external audit append summary to its manifest but does not append audit, redrive callback deliveries, record workflow decisions, call providers, execute target actions, mutate delivery facts, or embed raw callback/provider/payload material."
}

$encoded = $manifest | ConvertTo-Json -Depth 40 -Compress
Assert-NoRawDocumentText -Value $encoded -FieldName "workflow external callback batch redrive audit append result manifest"
...
Write-Host "OK   workflow external callback batch redrive audit append result manifest written: $OutputPath"
