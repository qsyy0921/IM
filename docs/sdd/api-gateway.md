...
conversation-service；群标题和头像 URI 的事实归 conversation-service 所有，BFF 不读
conversation-service 私表，也不在 gateway 内持久化 profile。
...
`NEXUSIM_API_GATEWAY_RATE_LIMIT_TENANT_PLANS_SOURCE` 默认 `auto`：存在 `TENANT_PLANS_JSON` 时使用 `inline`，否则存在 `TENANT_PLANS_FILE` 时使用 `file`，否则存在 `TENANT_PLANS_URL` 时使用 `url`，否则存在显式 `TENANT_PLANS_DB_DSN` 时使用 `db`，都不存在时为 `none`。通用 `NEXUSIM_PG_DSN` 只在显式 `SOURCE=db` 时作为 recovery 使用，不会在 `auto` 模式下隐式改变 tenant plan source。第一阶段支持 `inline/json`、`file`、`url` 与 `db`；如果配置为 `config-center` 或其它未知 source，api-gateway 必须在启动阶段 fail-closed，避免误以为完整配置中心 quota 已生效。当 `NEXUSIM_API_GATEWAY_RATE_LIMIT_TENANT_PLANS_RELOAD_INTERVAL` 配置为正 duration 时，source 必须是 `file`、`url` 或 `db`，api-gateway 会定期重读 / 拉取 tenant plan，并原子替换内存中的 plan map。

`db` source 读取 api-gateway 自有控制面表 `api_gateway_rate_limit_tenant_plans` 中 `enabled=true` 的行，字段为 `tenant_id / requests_per_second / burst / updated_at`。它会按最新 `updated_at` 生成 `quota-v1.db.<timestamp>` 版本，按计划内容计算 checksum-present 状态，并继续受 `MAX_AGE`、`REQUIRE_VERSIONED` 和 `REQUIRE_CHECKSUM` 约束。DB source 不读取业务服务内部表，不把 tenant 明细或 checksum 原文输出到 metrics。第一阶段提供本地 `NEXUSIM_API_GATEWAY_MODE=tenant-quota-audit / tenant-quota-set` operator，用于低敏审计、dry-run 预览和 upsert 该自有 quota 表；非 dry-run 可要求 `tenant_quota_approval` 低敏审批 manifest，执行前必须与本次 tenant / RPS / burst / enabled / source 精确匹配且未过期。这仍不是完整配置中心控制面，灰度发布、管理 API、provider-grade 审计和多环境发布治理仍是后续。
...
2026-06-18 补充：api-gateway tenant plan source 已新增 first-stage `db` adapter 和 expand-only migration `api_gateway_rate_limit_tenant_plans`。该 adapter 只读取 api-gateway 自有控制面表的 enabled row，生成 versioned / checksum-present snapshot，并支持 reload、max-age、checksum-required 和 versioned-required guard；`auto` 模式只在显式 `NEXUSIM_API_GATEWAY_RATE_LIMIT_TENANT_PLANS_DB_DSN` 存在时选择 DB，通用 `NEXUSIM_PG_DSN` 只作为显式 `SOURCE=db` recovery。这不是完整配置中心控制面，套餐审批、发布审计、灰度和多环境治理仍是后续。

2026-06-18 补充：api-gateway 已新增 first-stage 本地 `tenant-quota-audit` / `tenant-quota-set` operator。operator 只操作 api-gateway 自有 `api_gateway_rate_limit_tenant_plans` 表；`tenant-quota-set` 支持 `NEXUSIM_API_GATEWAY_TENANT_QUOTA_SET_DRY_RUN=true` 只输出 planned row，不连接数据库、不写表。非 dry-run 可启用 `NEXUSIM_API_GATEWAY_TENANT_QUOTA_SET_REQUIRE_APPROVAL=true` 并提供 `NEXUSIM_API_GATEWAY_TENANT_QUOTA_SET_APPROVAL_PATH`；审批 manifest 的 schema 为 `nexusim.api_gateway.tenant_quota_approval.v1`，只允许低敏 operator / approver / change / environment 字段，且 desired plan 必须和本次 set 参数精确匹配。JSON 输出只包含 tenant id、RPS、burst、enabled、source、更新时间和低敏审批摘要，不输出 DSN、token、业务 payload 或配置源凭据。该能力是本地 quota 控制面 / 审批包入口，不等于完整配置中心、灰度发布、计费或多环境发布治理。
...
2026-06-15 补充：api-gateway legacy descriptor quiet-window 已新增 observation 归档脚本 `tools/record-api-gateway-legacy-observation.ps1`。它基于 live `/debug/metrics` 或离线 snapshot 运行 legacy migration gate，并把 `api-gateway-metrics.json`、`legacy-gate-output.txt`、`legacy-observation-summary.json` 和 `legacy-observation-report.md` 写入 `H:\NexusIM\loadtest-results/<run>`。这只证明单次 snapshot 和所选 gate 参数，不等于生产全环境迁移完成。
...
2026-06-14 补充：本地 Prometheus 原型配置已新增 `deploy/local/docker-compose.prometheus.yml`、`prometheus.yml` 和 `prometheus-api-gateway-alerts.yml`。默认 scrape `host.docker.internal:11904/metrics`，并提供 api-gateway gRPC error、legacy descriptor traffic、legacy descriptor still-registered、legacy opt-in deadline expired、rate-limit Redis / identity / tenant-plan reload error、tenant quota snapshot stale、JWKS refresh failure 和 OTLP endpoint missing 的 first-stage alert rules；这些规则只用于本地开发 / 演示，不是生产 SLO、retention、Alertmanager route 或多服务 dashboard。

2026-06-14 补充：本地 Grafana dashboard 原型已新增 `deploy/local/docker-compose.grafana.yml`、datasource / dashboard provider 和 `api-gateway-observability.json`。默认 datasource 指向 `http://host.docker.internal:19090`，dashboard 覆盖 request rate、error rate、legacy/facade exposure、legacy descriptor registered、latency、rate-limit decisions / dependency errors、JWKS refresh failures 和 OTel enabled；它只用于本地开发 / 演示，不是生产 Grafana、权限、datasource secret 管理、retention 或 SLO dashboard。
