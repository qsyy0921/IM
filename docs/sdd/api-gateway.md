...
2026-06-18 补充：api-gateway 已新增 first-stage 本地 `tenant-quota-audit` / `tenant-quota-set` operator。operator 只操作 api-gateway 自有 `api_gateway_rate_limit_tenant_plans` 表；`tenant-quota-set` 支持 `NEXUSIM_API_GATEWAY_TENANT_QUOTA_SET_DRY_RUN=true` 只输出 planned row，不连接数据库、不写表。JSON 输出只包含 tenant id、RPS、burst、enabled、source 和更新时间，不输出 DSN、token、业务 payload 或配置源凭据。该能力是本地 quota 控制面入口，不等于完整配置中心、套餐审批、灰度发布、计费或多环境发布治理。
