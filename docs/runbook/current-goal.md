...
- 同日 `loadtest/ragagent` 已补真实业务 proposal source-chain 场景：
  ...
  并会在 preflight 中把 conversation-service endpoint 纳入必需检查。2026-06-25
  已恢复 Docker / PostgreSQL runtime，并通过
...
4. 后端 / AI 演示主线优先做：
   ...
   和完整 service-stack wrapper execute-mode 入口已补齐，并已通过真实本地服务栈
