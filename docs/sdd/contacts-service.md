...
- 联系人申请隐私设置只控制“陌生用户能否新建好友申请”和第一阶段陌生人 profile 可见性偏好；读取优先级为用户显式设置、租户默认、系统默认；其中 `allow_search_contact_requests=false` 只拒绝 `SEARCH` 来源的新建申请，仍可允许 DIRECT / GROUP / INVITE 等其它来源；`allow_profile_visibility=false` 只表达 contacts-service 可发布 / 投影的 profile 可见性偏好，不删除已有联系人，不影响已存在 pending request，不直接改变 message / conversation / delivery 权限。
...
| ContactPrivacySettings | 当前用户的联系人申请隐私设置和第一阶段 profile visibility 偏好 | 缺省视为 `allow_contact_requests=true, allow_search_contact_requests=true, allow_profile_visibility=true`；只有当前 auth user 可读写自己的设置；更新必须幂等并发布 `contact.privacy.updated.v1` |
...
缺省没有用户级 `contact_privacy_settings` 行时，读取租户级 `contact_tenant_privacy_defaults`；如果租户默认也不存在，则返回 `allow_contact_requests=true, allow_search_contact_requests=true, allow_profile_visibility=true, version=0, policy_source=SYSTEM_DEFAULT`。有租户默认时返回 `policy_source=TENANT_DEFAULT`，有用户显式设置时返回 `policy_source=USER`。
...
allow_profile_visibility (optional)
idempotency_key
```

`SetContactPrivacy` 只允许当前 auth user 修改自己的设置。`allow_search_contact_requests` 和 `allow_profile_visibility` 是兼容增量字段：未传时保留当前有效值，传入后可单独控制 `SEARCH` 来源的陌生申请和第一阶段陌生人 profile 可见性偏好。更新使用 `SET_CONTACT_PRIVACY` 幂等键；同一命令 replay 返回原设置快照，不重复发布事件；相同 idempotency key 但参数不同返回 conflict。
...
这些事件不表示 conversation membership 已变化，也不表示 message-service 发送权限立即变化。policy-service / conversation-service 后续可以消费 `contact.edge.blocked.v1` 建立自己的权限投影，但 contacts-service 不直接写其它服务内部表。`contact.privacy.updated.v1` 只表示该用户是否接受新的联系人申请、是否接受 `SEARCH` 来源的新申请，以及第一阶段 profile 可见性偏好；不表示已有联系人或已有 pending request 被撤销。
...
-> if row exists and value changes, update allow_contact_requests / allow_search_contact_requests / allow_profile_visibility and version = version + 1
...
- `SetContactPrivacy` 包含 command type、tenant、user、allow_contact_requests；若请求显式携带 `allow_search_contact_requests` 或 `allow_profile_visibility`，也纳入 command hash，未携带时保留当前有效值。
...
- `GetContactPrivacy` / `SetContactPrivacy` 只能读写当前 auth user 自己的联系人申请隐私设置；缺省允许好友申请、搜索来源申请和第一阶段 profile visibility。申请开关只 gate 新建 `SendContactRequest`，profile visibility 只表达后续 profile / search / policy projection 可消费的展示偏好，不直接撤销已有申请或联系人事实。
...
$env:NEXUSIM_CONTACTS_TENANT_PRIVACY_ALLOW_PROFILE_VISIBILITY='true'
.\contacts-service.exe
```
...
另有只读 `outbox-audit` 运维模式，可直接查询 `contacts_outbox` 当前行，并按 `outbox_id / event_id / tenant_id / aggregate_id / status / event_type / created_at RFC3339 时间窗口` 缩小排障范围；它不 redrive，不修改当前 outbox 状态。
...
另有只读 `outbox-repair-audit` 运维模式，可直接查询 `contacts_outbox_repair_audit` 历史，并按 `event_id / tenant_id` 缩小排障范围；它不 redrive，也不修改 `contacts_outbox` 当前状态。`outbox-repair-cleanup` 则按 `retention + batch_size` 清理过期 `contacts_outbox_repair_audit` 行，并支持 `event_id / tenant_id` 范围收窄；它支持 `NEXUSIM_CONTACTS_OUTBOX_REPAIR_CLEANUP_DRY_RUN=true` 只统计候选行不删除，低敏 JSON summary 会写出 `dry_run`；它只删除 repair 历史，不改写当前 outbox 状态。
