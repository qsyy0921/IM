...
2026-06-14 补充：本地 Grafana dashboard 原型已新增 `deploy/local/docker-compose.grafana.yml`、datasource / dashboard provider 和 `api-gateway-observability.json`。默认 datasource 指向 `http://host.docker.internal:19091`，dashboard 覆盖 request rate、error rate、legacy/facade exposure、legacy descriptor registered、latency、rate-limit decisions / dependency errors、JWKS refresh failures 和 OTel enabled；它只用于本地开发 / 演示，不是生产 Grafana、权限、datasource secret 管理、retention 或 SLO dashboard。
