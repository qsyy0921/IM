...
当前优先级：
1. 先治理已有 9 个微服务，新增服务后置；下一步优先做 `api-gateway` 入口治理：租户级 quota / rate-limit hardening、低敏 metrics 和稳定错误语义。`search-service` 只保留 SDD draft，不进入 proto / migration / skeleton。
