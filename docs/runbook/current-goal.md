...
`check:no-toolchain` 聚合 Web PWA、shell config、Web platform、shared runtime /
local-store contracts、Web shell lifecycle / automation / smoke-report contracts、
shell asset、desktop / Android action asset、clientweb smoke hook contract、
artifact readiness / install-plan / builder / collector contracts、Android builder
profile / wrapper contracts、desktop WebView metadata / login dry-run contracts、
Android metadata / login dry-run contracts、Android device / WebView devtools
readiness and parser contracts、Android platform readiness 和 shell smoke plan
checks；它不构建 native artifact
