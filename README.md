...
`plan:desktop-signing` 可以检查显式 `signtool`、证书来源和 timestamp URL readiness，
但只输出低敏计划，不执行签名。
...
unsigned local desktop bundle、desktop signing readiness plan 和 Android debug APK baseline 已有；`loadtest/clientweb` 已扩展到群成员列表、移除、
...
- 完整 Web / App / 桌面客户端；当前 Web / Windows PC shell 已有账号登录、注册、好友、
  ...
  package-local README / launcher support files、unsigned local portable zip bundle、desktop signing readiness plan、Android debug APK baseline 已落，真实双用户 direct + group client smoke 已通过；仍缺
