...
`contacts-service` 已完成联系人 / 好友关系第一条真实小闭环，并补齐接受 / 拒绝两条路径：
...
- 当前 smoke 已验证 ACCEPT 后双向 ACTIVE edge、DECLINE 后不创建 edge、outbox 清空、Kafka 读回 `contact.request.created.v1` / `contact.request.accepted.v1` / `contact.request.declined.v1`。
