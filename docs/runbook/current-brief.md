...
- 当前不是生产级 HA：本地 PostgreSQL `repmgr + pgpool` failover smoke、本地 PostgreSQL quorum observation、本地 Kafka KRaft 三 broker failover / controller-switch smoke、本地 Redis Sentinel quorum-loss fallback smoke、以及本地 Redis Sentinel network-partition fallback smoke 已补齐；服务发现、统一观测和部署编排仍是后续。PostgreSQL observation 已证明当前本地 `repmgr + pgpool` 拓扑不是 quorum-fenced：两个 standby 停止后 only-primary 仍可写。
...
3. 故障 smoke：继续补 Kafka 多故障 / ISR 抖动，并为 PostgreSQL 生产级 quorum / split-brain 设计补方案；Redis Sentinel 网络分区、Kafka controller-switch 和 PostgreSQL quorum observation 已有 clean run 和报告。
