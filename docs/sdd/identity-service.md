...
- Invalid TOTP codes are counted on the selected MFA factor, independent from password failure counters. Reaching `NEXUSIM_IDENTITY_MFA_MAX_FAILED_ATTEMPTS` within `NEXUSIM_IDENTITY_MFA_FAILURE_WINDOW` locks that factor for `NEXUSIM_IDENTITY_MFA_LOCK_DURATION` and public Login returns stable `mfa temporarily locked`. Login and Refresh transactions re-lock the selected ACTIVE TOTP factor and re-check `login_locked_until` before accepting the proof, so a factor locked after the app-layer factor read still fails closed and does not write session / refresh-token state.
- Invalid MFA recovery-code attempts are counted on the user row, independent from password and TOTP factor counters. Recovery-code risk has its own env keys: `NEXUSIM_IDENTITY_MFA_RECOVERY_MAX_FAILED_ATTEMPTS`, `NEXUSIM_IDENTITY_MFA_RECOVERY_FAILURE_WINDOW` and `NEXUSIM_IDENTITY_MFA_RECOVERY_LOCK_DURATION`. If they are unset, identity-service uses the built-in MFA risk defaults instead of inheriting the TOTP env values; a locked recovery-code path returns stable `mfa temporarily locked`.
- A valid MFA recovery code is consumed in the same PostgreSQL transaction that writes session / refresh-token state and clears user-level recovery-code failure state. The transaction re-locks the user row and re-checks `mfa_recovery_locked_until` before consuming the code, so a recovery-code path locked after the app-layer credential read still fails closed and does not write session / refresh-token state. Recovery-code proof does not update TOTP `last_used_at` and does not count as a TOTP failed-code attempt.
...
- `NEXUSIM_IDENTITY_MFA_SECRET_KEY` is the local AES-GCM encryption key input. The MFA secret manager no longer reuses gateway-token or push-token secrets. Production-like local profiles can use `NEXUSIM_IDENTITY_MFA_SECRET_KEYRING_JSON` / `NEXUSIM_IDENTITY_MFA_SECRET_KEYRING_FILE` with one current key version plus old key versions, so newly enrolled TOTP factors are encrypted with the current version while existing factors can still be decrypted during a rotation window. This is local envelope keyring support, not KMS/HSM. If no MFA key is configured, the service still starts and existing Login/JWKS flows are unaffected, but MFA factor RPCs and MFA-protected Login return stable `MFA_UNAVAILABLE` / `mfa temporarily unavailable` until the key is configured.
- `NEXUSIM_IDENTITY_MFA_RECOVERY_CODE_SECRET` is the required HMAC secret for recovery-code hashes. It no longer reuses the MFA encryption key, gateway-token secret or push-token secret. If absent, recovery-code generation and recovery-code MFA proof return stable `MFA_UNAVAILABLE`.
- `NEXUSIM_IDENTITY_PRODUCTION_KEY_GUARD=true` is an opt-in startup guard for production-like profiles. In `grpc` mode it requires RS256 gateway-token signing, an explicit MFA key or keyring, an explicit recovery-code secret and an explicit challenge request-limit secret; when challenge delivery uses durable `outbox`, it also requires an explicit challenge-delivery token key or keyring. The `challenge-delivery-worker` mode validates only the challenge-delivery token key / keyring. This guard prevents non-production key modes from silently reaching production-like runs; it is still not KMS/HSM-backed key management.
...
- `NEXUSIM_IDENTITY_SERVICE_MODE=challenge-request-limit-cleanup` is a one-shot operator that deletes stale hashed-target limiter rows whose `last_request_at` and `locked_until` are both older than the retention cutoff. It supports `NEXUSIM_IDENTITY_CHALLENGE_REQUEST_LIMIT_CLEANUP_DRY_RUN=true` to count matching rows without deleting and records `dry_run` in the low-sensitive JSON summary. It is a table-retention guard for random-target spam, not a risk-decision engine.
...
```text
NEXUSIM_IDENTITY_CHALLENGE_DELIVERY_MODE=noop|webhook|smtp|outbox
...
NEXUSIM_IDENTITY_CHALLENGE_SMTP_ADDR=smtp.example.com:587
NEXUSIM_IDENTITY_CHALLENGE_SMTP_FROM="NexusIM <no-reply@example.com>"
NEXUSIM_IDENTITY_CHALLENGE_SMTP_USERNAME=...
NEXUSIM_IDENTITY_CHALLENGE_SMTP_PASSWORD=...
...
NEXUSIM_IDENTITY_CHALLENGE_DELIVERY_WORKER_PROVIDER=webhook|smtp
...
Optional first-stage OIDC discovery is available at `/.well-known/openid-configuration` only when `NEXUSIM_IDENTITY_OIDC_DISCOVERY_ENABLED=true`. This endpoint is metadata for NexusIM gateway-token consumers, not external IdP federation or OAuth login. It requires an absolute URL issuer (`NEXUSIM_IDENTITY_OIDC_ISSUER`, or the effective gateway-token issuer when that issuer is already a URL) and a non-empty RS256 public JWKS. `NEXUSIM_IDENTITY_OIDC_JWKS_URI` may override the default `<issuer>/.well-known/jwks.json`. HTTP issuers / JWKS URIs are accepted only for localhost or private addresses; production-like public issuers must use HTTPS. HS256 / legacy modes intentionally cannot publish OIDC discovery because identity-service never exposes symmetric signing keys through JWKS.
...
The `grpc` mode also supports first-stage OpenTelemetry gRPC server spans, disabled by default. When enabled, spans inherit W3C `traceparent` from incoming gRPC metadata and only record low-sensitive, low-cardinality attributes: gRPC full method, status code, and latency. Spans must not include tokens, passwords, TOTP / recovery codes, user IDs, device IDs, session IDs, `trace_id`, `request_id`, challenge destinations, provider URLs, provider error bodies, or payloads. `x-nexusim-trace-id` / `x-nexusim-request-id` remain available for metadata / access-log correlation, but are not exported as span attributes.
