...
1. 9 服务必要收口和 AI 大模型应用底座转进：
   当前 9 个服务已跑通 IM 主链路，下一步不再把短期生产级测试、完整 HA、长压和生产 sizing 作为阻塞条件。只对现有服务做必要收口：消息编辑 / 撤回 / 删除的可检索 tombstone、群管理和 owner transfer、成员历史可见窗口、回执 / 未读 / 会话摘要、联系人隐私 / 分组 / 搜索、policy 决策来源和 audit 链路。AI 侧第一步是 `search-service v0.1`，后续依次进入 `memory-service`、`retrieval-gateway`、RAG、`summary-service`、Agent、`skill-registry`、`mcp-gateway`、`action-executor`；RAG / summary / Agent / action 不能直接读业务库或绕过 policy。

2. AI 大模型应用底座架构深化：
   需要把 `target-architecture-ai.md` 中的 group memory、StructuredMemoryEvent、Memory Graph、ProfileAggregate、EvidencePack、ActionProposal、Tool Policy、skill registry、MCP gateway 和 action executor 落成可执行 SDD / ADR。`memory-service` 已从 SDD / proto / migration contract 切到 foundation-active implementation 并跑通 clean projection smoke；`retrieval-gateway` 已完成 first-stage EvidencePack 字段 hardening（rerank score、source coverage、dedupe reason）；`rag-service` 已完成 first-stage read-only answer path、`loadtest/rag`、RAG eval execution adapter、真实 `retrieval -> rag` adapter smoke、provider boundary 和 citation verifier first pass；后续进入 `summary-service`，外部 LLM adapter 仍需 prompt boundary、token budget、PII / secret filter 和 provider failure fallback。AI eval harness first pass 已落低敏 case schema / validator，当前覆盖 retrieval miss、temporal version、attribution、permission leak、RAG grounded citations 和 tool policy violation；后续需要执行 adapter 覆盖 profile overgeneralization、action execution safety 和真实 Agent 输出。2025/2026 memory 论文和 benchmark 的设计启发必须继续进入 `memory-service`：source refs、speaker / audience scope、valid_from / valid_to、supersedes / contradicts、confidence、PENDING / ACTIVE / SUPERSEDED / REJECTED 状态、profile 不能从单条群消息直接升级。Agent 可以接入真实业务，但写动作必须走 policy precheck、tool policy、proposal / approval / executor / audit；高风险动作第一阶段禁止自动执行。
...
- `skill-registry`：登记可被 Agent 调用的技能、输入输出合约、权限和审计元数据。
- `mcp-gateway`：把外部工具 / MCP server 纳入租户、policy、审计和限流边界。
- `action-executor`：承接 proposal / approval 后的低风险动作执行和审计闭环。
- `ai-eval-service` / AI eval harness：first-stage 低敏 case schema / validator 已落；后续补真实 EvidencePack execution adapter、RAG answer adapter、Agent tool/action safety adapter。
