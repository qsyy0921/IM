...
5. 不把具体任务写死在 prompt 里；具体下一步以 current-brief.md 为准，current-goal.md 只作长档案和按需索引。
6. 公共契约、migration、事务、幂等、消息顺序、错误码、可运行链路完成时，再按 current-goal.md 的评审规则邀请独立评审。
7. 有意义的切片完成后运行必要检查，更新 current-brief.md；阶段状态变化时同步 current-goal.md 和对应 runbook/loadtest 报告。
8. 按 current-goal.md 的 GitHub 同步策略批量提交和推送，不为低风险小改动频繁推送。
```
...
| receipt-service SDD / implementation | `docs/sdd/receipt-service.md` 已新增 v0.1 Draft；定位为第三层 IM 产品能力，基于 `im.delivery.events` 重建送达 / 已读回执 read model，`delivery.ack.recorded.v1` 只表示 received，不等于 read；read 必须由客户端显式 `MarkRead` 推进；已补 `ReceiptAccessPort` 权限来源、`delivery.inbox_item.created.v1.sender_id`、receipt API proto、`im.receipt.events` schema、receipt migration、六层骨架、PostgreSQL repository、delivery event consumer、receipt outbox relay、`grpc` / `delivery-consumer` / `outbox-relay` 运行模式和 `MarkRead` 事务；repository 集成测试覆盖 inbox projection、ack projection、ack-before-inbox、read cursor 范围校验和 outbox 幂等；clean commit `3c28105` 已跑通真实进程 smoke：`im.delivery.events -> receipt projection -> GetReceiptState(received) -> MarkRead -> GetReceiptState(read)`，并验证 `MarkRead(seq+1)` 返回 `FailedPrecondition`；clean commit `abfe0ec` 已跑通 receipt outbox relay smoke：`receipt_outbox -> im.receipt.events`，`receipt_outbox PUBLISHED=2 / PENDING=0 / DLQ=0`，Kafka 读回 `receipt.message.received.v1` 和 `receipt.message.read.v1`；原始结果在 `H:\NexusIM\loadtest-results\receipt-service-outbox-smoke-20260610-005435`，报告见 `docs/runbook/loadtest/receipt-service/loadtest-report-20260610-receipt-outbox-smoke.md`；下一步可进入消息编辑/撤回/删除、会话列表/未读数或真实鉴权等第三层产品能力 |
| receipt-service conversation list / unread SDD | 已新增 `docs/sdd/receipt-service-conversation-list.md` v0.1 Draft；会话列表 / 未读数作为 receipt-service 扩展 projection 实现，不新增 `conversation-list-service`，以控制服务数量和复杂度；最小链路为 `im.delivery.events -> receipt_inbox_projection -> user_conversation_summaries -> ListConversations`，`MarkRead` 同事务更新 summary；明确 `delivery.ack.recorded.v1` 只是 received，不清 unread；API 需返回 `ProjectionWatermark`，Kafka checkpoint 必须在 DB 事务提交后推进；第一阶段不读 delivery/message/conversation 内部表、不返回消息正文、不做 pin/mute/archive/draft、不做 Redis unread counter |
