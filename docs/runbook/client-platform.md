...
- PC desktop and Android now both have first-stage TypeScript runtime adapters.
  ...
  the certificate exists, has a usable signing key and is not expired.
  `plan:desktop-signing` can carry an expected public signer subject policy from
  CLI, env or profile, but it remains a plan-only report and does not claim that
  the signer has been verified. Read-only signature verification and installer
  readiness remain blocked if a valid signature does not match it. It
  defaults to
