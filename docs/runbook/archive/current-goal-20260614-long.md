...
- api-gateway 补充：第一版 gRPC 入口 skeleton 已落地，复用共享 `internal/gatewayauth` 验证 gateway token，新增 `nexusim.gateway.v1.GatewayService` public facade proto，代理 conversation / message / delivery / receipt / contacts user-facing RPC，重写 body `AuthContext` 并注入 trusted metadata；facade 不包含服务间 `GetSendContext`；contacts 事实仍归 contacts-service，message-service 不同步依赖 contacts-service；legacy service descriptor 默认保留用于兼容，且可通过 `NEXUSIM_API_GATEWAY_REGISTER_LEGACY_DESCRIPTORS=false` 关闭。入口 gRPC 和下游 client 均已支持静态 TLS / mTLS。后端启用 verified-metadata auth 时必须配合 loopback / 内网隔离或 mTLS peer allowlist。clean commit `cff1668` 已跑通 demo 经真实 api-gateway 的 secure E2E smoke；clean commit `9335bd1` 已补 first-stage `/healthz`、`/readyz`、`/debug/metrics`、低敏 gRPC JSON access log 和默认 `api-gateway` audience，并跑通 `e2e-demo-api-gateway-audience-smoke-20260613-clean`。clean commit `bb13300` 已跑通 secure demo 经 `GatewayService` facade 的真实进程 smoke：summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac/gateway_auth_audience=api-gateway`，api-gateway metrics 显示 user-facing calls 均为 `/nexusim.gateway.v1.GatewayService/...`，报告见 `docs/runbook/loadtest/demo/loadtest-report-20260613-e2e-demo-api-gateway-facade-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\e2e-demo-api-gateway-facade-smoke-20260613-clean`；clean commit `c1328ca` 已跑通 legacy descriptor disabled 的 facade-only smoke，api-gateway metrics 只出现 `/nexusim.gateway.v1.GatewayService/...` user-facing method，报告见 `docs/runbook/loadtest/demo/loadtest-report-20260614-e2e-demo-api-gateway-facade-only-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\e2e-demo-api-gateway-facade-only-smoke-20260614-clean`；clean commit `f3b290f` 已跑通 contacts 经 `GatewayService` facade 的真实进程 smoke，summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac`，api-gateway access log 只出现 contacts facade method，contacts outbox `PUBLISHED=2/PENDING=0/DLQ=0`，报告见 `docs/runbook/loadtest/contacts-service/loadtest-report-20260614-contacts-api-gateway-facade-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\contacts-api-gateway-facade-smoke-20260614-clean`；clean commit `ccae224` 已补 first-stage correlation propagation：api-gateway 在 trace/request 缺失时生成 `trace_* / request_*`，写入下游 trusted metadata 和 gRPC response header，并让低敏 access log 使用最终 correlation；contacts facade correlation smoke 已通过，报告见 `docs/runbook/loadtest/api-gateway/loadtest-report-20260614-api-gateway-correlation-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\contacts-api-gateway-correlation-smoke-20260614-clean`。历史 `push-gateway` audience 只作为显式 env 兼容。已补第一阶段 gRPC rate limiter：默认关闭，`local` backend 为进程内 token bucket，`redis` backend 为跨实例 fixed-window counter，显式启用后按 method + token fingerprint / peer address 限流，返回 `ResourceExhausted`，并在 `/debug/metrics` 输出低敏 `rate_limit` 聚合；它不是完整 tenant quota / WAF / 风控系统。后续是更完整 OpenTelemetry trace、配额治理和逐步收敛 legacy descriptor。
  补充：clean commit `b022a09` 已补 contacts-service 下游 gRPC access log 输出 api-gateway 注入的 `trace_id / request_id`，contacts 经 `GatewayService` facade 的 downstream correlation smoke 已通过，summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac`，contacts-service 日志显示 `SendContactRequest / ListContactRequests / RespondContactRequest / ListContacts / GetContactState` 均带同一组 correlation；报告见 `docs/runbook/loadtest/api-gateway/loadtest-report-20260614-api-gateway-downstream-correlation-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\contacts-api-gateway-downstream-correlation-smoke-20260614-clean`。这仍不是全服务 OpenTelemetry / collector / alerting。
  补充：clean commit `ce69c88` 已补 message-service 下游 gRPC access log 输出 api-gateway 注入的 `trace_id / request_id`，secure E2E demo 经 `GatewayService` facade 的 message correlation smoke 已通过，summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac`，api-gateway 和 message-service 的 `SendMessage` access log 均显示 `trace_id=e2e-demo-send/request_id=e2e-demo-send`；报告见 `docs/runbook/loadtest/api-gateway/loadtest-report-20260614-api-gateway-message-correlation-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\e2e-demo-api-gateway-message-correlation-smoke-20260614-clean`。这仍不是全服务 OpenTelemetry / collector / alerting。
  补充：clean commit `50685b4` 已补 delivery-service 与 receipt-service 下游 gRPC access log 输出 api-gateway 注入的 `trace_id / request_id`，secure E2E demo 经 `GatewayService` facade 的 delivery / receipt correlation smoke 已通过，summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac`，api-gateway `PullInbox / ListConversations / MarkRead` 与下游 delivery-service `PullInbox`、receipt-service `ListConversations / MarkRead` access log 均显示同一组 correlation；报告见 `docs/runbook/loadtest/api-gateway/loadtest-report-20260614-api-gateway-delivery-receipt-correlation-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\e2e-demo-api-gateway-delivery-receipt-correlation-smoke-20260614-clean`。这仍不是全服务 OpenTelemetry / collector / alerting。
  补充：clean commit `c6c3bc5` 已补 conversation-service 下游 gRPC access log 输出 api-gateway 注入的 `trace_id / request_id`，secure E2E demo 经 `GatewayService` facade 的 conversation correlation smoke 已通过，summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac`，api-gateway `CreateMemberChange` 与 conversation-service `CreateMemberChange` access log 均显示 `trace_id=e2e-demo-join/request_id=e2e-demo-join`；报告见 `docs/runbook/loadtest/api-gateway/loadtest-report-20260614-api-gateway-conversation-correlation-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\e2e-demo-api-gateway-conversation-correlation-smoke-20260614-clean`。这仍不是全服务 OpenTelemetry；message-service -> conversation-service `GetSendContext` 服务间 RPC correlation 透传仍是后续项。
  补充：clean commit `4ecb05b` 已补 message-service conversation RPC client 的服务间 correlation metadata 透传，secure E2E demo 经 `GatewayService` facade 的 message -> conversation correlation smoke 已通过，summary `git_dirty=false/success=true/gateway_facade=true/gateway_auth_mode=hmac`，api-gateway `SendMessage`、message-service `SendMessage` 和 conversation-service `GetSendContext` access log 均显示 `trace_id=e2e-demo-send/request_id=e2e-demo-send`；报告见 `docs/runbook/loadtest/api-gateway/loadtest-report-20260614-message-conversation-correlation-smoke.md`，原始结果在 `H:\NexusIM\loadtest-results\e2e-demo-message-conversation-correlation-smoke-20260614-clean`。这仍不是全服务 OpenTelemetry / collector / alerting。
  补充：clean commit `ee4461e` 已补 api-gateway rate-limit 拒绝响应的 gRPC `RetryInfo`：local token bucket 使用下一枚 token 可用时间，Redis fixed-window 使用下一窗口剩余时间。客户端仍应叠加自身指数退避 / jitter；这不是完整 tenant quota / WAF / 风控系统。
  补充：clean commit `9b16b8c` 已修正 api-gateway Redis rate-limit fail-open 启动语义：`FAIL_OPEN=true` 时 Redis `PING` 失败只记录启动日志并继续启动，运行时 Redis 错误进入 `redis_error_count` 并放行；`FAIL_OPEN=false` 仍 fail-closed，启动探测失败会阻止进程启动。
  补充：clean commit `b4b3714` 已补 api-gateway 第一阶段 W3C `traceparent` 输入桥接：当 gateway token 和 `x-nexusim-trace-id` 都没有 trace id 时，从合法 `traceparent` 提取 32 hex trace id 并继续进入现有 `x-nexusim-trace-id` 下游 metadata、gRPC response header 和低敏 access log；非法 `traceparent` 被忽略并回退到本地生成。这是外部 trace 入口兼容，不是完整 OpenTelemetry span / collector / exporter。
  补充：clean commit `3473ffc` 已把 identity-service public / self-service RPC 接入 `GatewayService` facade，覆盖 `RegisterUser / Login / RefreshGatewayToken / RequestVerificationChallenge / ConfirmVerificationChallenge / RequestPasswordReset / ConfirmPasswordReset / BeginMFAEnrollment / ConfirmMFAEnrollment / DisableMFAFactor / RegenerateMFARecoveryCodes / RevokeMFARecoveryCodes`。这些入口不要求已有 gateway token，只补齐/透传 correlation 后转发到 identity-service；`IssueGatewayToken / RevokeDevice / RevokeSession / GetDeviceState` 不进入 public facade，也不注册 legacy `IdentityService` descriptor。clean commit `443029a` 已跑通 identity challenge delivery 经 `GatewayService` facade 的真实进程 smoke：`RegisterUser -> RequestVerificationChallenge(outbox) -> challenge-delivery-worker -> webhook token -> ConfirmVerificationChallenge`，summary `git_dirty=false/success=true/gateway_facade=true`，报告见 `docs/runbook/loadtest/identity-service/loadtest-report-20260614-identity-api-gateway-facade-smoke.md`。clean commit `d07373a` 已把该 smoke 扩展到 `Login -> RefreshGatewayToken`，summary `login.gateway_token_set=true/refresh_token_set=true`、`refresh_gateway_token.refresh_token_rotated=true`，且不写 token 明文；原始结果在 `H:\NexusIM\loadtest-results\identity-api-gateway-login-refresh-smoke-20260614-clean`。
  补充：clean commit `852a992` 已把同一 identity facade smoke 扩展到 `RequestPasswordReset -> ConfirmPasswordReset -> post-reset Login`，summary `git_dirty=false/success=true/gateway_facade=true`，`challenge_delivery_outbox.total=2/delivered=2/pending=0/dlq=0`，且不写 verification token、password reset token、gateway token 或 refresh token 明文；原始结果在 `H:\NexusIM\loadtest-results\identity-api-gateway-password-reset-smoke-20260614-clean`。
  补充：clean commit `5fbc622` 已把同一 identity facade smoke 扩展到 MFA lifecycle：`BeginMFAEnrollment -> ConfirmMFAEnrollment -> Login without MFA rejected -> Login with TOTP -> RegenerateMFARecoveryCodes -> RevokeMFARecoveryCodes -> DisableMFAFactor`，summary `git_dirty=false/success=true/gateway_facade=true`，factor 状态覆盖 `PENDING -> ACTIVE -> DISABLED`，recovery codes 再生成 / 吊销均为 10 条，且不写 TOTP secret、otpauth URI、recovery code 或 token 明文；原始结果在 `H:\NexusIM\loadtest-results\identity-api-gateway-mfa-facade-smoke-20260614-clean`。
  补充：clean commit `e464209` 已把同一 identity facade smoke 扩展到 Refresh step-up：启用 ACTIVE TOTP 后，旧 password-only refresh token 不带 MFA proof 返回 `FailedPrecondition`，带 TOTP proof 则成功签发并轮换 refresh token；summary `refresh_without_mfa.code=FailedPrecondition`、`refresh_with_mfa.refresh_token_rotated=true`，且不写任何 token / TOTP secret / recovery code 明文；原始结果在 `H:\NexusIM\loadtest-results\identity-api-gateway-refresh-stepup-smoke-20260614-clean`。
...
- 真实鉴权、设备绑定、session revoke：当前已完成 gateway token 本地验签、HS256/RS256/JWKS、issuer allowlist、RegisterUser / Login / RefreshGatewayToken、refresh token rotation / reuse detection、device/session revoke event、push-gateway deny-list projection 和 active close；已跑通 Login/RegisterUser 到 push-gateway 的 clean smoke 以及 device/session revoke active close smoke。identity-service hardening 已覆盖：密码 Login 失败计数 / 短锁定、Login 缺失/非 ACTIVE credential 的 dummy verifier timing hardening、email/phone verification、password reset challenge hash、challenge active cap / target-level request window throttle、password reset HMAC target limiter / cleanup operator / debug metrics、challenge webhook sender 第一版及 delivery failure 后过期补偿、challenge delivery debug metrics / failure class 聚合 / 持久状态审计 / encrypted outbox + worker retry-DLQ / outbox 聚合状态指标 / operator repair audit、challenge / delivery outbox / repair audit 低敏 failure class 持久化、MFA TOTP secret / challenge delivery token 本地版本化 keyring、gRPC 结构化日志 trace/request id 透传，并已跑通 `RequestVerificationChallenge(outbox) -> challenge-delivery-worker -> webhook token -> ConfirmVerificationChallenge` 真实进程 smoke；repair mode 支持 `audit`、`redrive-active-pending`、`cancel-inactive`，但不会复活 DLQ 后已过期的 challenge token，DLQ 用户必须通过正常 API 重新申请 challenge；密码重置后撤销 ACTIVE session/refresh token、TOTP MFA 生命周期、Login 强制 MFA、TOTP factor 失败计数 / 短锁定、`MFA_UNAVAILABLE`、MFA recovery codes 生成 / 再生成 / 吊销、recovery-code 登录 user-level 失败计数 / 短锁定、Refresh-token step-up、Refresh 期间直接提交 TOTP / recovery MFA proof、session MFA proof 组合约束、`/debug/metrics` 风险计数和 JWKS public-only / RS256 static key ring / one-shot rotate operator 边界：HS256 对称密钥不再通过 identity JWKS 暴露，RS256 key ring 只允许一个当前私钥签名和旧公钥 overlap，RSA 私钥至少 2048 bit，旧公钥 JWK 禁止 `k/d/p/q/dp/dq/qi/oth` 私钥材料；`gateway-token-keyring-rotate` 只更新本地 secret-bearing keyring 文件，不做 KMS/HSM 或跨主机分发。KMS/HSM 自动密钥管理、多 issuer 治理、真实 email/SMS provider 模板 / bounce、完整 account-enumeration/timing 防护、WebAuthn、OIDC federation、IP / device / tenant 自适应风控仍是后续项。
  ...
  补充：identity-service recovery-code 登录 / Refresh proof 风控已从 TOTP factor 风控中拆出独立配置：`NEXUSIM_IDENTITY_MFA_RECOVERY_MAX_FAILED_ATTEMPTS`、`NEXUSIM_IDENTITY_MFA_RECOVERY_FAILURE_WINDOW`、`NEXUSIM_IDENTITY_MFA_RECOVERY_LOCK_DURATION`；未配置时继续继承现有 `NEXUSIM_IDENTITY_MFA_*`，保持本地 smoke 和旧部署兼容。TOTP / recovery-code 成功 Login / Refresh proof 在最终 PostgreSQL 事务内会重新锁定对应 factor 或 user row 并检查 `login_locked_until` / `mfa_recovery_locked_until`，锁定期间不会消费 proof、写 session 或轮换 refresh token。Refresh 中提交 recovery-code proof 会填补缺失 session proof，但不会把已有 TOTP session proof 降级为 `RECOVERY_CODE`。
- 客户端 UI 或最小端到端演示。
