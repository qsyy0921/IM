...
func runRuntime(enableWS bool, enableDeliveryConsumer bool, enableIdentityConsumer bool) error {
	...
	if routeBackend == "redis" {
		...
		routeConfig := redisroute.Config{
			...
			ConversationSignalPolicy:  conversationSignalPolicy,
			ConversationRouteCacheTTL: envDuration("NEXUSIM_PUSH_CONVERSATION_ROUTE_CACHE_TTL", 250*time.Millisecond),
		}
		...
		if enableWS {
			redisSubscriber = redisroute.NewSubscriber(localRegistry, redisClient, redisroute.SubscriberConfig{
				...
				SignalFanoutWorkers:   envInt("NEXUSIM_PUSH_REDIS_SUBSCRIBER_SIGNAL_WORKERS", 4),
				SignalFanoutQueueSize: envInt("NEXUSIM_PUSH_REDIS_SUBSCRIBER_SIGNAL_QUEUE_SIZE", 4096),
				Logf:                  log.Printf,
			})
			go func() {
...
func envDurationAllowZero(name string, defaultValue time.Duration) time.Duration {
	value := strings.TrimSpace(os.Getenv(name))
	if value == "" {
		return defaultValue
	}
