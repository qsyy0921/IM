...
5. api-gateway 本地 Prometheus / Grafana 告警面已继续补强；下一轮如仍做 api-gateway，应聚焦 legacy quiet-window 真实观察或配置中心 quota 边界，否则按 `identity-service -> message-service -> conversation-service -> delivery-service -> push-gateway -> receipt-service -> contacts-service -> policy-service` 继续清 P2。
