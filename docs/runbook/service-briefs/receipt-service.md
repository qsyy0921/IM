...
- 已支持 archived-only、unread、pinned、muted-only、exclude-muted、legacy 单标签 `tag_filter`、多标签 all-match `tag_filters[]`、draft-only、`last_source_event_type` 过滤，`UPDATED_AT` / `PINNED_UPDATED_AT` / `UNREAD_UPDATED_AT` / `DRAFT_UPDATED_AT` 排序，以及 archive / pin / mute / tags / draft 的最小会话列表能力；分页 cursor 会绑定 sort / archived / archived-only / unread / pinned / muted / exclude-muted / tag / multi-tag / draft-only / last-source-event-type 过滤条件和排序边界，避免串页。
...
- 已补只读 `outbox-audit`，以及 `outbox-repair` / 只读 `outbox-repair-audit` / `outbox-repair-cleanup` 运维模式，可直接审计、redrive 和清理 `receipt_outbox` repair 历史；`outbox-repair` 支持 `NEXUSIM_RECEIPT_OUTBOX_REPAIR_REASON_FILE` 读取 repair reason 原文；`outbox-repair-cleanup` 支持 `NEXUSIM_RECEIPT_OUTBOX_REPAIR_CLEANUP_DRY_RUN=true` 只统计候选行不删除，JSON summary 会输出 `dry_run`；`outbox-audit` 支持按 `created_at` RFC3339 时间窗口过滤，并在 JSON 输出当前 compacted filters；`outbox-repair-audit` 支持按 event / tenant / `repaired_at` RFC3339 时间窗口缩小范围并输出 compacted filters；`outbox-audit` / `outbox-repair` / `outbox-repair-audit` / `outbox-repair-cleanup` 可通过 `NEXUSIM_RECEIPT_OUTBOX_AUDIT_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_AUDIT_OUTPUT` / `NEXUSIM_RECEIPT_OUTBOX_REPAIR_CLEANUP_OUTPUT` 写低敏 JSON 结果或 cleanup summary，便于 operator 留存证据。
...
- PostgreSQL repository 已把会话列表、会话偏好、分页 cursor、delivery projection、MarkRead cursor 和 receipt outbox 写入逻辑拆到独立同 package 文件，主 repository 文件已收敛到约 400 行；仓储集成测试也已按会话列表主题拆分，避免 receipt projection / conversation summary 继续堆进单个大文件。
