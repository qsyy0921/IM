...
func run(args []string, getenv func(string) string) error {
	...
	if cfg.ServiceMetricsURL != "" {
		...
		applyLatency(
			&result.RepositoryPoolAcquireRecentMS,
			&result.RepositoryPoolAcquireRecentP95MS,
	...
	if cfg.RelayMetricsURL != "" {
		...
		applyValue(
			&result.KafkaPublishRecordsPerCallRecent,
			&result.KafkaPublishRecordsPerCallRecentP95,
		...
		applyLatency(
			&result.OutboxProcessReadyActiveRecentMS,
			&result.OutboxProcessReadyActiveRecentP95MS,
		...
		applyValue(
			&result.OutboxFetchedPerCallRecent,
			&result.OutboxFetchedPerCallRecentP95,
...
func latencyMetrics(snapshot metricsSnapshot) map[string]latencySnapshot {
	...
	addLatency(metrics, "repository_pool_acquire_recent_latency_ms", snapshot.RepositoryPoolAcquireRecentLatencyMS)
	...
	addLatency(metrics, "outbox_process_ready_active_recent_latency_ms", snapshot.OutboxProcessReadyActiveRecentLatencyMS)
...
func valueMetrics(snapshot metricsSnapshot) map[string]valueSnapshot {
	...
	addValue(metrics, "kafka_publish_records_per_call_recent", snapshot.KafkaPublishRecordsPerCallRecent)
	addValue(metrics, "outbox_fetched_per_call", snapshot.OutboxFetchedPerCall)
	addValue(metrics, "outbox_fetched_per_call_recent", snapshot.OutboxFetchedPerCallRecent)
	if len(metrics) == 0 {
		return nil
	}
	return metrics
